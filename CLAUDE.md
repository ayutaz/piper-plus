...
## 🚀 現在の状態: 6言語マルチリンガル対応 + つくよみちゃん6langファインチューニング完了

**ブランチ**: `feat/multilingual-phonemizer`
...
18. ✅ **つくよみちゃん v4 emb-lang-fix 再FT完了**: 500 epoch / --resume-from-multispeaker-checkpoint / --freeze-dp 自動有効化 (2026-03-05) — emb_lang[1] EN 保持版、後処理で JA→EN コピー、JA 1.23s / EN 0.70s / 混合 0.71s
19. **HuggingFace アップロード**: v4 バイリンガル + つくよみちゃん v4 + つくよみちゃん v4 emb-lang-fix をアップロード
...
**`--language-balanced-sampling` は自動有効化**: 6lang/v4 ともに話者数比 ≥ 3:1 閾値により自動的に有効化される。明示指定も可能だが不要。
...
| **emb-lang-fix (500ep)** | **1.23s** | `--resume-from-multispeaker-checkpoint` + 後処理 emb_lang コピー |
| **freeze-dp (500ep)** | **1.76s** | DP 凍結で劣化を抑制 |
| no-freeze (500ep, 旧) | 1.11s | DP 崩壊で早送り |

- 凍結なし (1.11s) → 凍結あり (1.76s) で **59% 改善**
- emb-lang-fix (1.23s) は `--resume-from-multispeaker-checkpoint` による自動変換方式（手動チェックポイント変換不要）
...
### つくよみちゃん v4 emb-lang-fix 再ファインチューニング (2026-03-05 完了)

v4バイリンガルモデルから `--resume-from-multispeaker-checkpoint` で転移学習を再実行。emb_lang[1] EN 保持版。

**2段階方式:**
1. **学習時**: `--resume-from-multispeaker-checkpoint` で emb_lang[1] (EN) を元の embedding + emb_g_mean 補正のまま保持。凍結された DP が正しい EN conditioning を受け取れるようにする。`--freeze-dp` は自動有効化。
2. **後処理**: ONNX エクスポート前に `emb_lang[0]` (JA=つくよみちゃん) → `emb_lang[1]` (EN) にコピーして声質を統一。cosine similarity: 0.34 → 1.0 に修正。
...
nohup /data/piper/.venv/bin/python -m piper_train \
  ...
  --resume-from-multispeaker-checkpoint \
    /data/piper/output-bilingual-ja-en-v4/lightning_logs/version_0/checkpoints/epoch=149-step=269700.ckpt \
  --default_root_dir /data/piper/output-tsukuyomi-finetune-v4-emb-lang-fix \
  > /data/piper/training_tsukuyomi_v4_emb_lang_fix.log 2>&1 &
```
...
| テキスト | 音声長 |
|---------|--------|
| JA「こんにちは、つくよみちゃんです。」 | 1.23s |
| EN "Hello, how are you today?" | 0.70s |
| 混合「今日はgood morningですね」 | 0.71s |

**生成モデル:** `/data/piper/output-tsukuyomi-finetune-v4-emb-lang-fix/tsukuyomi-v4-emb-lang-fix.onnx`

**チェックポイント:** `output-tsukuyomi-finetune-v4-emb-lang-fix/lightning_logs/version_0/checkpoints/epoch=499-step=22000.ckpt`
...
### 多言語 Phonemizer (6言語) ✅ NEW (2026-03-07)

`BilingualPhonemizer` を一般化した N言語対応版。`UnicodeLanguageDetector` で言語を自動検出し、各言語の Phonemizer に委譲。

**対応言語:**

| 言語 | コード | Phonemizer | 依存 |
|------|--------|------------|------|
| 日本語 | ja | JapanesePhonemizer (既存) | pyopenjtalk |
| 英語 | en | EnglishPhonemizer (既存) | g2p-en |
| 中国語 | zh | ChinesePhonemizer (新規) | pypinyin (MIT) |
| 韓国語 | ko | KoreanPhonemizer (新規) | g2pk2 (Apache-2.0), optional |
| スペイン語 | es | SpanishPhonemizer (新規) | 規則ベース (依存なし) |
| ポルトガル語 | pt | PortuguesePhonemizer (新規) | 規則ベース (依存なし) |
| フランス語 | fr | FrenchPhonemizer (新規) | 規則ベース (依存なし) |

**新規ファイル:**
- `src/python/piper_train/phonemize/multilingual.py` — `MultilingualPhonemizer`
- `src/python/piper_train/phonemize/multilingual_id_map.py` — 統一IDマップ生成
- `src/python/piper_train/phonemize/chinese.py` — 中国語 Phonemizer
- `src/python/piper_train/phonemize/korean.py` — 韓国語 Phonemizer
- `src/python/piper_train/phonemize/spanish.py` — スペイン語 Phonemizer
- `src/python/piper_train/phonemize/portuguese.py` — ポルトガル語 Phonemizer
- `src/python/piper_train/phonemize/french.py` — フランス語 Phonemizer
- `src/python/piper_train/phonemize/zh_id_map.py` — 中国語音素インベントリ
- `src/python/piper_train/phonemize/ko_id_map.py` — 韓国語音素インベントリ
- `src/python/piper_train/phonemize/es_id_map.py` — スペイン語音素インベントリ
- `src/python/piper_train/phonemize/pt_id_map.py` — ポルトガル語音素インベントリ
- `src/python/piper_train/phonemize/fr_id_map.py` — フランス語音素インベントリ

### SpeakerBalancedBatchSampler __len__ 修正 ✅ NEW (2026-03-07)

`__len__` を `min(speaker_utterances)` ベースから `sum(usable_samples)` ベースに修正。6言語環境での `__len__` が実際の `__iter__` と 0.05% 以内で一致するようになった。

**実装ファイル:**
- `src/python/piper_train/vits/dataset.py` — `SpeakerBalancedBatchSampler.__len__()` 修正

### バイリンガル (JA+EN) Phonemizer ✅ (2026-02-02)

文内コードスイッチング対応のバイリンガル音素化。Unicode範囲で言語セグメントを自動検出し、各言語のPhonemizerに委譲。多言語 Phonemizer の前身。
...
**推奨ワークフロー — 2段階方式 (2026-03-05 確立):**
1. **学習時**: `--resume-from-multispeaker-checkpoint` で emb_lang[1] (EN) を元の embedding + emb_g_mean 補正のまま保持。凍結された DP が正しい EN conditioning を受け取れる。
2. **後処理 (ONNX エクスポート前)**: 学習完了後に `emb_lang[0]` (JA=ファインチューニング済み) → `emb_lang[1]` (EN) にコピーして声質を統一。これにより EN テキスト発話時もファインチューニング済み話者の声質で合成される。
