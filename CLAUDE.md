...
## 🚀 現在の状態: VITS2 FT品質検証完了 — VITS1が現時点でベスト

**ブランチ**: `feat/vits2-upgrade` (VITS2実装、devからの派生、最新devリベース済み)

### 現在の状況 (2026-04-05)

**VITS2 FT実験の結論:** 100発話シングルスピーカーFTではVITS1の品質に到達できていない。Feature E (Mel Posterior Encoder) の個別検証でも深刻な品質劣化を確認。詳細は `docs/vits2-ft-experiments.md` を参照。

**完了:**
- VITS2 つくよみちゃんFT実験 gin256 v1-v5, gin512 v1-v3 全完了
- ベースモデル構成変更実験 B1, B2 完了（gin256+linspec, gin512+linspec）
- **T-E個別検証 (VITS1 + Mel Posterior Encoder)** — mid -25dB, hi -39dBで**NG確定**
- **v4「gin256容量不足」は誤診** — 真の原因は `speaker_conditioned_encoder` のsilent drop
- gin512 ベースモデルは高周波欠損 (conditioning/acoustic比率2x過剰)
- gin256 v5 FTで周波数特性はVITS1同等に到達、だが速度・声質で劣る
- TextEncoder cond_proj バグ発見・修正 (models.py:218)
- gin_channels自動検出修正 (__main__.py) — ベースcheckpointから継承
- CI修正 (F811重複メソッド) プッシュ済み

**発見したバグ:**
1. **TextEncoder cond_proj未実行** — `self.encoder(x, x_mask)` が `g` を渡していなかった (gin256 FTのためrevert済み)
2. **gin_channels自動設定** — 常に512に設定されていたのをベースcheckpointから継承に修正
3. **gin256 v4の誤診** — 「容量不足」ではなく `speaker_conditioned_encoder=False` による cond_layer/cond_proj silent drop

**VITS2固有機能の個別検証結果:**
| 機能 | 検証 | 結果 |
|------|------|------|
| E: Mel Posterior Encoder | T-E (VITS1+Eのみ, 20ep) | **❌ NG** — 全帯域で深刻な劣化 (mid -25dB, hi -39dB) |
| A: Deterministic DP | 未実施 | — |
| B: Duration Discriminator | 未実施 | — |
| C: Speaker-Conditioned Enc | 未実施 | — |
| D: Noise-Scaled MAS | 未実施 | — |

**次にやること:**
1. **残りのVITS2機能個別検証 (T1-T4)** — A〜Dの影響を個別測定（Eがダメだったので残りも要確認）
2. **品質がVITS1を超えたら feat/vits2-upgrade → dev マージ** — PR作成
3. **全機能がVITS1以下** → VITS2はベースモデル事前学習用として提供、FTはVITS1方式を推奨

### VITS2 FTパイプライン改善点 (全実験から判明)

| 問題 | 原因 | 対策 | 検証状況 |
|------|------|------|---------|
| Duration 30-130%長い | frozen deterministic DPの特性 | DP unfreeze + DurDisc + dp-lr | v4で失敗 (speaker_cond_enc dropが原因)、v6で再テスト予定 |
| 外国人っぽいJA発音 | emb_g_mean が全571話者の平均 | `--keep-emb-g` + 言語別話者mean初期化 | v3→v4で修正済み |
| ノイズ (v4) | ~~gin256容量不足~~ → **speaker_cond_enc silent drop** | `--speaker-conditioned-encoder` を**保持** | v5で修正確認済み |
| gin512 水中感 | デコーダ conditioning/acoustic比率 2x過剰 | g*0.5で周波数回復するが声質不自然 | gin512はFTに不適 |
| gin512 高周波欠損 | CosineAnnealingLR + 大パラメータ数 = 有効学習不足 | 200ep再学習 → 改善不十分 | 根本的にconditioning比率の問題 |

**重要な注意事項:**
- `--speaker-conditioned-encoder` はベースモデルと**必ず一致**させること。外すとcond_layer/cond_projがsilent dropされる
- gin_channels はベースcheckpointから自動継承される (`--resume-from-multispeaker-checkpoint` 使用時)
- gin512はベースモデル品質に問題があり、FTには gin256 を推奨
- GPUドライバがNVMLエラーになった場合はPod再起動が必要
...
## VITS2 6言語学習 gin_channels=512 (2026-03-25 完了)

VITS2アーキテクチャ + gin_channels=512で6言語ベースモデルを学習。gin_channels=256版ではFT時に滑舌・ノイズが劣化したため512に変更。

### VITS2 アーキテクチャ変更点

| 項目 | VITS1 | VITS2 (gin=256, 旧) | VITS2 (gin=512, 現在) |
|------|-------|---------------------|----------------------|
| Posterior Encoder入力 | Linear Spec (513ch) | Mel Spec (80ch) | Mel Spec (80ch) |
| MAS | 通常 | Noise-Scaled | Noise-Scaled |
| Duration Predictor | Stochastic (SDP) | Deterministic (DP) | Deterministic (DP) |
| Duration判別器 | なし | Duration Discriminator V2 | Duration Discriminator V2 |
| TextEncoder | 通常 | Speaker-Conditioned | Speaker-Conditioned |
| gin_channels | 512 | 256 | **512** |
| LRスケジューラ | ExponentialLR | CosineAnnealingLR + Warmup | CosineAnnealingLR + Warmup |
| Optimizer数 | 2 (G, D) | 3 (G, D, DurDisc) | 3 (G, D, DurDisc) |
...
| 学習時間 | ~28時間 (4x V100) |
| batch_size | 24 |
| チェックポイント | `output-vits2-6lang-gin512/checkpoints/epoch=59-step=198999.ckpt` |
| パラメータ数 | 77.1M (G 29.7M + D 46.7M + DurDisc 654K) |

### VITS2 gin256 学習結果 (旧版、参考)
...
| **FT結果** | **gin_channels=256ではFT時に滑舌・ノイズ劣化 → gin512に移行** |
...
### Template D: VITS2 シングルスピーカー ファインチューニング (改善版)
...
nohup uv run python -m piper_train \
  ...
  --mas-noise-start 0 \
  --mel-posterior-encoder \
  --no-sdp --use-duration-discriminator --no-freeze-dp \
  --keep-emb-g \
...
**Template D 改善点 (gin=256実験から判明):**
- `--dp-lr 1e-4`: DP専用高LR (base_lr×5) でDP適応を加速
- `--warmup-epochs 3 --cosine-scheduler`: 3-optimizer安定化
- `--mas-noise-start 0`: FTではMAS noiseを無効化
- `--no-sdp --use-duration-discriminator --no-freeze-dp`: DP unfroze + DurDisc監視
- `--keep-emb-g`: frozen emb_g保持 (言語別話者mean初期化)
- ~~`--speaker-conditioned-encoder` を削除: 単一話者FTでは不要~~ → **必ず保持すること** (外すとcond_layer/cond_projがsilent dropされノイズ発生。v4失敗の根本原因)
- `CUDNN_BENCHMARK=0 TORCH_CUDNN_V8_API_DISABLED=1`: cuDNNクラッシュ回避
...
| 用途 | パス | 状態 |
|------|------|------|
| **つくよみちゃん 6lang-v2 (VITS1)** | `/data/piper/output-tsukuyomi-finetune-6lang-v2/tsukuyomi-6lang-v2-fixed.onnx` | 500 epoch完了 (2026-03-16) -- emb_lang後処理済み、全6言語テスト成功、**現時点でベスト品質** |
| **多言語 6lang ベースモデル (VITS1)** | `/data/piper/output-multilingual-6lang/` | 75 epoch完了 (2026-03-16) -- epoch=74-step=504712.ckpt、571話者 |
| つくよみちゃん VITS2 gin256 v5 | `/data/piper/output-tsukuyomi-finetune-vits2-gin256/tsukuyomi-vits2-gin256.onnx` | 500 epoch完了 (2026-03-30) -- 33.4MB FP16、周波数はVITS1同等だが速度・声質で劣る |
| VITS2 gin256 ベースモデル | `/data/piper/output-vits2-6lang/vits2-6lang-60epoch.onnx` | 60 epoch完了 (2026-03-20) -- 34MB FP16、571話者、cond/conv_pre比=2.67 (正常) |
| VITS2 gin512 v2 ベースモデル | `/data/piper/output-vits2-6lang-gin512-v2/vits2-gin512-200ep.onnx` | 200 epoch完了 (2026-03-30) -- 37.4MB FP16、高周波欠損あり (cond/conv_pre比=6.16, FT非推奨) |
| VITS2 gin512 v1 ベースモデル | `/data/piper/output-vits2-6lang-gin512/` | 60 epoch完了 (2026-03-25) -- 高周波欠損あり (FT非推奨) |
| T-E: VITS1+MelPE (20ep) | `/data/piper/output-vits1-mel-pe-test/vits1-mel-pe-20ep.onnx` | 20 epoch完了 (2026-04-05) -- **❌ NG** mid -25dB, hi -39dB, Feature E単体でも品質劣化 |
