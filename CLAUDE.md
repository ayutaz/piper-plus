...
スタイルベクトル条件付けにより、感情 / 表現スタイルを直接モデルに注入可能。既存モデルは `--style-vector-dim 0` (既定) で完全後方互換。Fork `yusuke-ai/piper-plus` コミット `314b3355` をベースに取り込み (PE-A emotion loss は Phase 4 で別途取り込み予定)。
