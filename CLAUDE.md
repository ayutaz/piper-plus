...
## 🚀 現在の状態: v3 ONNX完了・つくよみちゃん v3ファインチューニング完了 → HuggingFace アップロード待ち
...
10. **HuggingFace アップロード**: v3 バイリンガル + つくよみちゃん v3 をアップロード
11. **不要チェックポイント削除**: `output-bilingual-ja-en/lightning_logs/version_28/checkpoints/` に全200エポック分 (~176GB) が残存。`epoch=199` と `last.ckpt` 以外は削除してディスク節約可能
