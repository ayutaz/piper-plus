...
スタイルベクトル条件付けにより、感情 / 表現スタイルを直接モデルに注入可能。既存モデルは `--style-vector-dim 0` (既定) で完全後方互換。Fork `yusuke-ai/piper-plus` コミット `314b3355` をベースに取り込み。PE-A emotion loss (CLI `--pea-emotion-*`) と style bank ツール (`build_pea_style_bank.py`) も併せて統合済み。

| モード | 注入箇所 | 要件 |
|--------|---------|------|
| `global` (既定) | VITS 大域条件 `g` に加算 (decoder / flow / duration predictor に伝播) | `gin_channels > 0` |
| `text` | TextEncoder の scaled embedding に加算 | 制約なし |

**学習時 CLI オプション:**
- `--style-vector-dim N` (デフォルト: 0): スタイルベクトル次元。0 で無効化 (既存モデルと bit-for-bit 一致)
- `--style-condition-dropout FLOAT` (デフォルト: 0.0): 学習時に style を確率的に zero-mask
- `--style-condition-mode STR` (デフォルト: "global", choices=[global, text]): 注入モード
- `--load_weights_from_checkpoint PATH` (shape-aware partial load): 既存チェックポイントから style_proj 付きモデルへ fine-tune する際に利用。shape 不一致 tensor は WARNING 付きで skip

**推論時 JSONL フィールド (`infer.py`):**
- `style_vector`: inline リスト (例 `[0.1, 0.2, ...]`)
- `style_vector_path`: `.npy` / `.pt` / `.pth` へのパス
- `--device` CLI オプション追加 (cuda/cpu)

**実装:**
- `vits/models.py` (TextEncoder / SynthesizerTrn 拡張、style_proj 最終層を zero 初期化)
- `vits/dataset.py` (Utterance.style_vector_path / emotion、Batch.style_vectors / emotions、BatchCollator で事前割当 + slice-copy)
- `vits/lightning.py` (hparams + batch.style_vectors → model_g.forward / model_g.infer 伝播)
- `vits/commons.py` (slice_segments を `list(x.shape)` ベースに N-D 一般化)
- `__main__.py` (argparse 4 オプション + `load_checkpoint_weights()` 実装)
- `infer.py` (`_style_vector_to_tensor()` helper、`--device` 対応)

**テスト:**
- `src/python/tests/test_style_vector_conditioning.py` (8 件)
- `src/python/tests/test_load_weights_from_checkpoint.py` (3 件)
...
| Style Vector テスト | `src/python/tests/test_style_vector_conditioning.py` |
| Load Weights テスト | `src/python/tests/test_load_weights_from_checkpoint.py` |
