...
## 🟡 現在の状態: バイリンガル (JA+EN) モデル — WavLM無効、0 epochから再学習

**ブランチ**: `feat/bilingual-phonemizer`

### バイリンガルモデル学習状況 (2026-02-11 更新)

| 項目 | 値 |
|------|-----|
| エポック | **0** / 200 — WavLM無効、0 epochから再学習中 |
...
**WavLM無効化の経緯 (2026-02-11):** WavLM有効時の学習速度が 0.03 it/s と極めて遅く (JA-only比 3-5倍)、完了まで推定19日 ($6,300) かかる見込みだった。WavLM無効にすることで推定11日 ($3,800) に短縮。JA-only完了モデルもWavLMなしで学習されており、同条件で品質比較可能。`--no-wavlm` フラグを新規追加し対応。
...
### 学習開始コマンド (0→200 epoch、WavLM無効、修正済みデータセット)
...
**注意:** `--no-wavlm` でWavLM Discriminatorを無効化し、batch-size 20で学習。`--max-phoneme-ids 400` で長いシーケンス (109件, 0.15%) を除外しメモリスパイクを防止。
...
**Phase C 状況:** データセット再生成完了。WavLM無効 (`--no-wavlm`)、batch-size 20 で 0 epochから再学習中
...
| 用途 | パス | 状態 |
|------|------|------|
| **バイリンガル JA+EN** | `/data/piper/output-bilingual-ja-en/` | 🟡 WavLM無効 (`--no-wavlm`)、batch-size 20、0 epochから再学習中 |
