...
- **SSML 基本サポート** (4 ランタイム + WASM 解析 API) — `<speak>`, `<break>`, `<prosody rate>` を Python/Rust/C#/Go で実装 (W3C サブセット)。実装: `g2p/piper_plus_g2p/ssml.py`, `piper-core/src/ssml.rs`, `PiperPlus.Core/Ssml/SsmlParser.cs`, `go/piperplus/ssml/parser.go`。WASM/JS は piper-wasm の `isSsml` / `parseSsml` で同パーサーを expose (TTS 統合は openjtalk-web の synthesize 側で iterate して silence 挿入 + length_scale 切替)。
