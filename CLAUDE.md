...
12. **v4 学習開始**: `--language-balanced-sampling` 自動有効化（話者比 15.5:1 ≥ 3.0 閾値）、350 epoch 予定
...
**注意:** `--language-balanced-sampling` は話者数比 15.5:1 により自動有効化される（明示指定も可）。v4 は EN 最小話者が 180 発話のため v3 (16発話) よりバッチ効率が良い。`--max_epochs 350` は v3 と同設定。
