...
1. **英語データ増強**: 現在EN比率18% (13,000/73,148) で英語の発音が不明瞭。LibriTTS / VCTK / Hi-Fi TTS等の追加で改善可能
2. **不要チェックポイント削除**: `output-bilingual-ja-en/lightning_logs/version_28/checkpoints/` に全200エポック分 (~176GB) が残存。`epoch=199` と `last.ckpt` 以外は削除してディスク節約可能
3. **HuggingFaceアップロード**: バイリンガルモデル (ONNX + config.json) の公開

**注意:** WavLM Discriminatorは事前学習では不要。ファインチューニング時に検討する可能性はあるが、事前学習フェーズでは `--no-wavlm` で十分。
