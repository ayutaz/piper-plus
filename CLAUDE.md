...
- **SSML 基本サポート** (4 ランタイム) — `<speak>`, `<break>`, `<prosody rate>` を Python/Rust/C#/Go で実装 (W3C サブセット)。実装: `g2p/piper_plus_g2p/ssml.py`, `piper-core/src/ssml.rs`, `PiperPlus.Core/Ssml/SsmlParser.cs`, `go/piperplus/ssml/parser.go`。
...
| iOS Swift G2P (SPM) | `PiperPlusG2P` (Swift product, 別 xcframework) | 1.14.0+ | XCTest (CI) | `Sources/PiperPlusG2P/`, `Tests/PiperPlusG2PTests/`, `Package.swift` (Issue #387) |
