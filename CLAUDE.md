...
## 🚀 現在の状態: 6言語マルチリンガル対応 (コード: 7言語対応、sv含む)

> **Note:** G2Pコードは7言語 (JA/EN/ZH/ES/FR/PT/SV) に対応済み。学習済みモデルは6言語 (SV未学習)。
...
> **Note:** スウェーデン語 (sv) はG2Pコード実装済みだが、学習済みモデルには未含有 (sv=6)。
...
7言語マルチリンガル対応のクロスプラットフォーム .NET CLI。Python実装と同等の音素化パイプラインを C# で再実装。

| 項目 | 詳細 |
|------|------|
| TFM | PiperPlus.Core: net8.0, PiperPlus.Cli: net9.0 |
| 対応言語 | JA, EN, ZH, ES, FR, PT, SV (7言語) |
...
| バージョン | 0.1.1 |
| 対応言語 | JA, EN, ZH, ES, FR, PT, SV (7言語) |
| 音素化 | OpenJTalk WASM (JA), SimpleEnglishPhonemizer (EN), キャラクタベース (ZH/ES/FR/PT/SV) |
...
- `SimpleUnifiedPhonemizer` — 7言語音素化 (eSpeak-ng 不使用)
