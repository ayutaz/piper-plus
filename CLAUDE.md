...
1. **⚠️ Pod再起動**: GPU (`/dev/nvidia*`) にアクセスできない状態。Kubernetes PodのGPUリソース割り当てが切れている。Pod再起動で復旧する見込み
2. **GPU動作確認**: 再起動後に `nvidia-smi` と `torch.cuda.is_available()` を確認
3. **1 epoch検証学習**: enhanced-fixedデータセットで1 epochテスト実行（下記コマンド）
4. **本学習 (200 epoch)**: 検証OKなら本学習開始
