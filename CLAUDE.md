...
11. **v4 学習開始**: `--language-balanced-sampling` 必須、350 epoch 予定
12. **v4 学習完了後**: ONNX 変換 + 推論テスト
13. **つくよみちゃん v4ベースファインチューニング**: v4 モデルをベースに転移学習
14. **HuggingFace アップロード**: v4 バイリンガル + つくよみちゃん v4 をアップロード
15. **不要チェックポイント削除**: `output-bilingual-ja-en/lightning_logs/version_28/checkpoints/` に全200エポック分 (~176GB) が残存。`epoch=199` と `last.ckpt` 以外は削除してディスク節約可能
...
| JA:EN バッチ比 (補正後) | 1:1 | 1:1 | 1:1 | **1:1 (language-balanced-sampling 必須)** |
| Fix A/B (言語条件) | ❌ | ✅ | ✅ 継承 | ✅ 継承 |
| 学習期間 | - | 4h19分 (200ep) | ~3日 (350ep) | **未学習** |
...
**⚠️ `--language-balanced-sampling` が必須**: v4 も EN 310 話者 vs JA 20 話者のため、なしでは JA がバッチの約 6% しか学習されない。
...
**注意:** `--language-balanced-sampling` で JA:EN = 50:50 を強制。v4 は EN 最小話者が 180 発話のため v3 (16発話) よりバッチ効率が良い。`--max_epochs 350` は v3 と同設定。
...
**問題:** v3/v4 では `--language-balanced-sampling` なしで JA がバッチの数% しか学習されない（v2 は 50:50 で問題なし）。
