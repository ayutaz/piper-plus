...
12. **v4 学習開始**: `--language-balanced-sampling` 必須、350 epoch 予定
13. **v4 学習完了後**: ONNX 変換 + 推論テスト
14. **つくよみちゃん v4ベースファインチューニング**: v4 モデルをベースに転移学習
15. **HuggingFace アップロード**: v4 バイリンガル + つくよみちゃん v4 をアップロード
16. **不要チェックポイント削除**: `output-bilingual-ja-en/lightning_logs/version_28/checkpoints/` に全200エポック分 (~176GB) が残存。`epoch=199` と `last.ckpt` 以外は削除してディスク節約可能
