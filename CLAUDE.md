...
**VITS2 FT実験の結論:** 100発話シングルスピーカーFTではVITS1の品質に到達できていない。詳細は `docs/vits2-ft-experiments.md` を参照。
...
**次にやること:**
1. **VITS2 gin256 v6 FT** — speaker_cond_enc=True + no-freeze-dp + dp-lr=5e-5 + DurDisc (未テスト構成)
   - v4の失敗原因(speaker_cond_enc drop)を修正済みでDP unfreezeすればduration改善の可能性
2. **品質がVITS1を超えたら feat/vits2-upgrade → dev マージ** — PR作成
3. **品質がVITS1以下のまま** → VITS2はベースモデル事前学習用として提供、FTはVITS1方式を推奨
