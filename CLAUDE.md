...
- `src/python/tests/test_speaker_balanced_sampler.py` - テストコード（GitHub Actions実行）
...
piper_trainはマルチGPU学習時に**自動学習率スケーリング**を適用します。

**計算式:**
```
effective_batch_size = batch_size × num_gpus
learning_rate = base_lr × (effective_batch_size / 16)
```

**例:**
| 設定 | batch_size | GPUs | effective_batch | base_lr | 実際のlr |
|------|-----------|------|-----------------|---------|----------|
| 5話者（成功） | 5 | 4 | 20 | 2e-4 | **0.00025** |
| 20話者 batch=5 | 5 | 4 | 20 | 2e-4 | 0.00225* |
| 20話者 batch=20 | 20 | 4 | 80 | 2e-4 | **0.001** |
| 20話者 batch=32 | 32 | 4 | 128 | 1e-4 | **0.0008** |

*注: batch=5の20話者モデルは別の設定でスケーリングが異なった

### 20話者モデル失敗の原因（学習率観点）

| モデル | 実際のlearning_rate | 結果 |
|--------|-------------------|------|
| 5話者（成功） | 0.00025 | ✅ Duration Predictor正常 |
| 20話者 batch=5 | ~0.00225 | ❌ 9x高い → 崩壊 |
| 20話者 batch=20 | 0.001 | ❌ 4x高い → 崩壊 |
| 20話者 batch=32 | 0.0008 | ❌ 3.2x高い → 崩壊 |
...
for sid in [0, 5, 10, 15, 19]:
    with torch.no_grad():
        ...
        g = model.model_g.emb_g(speaker).unsqueeze(-1)
        logw = model.model_g.dp(x, x_mask, g=g, reverse=True, noise_scale=0.8)
        w = torch.exp(logw) * x_mask
        print(f'Speaker {sid}: {w.sum().item():.1f} frames')
"
```
...
### Duration Predictor崩壊問題（2025-12-08 調査完了）

**原因**:
- 初期化時点では両モデルとも~30フレーム予測（差なし）
- 1エポック後: 5話者は58-131フレームに増加 ✅、20話者は16フレームに減少 ❌
- 話者数増加によるバッチ内多様性の上昇が原因

**解決策**:
1. ✅ **SpeakerBalancedBatchSampler** - 実装完了、効果あり
2. ✅ **学習率1e-4** - Duration Predictor崩壊を防止
3. 🔴 **学習ステップ数増加** - 200エポック以上が必要（現在対応中）
