...
def benchmark_texts(piper_path: str, model_path: str, texts: List[str]) -> Dict:
    ...
    for i, text in enumerate(texts):
        ...
        for _ in range(3):  # Average of 3 runs
            latency = measure_first_byte_latency(piper_path, model_path, text, use_raw=True)
            raw_latencies.append(latency)
        
        # Measure first byte latency (WAV mode)
        wav_latencies = []
        for _ in range(3):
            latency = measure_first_byte_latency(piper_path, model_path, text, use_raw=False)
        ...
        result = {
            ...
            "raw_latency_std": statistics.stdev(raw_latencies) if len(raw_latencies) > 1 else 0,
            "wav_latency_avg": statistics.mean(wav_latencies),
            "wav_latency_std": statistics.stdev(wav_latencies) if len(wav_latencies) > 1 else 0,
        ...
        print(f"  Raw mode latency: {result['raw_latency_avg']*1000:.1f} ± {result['raw_latency_std']*1000:.1f} ms")
        print(f"  WAV mode latency: {result['wav_latency_avg']*1000:.1f} ± {result['wav_latency_std']*1000:.1f} ms")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Piper streaming performance")
    parser.add_argument("--model", required=True, help="Path to ONNX model")
    parser.add_argument("--output", help="Output JSON file for results")
    parser.add_argument("--japanese", action="store_true", help="Include Japanese test texts")
    ...
    print(f"\nLatency range:")
    print(f"  Raw mode: {min(all_raw_latencies)*1000:.1f} - {max(all_raw_latencies)*1000:.1f} ms")
    print(f"  WAV mode: {min(all_wav_latencies)*1000:.1f} - {max(all_wav_latencies)*1000:.1f} ms")
