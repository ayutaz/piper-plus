...
def main():
    ...
    test_cases = [
        ...
        ("Long text", "In recent years, text-to-speech technology has advanced significantly, enabling more natural and expressive synthetic voices that can be used in various applications."),
    ]
    ...
    for model_name, model_path in models:
        ...
        for text_name, text in test_cases:
            ...
            if regular_latencies and streaming_latencies:
                ...
                reg_std = statistics.stdev(regular_latencies) if len(regular_latencies) > 1 else 0
                stream_avg = statistics.mean(streaming_latencies)
                stream_std = statistics.stdev(streaming_latencies) if len(streaming_latencies) > 1 else 0
