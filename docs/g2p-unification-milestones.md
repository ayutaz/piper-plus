...
|---|---|---|
| Python | `piper_train/phonemize/` (24 ファイル) | `piper_plus_g2p` (19 ファイル) |
...
- **修正**: 約 10 行追加。`CustomDictionary` は既に `piper_plus_g2p/custom_dict.py` に存在する
- **パターン**:
  ```python
...
## M1: Python 移行 — piper_train が piper_plus_g2p を使う

> **目的**: `piper_train/phonemize/` の全 import を `piper_plus_g2p` に置換し、旧コードを削除
...
|---|---|---|
| `infer_onnx.py:31,62` | `from .phonemize.multilingual import UnicodeLanguageDetector` | `from piper_plus_g2p import UnicodeLanguageDetector` |
| `infer_onnx.py:106` | `from .phonemize.registry import get_phonemizer` | `from piper_plus_g2p import get_phonemizer` |
| `vits/lightning.py:177` | `from ..phonemize.registry import get_phonemizer` | `from piper_plus_g2p import get_phonemizer` |
| `update_model_config.py:16` | `from piper_train.phonemize.token_mapper import FIXED_PUA_MAPPING, TOKEN2CHAR` | `from piper_plus_g2p.encode.pua import FIXED_PUA_MAPPING, TOKEN2CHAR` |
...
移行後: JapanesePhonemizer(custom_dict=...).phonemize_with_prosody(text)
         → piper_plus_g2p は BOS/EOS を埋め込まない (IPA-first)
...
移行後: piper_plus_g2p.MultilingualPhonemizer.phonemize_with_prosody(text)
         → 同様にストリップ
...
2. パス B: `MultilingualPhonemizer` + `post_process_ids()` → `piper_plus_g2p.MultilingualPhonemizer` + `PiperEncoder`
3. トークン → ID 変換ループ (行 737-743, 880-886) を `PiperEncoder.encode_with_prosody()` に統一
4. prosody の `dict` → `ProsodyInfo` 型統一 (piper_plus_g2p は `ProsodyInfo` dataclass を返す)
...
|---|---|---|
| ja | `JapanesePhonemizer::new_bundled()` | `piper_plus_g2p::japanese::JapanesePhonemizer::new_bundled()` |
| en | `EnglishPhonemizer::new_with_dict(&path)` | `piper_plus_g2p::english::EnglishPhonemizer::new_with_dict(&path)` |
| zh | `ChinesePhonemizer::new(&single, &phrases)` | `piper_plus_g2p::chinese::ChinesePhonemizer::new(&single, &phrases)` |
| es/fr/pt/ko/sv | `XxxPhonemizer::new()` | `piper_plus_g2p::xxx::XxxPhonemizer::new()` |
...
- **piper-g2p**: `piper_plus_g2p::encode` に同一関数あり
...
1. **preprocess.py の日本語モノリンガルパス (M1-4)**
   ...
   - piper_plus_g2p は IPA-first で BOS/EOS を埋め込まない
...
4. **BilingualPhonemizer の後方互換 (M1-3)**
   - `get_bilingual_id_map()` は piper_plus_g2p の `get_phoneme_id_map("ja-en")` に対応
...
9. **CustomDictionary**: 両実装で API 同一 (piper_plus_g2p 側に 10MB 制限あり)
