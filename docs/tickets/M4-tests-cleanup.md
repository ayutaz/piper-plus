...
1. **動作保証テスト**: voice なしで正しく動作することを明示的に検証するテストを 6 件追加する。M1-M3 では既存テストの安全網に依存していたが、M4 では「voice がない状態が正常であること」を積極的に断言するネガティブテストを導入する。
...
本チケットは 1 名のエージェントで実施可能。作業量が限定的 (テスト 6 件 + クリーンアップ ~50 行) であり、並行作業の余地はない。

| 役割 | 人数 | 担当 |
|------|------|------|
| 実装担当 | 1 | テスト追加 (C++ 2 件、JS 3 件、shell 1 件) + クリーンアップ (ファイル削除、ドキュメント更新、バージョンバンプ、CHANGELOG) |

**作業順序:**
1. テスト 6 件を追加して GREEN を確認
...
**スコープ内:**
- C++ テスト 2 件追加 (`test_openjtalk_optimized.cpp`)
...
| CI ワークフロー | 期待 |
|---------------|------|
| `_build-test-cpp.yml` | テスト 1, 2 が PASS (またはバイナリ不在時 SKIP) |
...
- [ ] 新規テスト 6 件が M1-M3 の変更内容と整合しているか
...
M4 で追加する 6 件のテストは、テストピラミッドの異なるレイヤーに分散している。ゼロから設計するなら、各テストがどのレイヤーに位置するかを明示的に分類し、カバレッジの偏りを可視化すべきだった:
...
M4 の CHANGELOG と M2 の breaking change 管理は文書レベルの対応に留まっているが、npm エコシステムの標準ワークフローと IndexedDB の実データ残留問題に対処できていない。M2 (6.4, 6.5) で指摘されている IndexedDB キャッシュ破壊と npm deprecation を、M4 のクリーンアップスコープに含めるべきだった:

1. **CHANGELOG に IndexedDB マイグレーション手順を追記する:**
   現在の CHANGELOG (セクション 2.2.4) には Breaking Changes と Removed のみ記載されているが、`0.2.x` で `includeVoice: true` を使っていたユーザーの IndexedDB には `voice/mei_normal.htsvoice` (~50MB) が残留する。以下を Migration セクションとして追記すべき:
   ```markdown
   ### Migration from 0.2.x
   If you previously used `includeVoice: true`, call `dictLoader.clearCache()`
   once after upgrading to free ~50MB of cached voice data from IndexedDB.
   ```

2. **npm publish 手順 (セクション 7) に deprecation ステップを追加する:**
   ```bash
   # M4 マージ後、0.3.0 publish 後に実行
   npm deprecate "@piper-plus/g2p@<0.3.0" \
     "voiceData/includeVoice API removed in 0.3.0. See CHANGELOG for migration."
   ```
   `npm-publish.yml` の `g2p-v*` タグトリガーに post-publish ステップとして組み込むのが理想的。

3. **IndexedDB バージョンの自動マイグレーション:**
   M2 で `DB_VERSION` を `1` -> `2` に上げ、`onupgradeneeded` で voice キャッシュを自動削除するのが最も利用者に優しい。ただしこれは M2 のスコープであり、M4 としては CHANGELOG での手動手順の文書化が最低限の対応となる。

M2 の breaking change 管理 (6.5) と M4 の CHANGELOG 管理 (6.4) が一貫したフローを構成するためには、**API 変更 (M2) -> バージョンバンプ + deprecation (M4) -> publish + deprecation 通知 (post-M4)** の 3 段階を明示的にトラッキングする仕組みが必要である。
