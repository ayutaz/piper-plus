...
1. **動作保証テスト**: voice なしで正しく動作することを明示的に検証するテストを 9 件追加する。M1-M3 では既存テストの安全網に依存していたが、M4 では「voice がない状態が正常であること」を積極的に断言するネガティブテストを導入する。
...
it('DictLoader.loadJaDict should not include voiceData in its contract', async () => {
    // Primary: Contract test — verify API surface and return type
    ...
    // Verify that loadJaDict() does not accept includeVoice option
    // and the return contract excludes voiceData.
    // (actual fetch requires a browser environment, so we verify the
    //  constructor and method signature do not reference voice concepts)
    assert.strictEqual(
        loader.loadJaDict.length <= 1, true,
        'loadJaDict should accept at most 1 parameter (options without includeVoice)'
    );

    // Fallback: Source-level structural check for defense in depth
...
#### テスト 8: `IgnoresVoiceEnvVar` (C++)

- **ファイル**: `src/cpp/tests/test_openjtalk_optimized.cpp`
- **目的**: `OPENJTALK_VOICE` 環境変数が設定されていても無視されることを確認する。M1 で voice 参照コードを完全除去した後、環境変数が設定された状態でも音素抽出が正常に動作し、voice ファイルへのアクセスが発生しないことを保証する再混入防止テスト。
- **アサーション**:
  - `OPENJTALK_VOICE` に存在しないパスを設定しても音素抽出が成功すること
  - 返り値が空文字列でないこと
- **実装コード例**:

```cpp
TEST_F(OpenJTalkOptimizedTest, IgnoresVoiceEnvVar) {
    // Set OPENJTALK_VOICE to a nonexistent path — must be ignored after M1
#ifdef _WIN32
...
#### テスト 9: `WasmBinarySizeRegression` (shell)

- **ファイル**: `src/wasm/openjtalk-web/test/verify-build.sh`
- **目的**: voice 除去後に WASM バイナリサイズが想定範囲内であることを確認する。voice 依存が再混入した場合、バイナリサイズ増加として検知する回帰テスト。
- **アサーション**:
  - `dist/rust-wasm/` 配下の `.wasm` ファイルの合計サイズが上限しきい値以下であること
- **実装コード例**:
...
本チケットは 1 名のエージェントで実施可能。作業量が限定的 (テスト 9 件 + クリーンアップ ~50 行) であり、並行作業の余地はない。

| 役割 | 人数 | 担当 |
|------|------|------|
| 実装担当 | 1 | テスト追加 (C++ 4 件、JS 3 件、shell 2 件) + クリーンアップ (ファイル削除、ドキュメント更新、バージョンバンプ、CHANGELOG) |

**作業順序:**
1. テスト 9 件を追加して GREEN を確認
...
**スコープ内:**
- C++ テスト 4 件追加 (`test_openjtalk_optimized.cpp`)
- JS テスト 3 件追加 (`test-g2p-contract.js`)
- shell 検証スクリプト 2 件更新 (`verify-build.sh`)
...
| 5 | `DictLoader returns without voiceData` | `test-g2p-contract.js` | Node.js test runner | なし (契約テスト + ソースコード静的検証) |
...
| 8 | `IgnoresVoiceEnvVar` | `test_openjtalk_optimized.cpp` | GoogleTest | OpenJTalk バイナリが利用可能 (不在時は GTEST_SKIP) |
| 9 | `WasmBinarySizeRegression` | `verify-build.sh` | shell (exit code) | ビルド成果物が存在 |

**テスト追加後の期待結果:**

| CI ワークフロー | 期待 |
|---------------|------|
| `_build-test-cpp.yml` | テスト 1, 2, 7, 8 が PASS (またはバイナリ不在時 SKIP) |
| `g2p-wasm-ci.yml` | テスト 3, 4, 5 が PASS |
| `wasm-build.yml` | テスト 6, 9 が PASS (`verify-build.sh` 内) |
...
- [ ] 新規テスト 9 件が M1-M3 の変更内容と整合しているか
...
M4 で追加する 9 件のテストは、テストピラミッドの異なるレイヤーに分散している。ゼロから設計するなら、各テストがどのレイヤーに位置するかを明示的に分類し、カバレッジの偏りを可視化すべきだった:
...
| テスト 5: `DictLoader returns without voiceData` | Integration (契約) | API 契約 + ソースコード構造検証 | < 50ms |
| テスト 6: `No voice files in assets` | E2E | ビルド成果物検証 | < 1s |
| テスト 7: `CommandWithoutVoiceFlag` | Unit | C++ コマンド構築 `-m` 不在 | < 100ms |
| テスト 8: `IgnoresVoiceEnvVar` | Unit | C++ 環境変数無視の再混入防止 | < 100ms |
| テスト 9: `WasmBinarySizeRegression` | E2E | WASM バイナリサイズ回帰検知 | < 1s |
...
#### 6.4 第2回設計レビュー結果からの追加タスク (2026-04-11)

5 チームによる M1/M2 設計レビューから M4 に追加すべきタスク・考慮事項が特定された。

**追加タスク 4.11: `OPENJTALK_VOICE` テストフィクスチャのクリーンアップ** :white_check_mark:

**ファイル:** `src/cpp/tests/test_dictionary_manager.cpp`
**状態:** 完了 (commit fcea77a2)
**目的:** `SetUp()` / `TearDown()` で `OPENJTALK_VOICE` を save/restore しているコード (L47, L89-100) は、M1 で機能コードから参照が消えたため dead test code。除去してテストの意図を明確にする。

**追加考慮事項 4.12: テスト順序のリスク認識**

M1/M2 で削除 → M4 でテスト追加という順序は、理想 (テスト追加 → 削除 → 検証) ではない。ただし以下の理由でリスクは受容可能:
- `get_openjtalk_voice_path()` は常に NULL を返していた (dead branch 除去であり動作変更ではない)
- `execlp` / `snprintf` 引数変更は `-m <voice>` フラグの除去であり、機能追加ではない
- 既存テスト (`BasicConversion` 等) が phonemizer パスをカバーしている

**この認識を M4 作業着手前に再確認し、既存テストの PASS を最初に検証すること。**

**追加考慮事項 4.13: `find_openjtalk_binary()` 共通化の後続課題登録**

品質レビューで `openjtalk_wrapper.c` と `openjtalk_optimized.c` の binary 検索ロジック重複が指摘された。M4 スコープ外だが、後続のリファクタリング課題として Issue に登録することを推奨。

#### 6.5 破壊的変更のバージョニング / CHANGELOG 管理の理想的なフロー
...
## 8. 第4回設計レビュー結果 (2026-04-11)

5 チーム (アーキテクチャ / セキュリティ / コード品質 / ビルドシステム / テスト戦略) による M1-M4 全体レビューを実施。

### 修正済み (本コミット)

| # | 重要度 | チーム | 指摘 | 対応 |
|---|--------|--------|------|------|
| C2 | CRITICAL | Test | テスト 1,2,7 の環境変数 save/restore 欠落 | テスト 8 のパターンに統一 |
| H1 | HIGH | Arch/Quality | stub 関数が exit(1) で強制終了 | エラーコード返却に変更 |
| H3 | HIGH | Build | PiperPlusShared.cmake に hts_engine_stub 依存欠落 | add_dependencies に追加 |
| H4 | HIGH | Quality | HTS_ENGINE_DIR 変数名が混乱を招く | HTS_STUB_DIR にリネーム |
| H5 | HIGH | Test | テスト 5 のソースコード文字列検索が脆い | 契約テストと防御テストに分離 |
| H6 | HIGH | Test | WASM サイズしきい値の根拠が未文書化 | コメントで根拠を説明 |
| C1 | CRITICAL | Security | openjtalk_optimized.c のパス検証欠落 | openjtalk_is_safe_path() 追加 |
| M1 | MEDIUM | Security | env var パス検証欠落 | access() + safe_path 条件結合 |
| M2 | MEDIUM | Security | snprintf バッファ長チェック欠落 | 事前長チェック追加 |
| M3 | MEDIUM | Build | FATAL_ERROR メッセージが Windows 非対応 | Windows コマンド追記 |
| M4 | MEDIUM | Build | piper_common の hts_engine_stub 依存欠落 | add_dependencies 追加 |
| L1 | INFO | Arch | stub バージョン文字列が実 HTS Engine と紛らわしい | "1.10-stub" に変更 |

### 後続タスクとして登録 (スコープ外)

| # | 重要度 | 指摘 | 理由 |
|---|--------|------|------|
| H2 | HIGH | IndexedDB DB_VERSION 未バンプ | npm publish 前に対応必要。dict-loader.js のキャッシュ戦略変更が必要 |
| L2 | LOW | WASM ABI バージョンゲート | 新機能追加。pre-1.0 で影響限定的 |
| M6 | MEDIUM | M1-M4 ドキュメント散在 | アーカイブ + アーキテクチャ文書統合は別 Issue で対応 |
| M7 | MEDIUM | Windows _putenv_s セマンティクス差 | プラットフォーム固有の制限。CI で検証 |
| M8 | MEDIUM | Shell find コマンドの可搬性 | macOS CI で検証。現状で動作 |

### 主要な知見

- **exit(1) vs エラーコード**: stub 関数は exit(1) ではなくエラーコードを返すべき。テスト可能性と安定性が向上する。
- **CMake 変数名は意図を反映すべき**: `HTS_ENGINE_DIR` → `HTS_STUB_DIR` で stub-only であることを明示。
- **テストの環境変数分離**: GoogleTest はプロセス分離しないため、全テストで save/restore パターンを徹底する。
- **セキュリティ**: コマンド構築前のパス検証は全コードパスで一貫して適用する。
