...
**着手前提**: なし (旧 ticket では T-016 完了推奨と記載していたが、 T-016 は 2026-05-20 scope-out 確定、 wolfi-base trial は失敗実証済のため流用不可。 PR #524 で webui と同 PR の trial bundle として直接着手・merged)

> **M3 内推奨実装順 (旧計画、 PR #524 trial bundle で実際の cadence は変更済)**: 旧 ticket 設計 (2026-05-19) では cpp-dev → 本 ticket → webui → python-inference → wyoming を推奨していたが、 PR #524 で本 ticket と T-013 (webui) を bundle merged、 PR #526 で cpp-dev (T-016) scope-out 確定により再編済。 本 image は runtime image なので shared lib 解決と ldconfig 周りに特有の懸念があり、 PR #524 では builder = `debian:12-slim` (ubuntu:24.04 から変更、 final stage `gcr.io/distroless/cc-debian12` との glibc ABI 整合のため) で trial 成功。
...
**推奨 (旧)**: ticket 設計時は `cgr.dev/chainguard/cc-dynamic` を推奨し cpp-dev (旧 T-016) と base registry 統一する想定だったが、 PR #524 trial で cpp-dev は wolfi-base packaging 不足により scope-out 確定 (PR #526)。 本 ticket は PR #524 で builder = `debian:12-slim` + final = `gcr.io/distroless/cc-debian12` で trial 成立、 base registry 統一の前提は変更済。 promotion PR で cc-dynamic vs distroless/cc-debian12 の比較は M3 retrospective に持ち越し。
...
| `docs/reference/distroless-migration.md` | 新規 (旧計画では T-016 で初版作成想定) | distroless 4 image 共通 doc (cpp-dev scope-out で未作成のまま、 promotion PR で別途初版作成予定) |
...
format (FR-1.4): distroless-report.md format (旧 T-016 で想定された M3 共通 format、 cpp-dev scope-out で未作成、 本 ticket / T-013 の promotion PR で別途定義予定)。 base image / size / CVE / layers を表示。
...
| **Spec / Doc author** | 1 | distroless-migration.md 新規 (旧計画では T-016 が初版予定だったが scope-out で未作成のため、 本 ticket の promotion PR で初版作成 + cpp-inference 知見を含む) | doc |
| **Reviewer** | 1 | shared lib path / OpenJTalk dict 配置確認 | review |

**並列度 (実績)**: PR #524 で T-013 (webui) と同 PR の trial bundle として直接着手・merged。 旧計画では cpp-dev (T-016) 完了後に着手し wolfi-base 知見を流用予定だったが、 cpp-dev は wolfi-base trial で構造的失敗 (apk packaging 不足) を経て scope-out 確定 (PR #526)、 流用前提は破棄済。
...
- distroless-migration.md (旧計画では T-016 共通 doc、 cpp-dev scope-out で未作成、 本 ticket の promotion PR で初版作成予定) に cpp-inference 知見を含める
...
- **採否**: PR #524 trial で `debian:12-slim` + `gcr.io/distroless/cc-debian12` (cc-dynamic ではなく) を採用、 size -2.2% を実証 (canonical 237MB → trial 232MB)。 cpp-dev wolfi-base trial の構造的失敗で Wolfi fallback も不採用、 Debian-glibc baseline で確定。
...
PR #524 trial 結果: **`gcr.io/distroless/cc-debian12` 採用** (cc-dynamic ではなく Debian-glibc baseline で builder と ABI 整合)。 旧計画の「T-016 retrospective で Wolfi vs distroless 比較」 は cpp-dev scope-out で消失、 M3 全体 retrospective に持ち越し。 v2 では rpath 埋め込みで LD_LIBRARY_PATH 不要化 (CMakeLists.txt 修正) を検討。
...
- Wolfi base への full migration は cpp-dev (旧 T-016) の wolfi-base trial で構造的失敗実証済 (PR #524、 OpenJTalk / mecab packaging 不足)、 採用不可
...
- [x] cc-dynamic vs Wolfi base どちらを採用するか (PR #524 で Debian-glibc baseline = `gcr.io/distroless/cc-debian12` 採用確定、 cpp-dev wolfi-base trial 失敗で Wolfi は採用不可と実証)
...
- 先行チケット: なし (旧 T-016 は 2026-05-20 scope-out、 ticket file 削除済、 経緯は PR #524 / #526)
