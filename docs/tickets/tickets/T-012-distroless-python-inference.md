...
**Status**: 着手中 (trial 投入済 — PR #523 merged 2026-05-20。 `docker/python-inference/Dockerfile.cpu.distroless` 新設、 既存 Dockerfile.cpu / docker-compose / deploy-huggingface.yml は不変更。 canonical 置換は promotion PR で別途、 HF Space staging 検証後に着手)
**PR**: #523 (merged 2026-05-20、 base `gcr.io/distroless/python3-debian12`、 multi-stage with `python:3.11-slim-trixie` builder)
**担当 (予定)**: Claude Code (agent team) + maintainer review
**着手前提**: T-013 完了必須 (Gradio + pyopenjtalk distroless multi-stage 知見を確立してから着手)

> **Trial PR scope note (PR #523、 merged 2026-05-20)**
>
> M3 distroless 化を順序を変更して **T-012 自体を trial PR として derisk** する方針に変更。 ticket original の旧推奨順 (cpp-dev → cpp-inference → webui → python-inference → wyoming、 2026-05-20 PR #526 で cpp-dev は scope-out 確定) より先に T-012 着手する理由:
>
...
>
> trial PR で「distroless build が成立し size 削減効果がある」 ことを実証してから T-013/T-014/T-015 + canonical 置換 PR に進む 4+ PR cadence (旧計画では cpp-dev も含め 5 image だったが、 PR #526 で cpp-dev scope-out 確定により縮小)。

> **M3 内推奨実装順 (旧計画、 PR #523 trial 着手で実際の cadence は変更済)**: 旧 ticket 設計 (2026-05-19) では cpp-dev → cpp-inference → webui → python-inference → wyoming を推奨していたが、 PR #523 (本 ticket trial 投入) と PR #526 (cpp-dev scope-out 確定) で再編済。 本 image は HuggingFace Space (`ayousanz/piper-plus`) の deploy 対象であり、 cold start 失敗が user-facing 影響を直接発生させる (R-4) ため、 trial で derisk してから promotion (canonical 置換) PR で staging 検証必須。
>
...
| `cgr.dev/chainguard/wolfi-base` + `apk add python-3.13` | shell + apk で柔軟 | size 大、 PR #524 wolfi-base trial で apk packaging 不足 (OpenJTalk / mecab tooling chain は Debian apt 前提) と判明、 dev image 文脈での失敗実証済 |
...
| 2026-05-20 | trial PR #523 merged。 `docker/python-inference/Dockerfile.cpu.distroless` 投入 (base = `gcr.io/distroless/python3-debian12`、 multi-stage with `python:3.11-slim-trixie` builder)。 canonical Dockerfile.cpu / docker-compose / deploy-huggingface.yml は不変更。 9 file +420/-8 で新規 CI workflow (`python-inference-distroless-trial.yml`) 追加、 build 成立 + size 削減 + import smoke test (ONNX Runtime / FastAPI / uvicorn / soundfile dlopen) 実証済。 CVE 数値計測 + HF Space staging cold start 検証 + canonical 置換は promotion PR で観測期間後に実施。 | Claude Code |
