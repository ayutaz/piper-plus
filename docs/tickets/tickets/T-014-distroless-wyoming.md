...
> **M3 内推奨実装順 (旧計画、 PR #524 trial bundle で実際の cadence は変更済)**: 旧 ticket 設計 (2026-05-19) では cpp-dev → cpp-inference → webui → python-inference → wyoming を推奨していたが、 PR #523 / #524 で T-012 / T-013 / T-015 が trial 投入済、 PR #526 で cpp-dev scope-out 確定により 4 image cadence に再編済。 本 image は Home Assistant addon (`addon-piper-plus` 等) として deploy される。 HA supervisor との互換性 (s6-overlay / init system / addon manifest) を維持する必要があり、 distroless 4 image 中で最後に着手予定。 distroless 移行前に **s6-overlay が現状の Dockerfile に含まれているかを spike 確認** する必要あり。
...
- M3 distroless 4 image (cpp-dev は 2026-05-20 scope-out で対象外) 完了後、 M3 retrospective を実施 (Wolfi vs distroless vs Chainguard 選定の総括、 PR #524 wolfi-base 試行失敗の構造的理由も含む)
