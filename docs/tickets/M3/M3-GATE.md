...
M3-1 完了
  ...
  └─ 不合格 → No-Go
       │
       ├─ 選択肢A: F5-TTS 方式の調査のみ実施
       │   - Flow Matching DiT ベース
       │   - Speaker Encoder 不要 (参照音声を直接入力)
       │   - 工数: +45-60d (アーキテクチャ全面刷新)
       │   - v2.0.0 では調査のみ実施し、本格実装は v3.0.0
       │   - v2.0.0 スケジュール: Voice Cloning なしでリリース
       │
...
M3-03 完了後に Stage 2 品質ゲートを設け、Voice Cloning の最終品質を判定する。Stage 2 不合格の場合は v2.0.0 から Voice Cloning を外す退路を確保する。

### Stage 2 品質ゲート (M3-3 完了後)

| メトリクス | 合格基準 | 不合格基準 |
|-----------|---------|-----------|
| MOS (音質) | > 3.5 | < 3.0 |
| 話者類似度 (cosine similarity) | > 0.80 | < 0.70 |
| 既存話者の品質劣化 | MOS 差 < 0.3 | MOS 差 > 0.5 |

**Stage 2 不合格時のアクション:**
- v2.0.0 から Voice Cloning (M3-2, M3-3, M3-4) を外す
- v2.0.0 のスコープは M3-5 (VITS2) + M3-6 (ZH増量) + M3-7 (SSML) + M3-8 (MOS縮小版) のみ
- Voice Cloning は v2.1.0 に延期し、アーキテクチャ再検討を実施
...
- **No-Go (選択肢A) の場合**: M3-2,3,4 を全て破棄し、F5-TTS アーキテクチャの調査チケットを新規作成 (調査のみ、本格実装は v3.0.0)。M3-5,6,7,8 は影響なし
- **No-Go (選択肢B) の場合**: M3-2,3,4 を v2.1.0 に移動。v2.0.0 のスコープは M3-5,6,7,8 のみ。リリースノートに Voice Cloning 延期を記載
- **Stage 2 ゲート (M3-3 完了後)**: MOS > 3.5 + 話者類似度 cosine > 0.80 を判定。不合格なら v2.0.0 から Voice Cloning を外し、v2.1.0 に延期
