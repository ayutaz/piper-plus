...
具体的には、**M3 完了時点で iOS 利用者の実数把握 (download metrics、Issue 受信頻度、Discord 言及) を行い、案 X か案 Z かを 6 ヶ月以内に判断する** ことを強く推奨する。負債は時間で利息が増える。M1 の URL swap が 1 年間「とりあえず動いている」状態で放置されることが、最も避けたいシナリオである。
...
- [ ] §11.9 の「6 ヶ月以内に案 X か案 Z かを判断」のリマインダーをカレンダー登録 (担当: メンテナ)
