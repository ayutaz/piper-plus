...
**Milestone ID**: `M2`
**Tier**: Tier 2
**Status**: 一部完了 (2026-05-19、 spec sync 5 件 = PR #517 merge / doc examples 3 phase は計画中)
...
完了済み 7 件 (旧 T-004〜T-010) は ticket file を削除済み (履歴は merged PR #517 / #520 / #521 と git log を canonical reference として参照)。 残るのは T-011 のみ。
...
| 範囲 | merged PR |
|------|-----------|
| spec sync gate × 5 (release-versions / model-sha256 / artifact-retention / swift-g2p / test-flake-retry) | PR #517 (commit `f3ef12cd`) |
| doc examples audit (PR-A) | PR #520 (commit `1ee5144c`) |
| doc examples informational gate (PR-B) + test-flake-retry 4 runtime 拡張 | PR #521 (commit `206b5c66`) |

### 依存関係

- T-011 は PR #521 で導入した informational gate の **1 ヶ月運用観測** が前提。 false positive 率と sticky comment の signal 安定性を見てから user 判断で `continue-on-error: true` を外して blocker 化する mode flip。
...
| 2026-05-19 | spec sync 5 件 (T-004〜T-008) を PR #517 で merge → Status を 一部完了 に更新 | Claude Code |
