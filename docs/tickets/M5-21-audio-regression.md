...
```
tests/fixtures/c-api/
  audio-regression-baseline.json
```
...
| リスク | 影響度 | 対策 |
|--------|--------|------|
| プラットフォーム間の浮動小数点差異 | 高 | deterministic 合成でも CPU アーキテクチャ (x86 vs ARM) で微小差異が出る可能性。プラットフォーム別ベースラインまたは許容差付き比較を検討 |
