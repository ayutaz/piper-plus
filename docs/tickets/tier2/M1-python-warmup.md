...
- [x] 全 5 箇所の Python 推論エントリポイントで warmup が呼び出される
- [x] Rust/C# と同一のダミー入力パラメータ (100 phonemes, scales=[0.667, 1.0, 0.8])
- [x] warmup 例外がアプリケーションをクラッシュさせない
- [x] `PIPER_DISABLE_WARMUP=1` で無効化可能
- [x] 既存テスト全 PASS + 新規テスト 5件以上 PASS
...
- [x] `uv run pytest src/python/tests/test_ort_utils.py -v` 全 PASS (既存テスト + 新規 TestWarmup)
- [x] `uv run ruff check src/python/piper_train/ort_utils.py` PASS
- [x] `uv run ruff format --check src/python/piper_train/ort_utils.py` PASS
- [x] 全変更ファイルに対して `ruff check` / `ruff format --check` PASS
...
- [x] **定数一致**: `WARMUP_PHONEME_LENGTH=100`, `DEFAULT_WARMUP_RUNS=2`, BOS=1, EOS=2, dummy=8, scales=[0.667, 1.0, 0.8] が Rust/C# と一致
- [x] **例外処理**: 全呼び出し箇所で例外がアプリケーションをクラッシュさせないこと
- [x] **ログレベル**: 成功時 `info`、失敗時 `warning`、無効化時 `info`
- [x] **環境変数**: `PIPER_DISABLE_WARMUP` の値チェックが大文字小文字非依存 (`.lower()`)
- [x] **voice.py 同期**: インライン実装が `ort_utils.py` と同一のロジック
- [x] **import 整理**: 新規 import が ruff の import order ルールに準拠
- [x] **テスト網羅性**: 正常系 / 異常系 / 環境変数 / runs=0 / オプション入力パターン全て
- [x] **WebUI ロック**: warmup がキャッシュロック内で実行され、1 モデル 1 回のみ
- [x] **streaming**: encoder のみ warmup、decoder は未対応の旨がコメントに記載
- [x] **既存テスト非破壊**: `uv run pytest src/python/tests/ --override-ini="addopts="` で既存テスト全 PASS
