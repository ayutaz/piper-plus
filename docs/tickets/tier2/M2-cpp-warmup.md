...
- [x] `warmupModel()` のダミー入力値が Rust/C# と一致している (BOS=1, EOS=2, dummy=8, scales=[0.667, 1.0, 0.8], length=100)
- [x] `session.hasMultiSpeaker` (L700) / `hasLidInput` (L719) / `hasProsodyInput` (L729) の条件分岐が `synthesize()` と同一パターン
- [x] 入力テンソル名の順序が ONNX モデル定義と一致: `input` → `input_lengths` → `scales` → `sid` → `lid` → `prosody_features`
- [x] テンソルのライフタイムが `Run()` 完了まで保持されている (ローカル変数が if ブロック内で破棄されない)
- [x] 出力テンソル名に `output` と `durations` の両方が含まれている (`session.hasDurationOutput` に基づく)
- [x] 全例外がキャッチされている (`std::exception` + `...`)
- [x] `spdlog::info` で warmup 完了メッセージが出力されている (回数 + 経過時間)
- [x] `--no-warmup` が `knownFlags` に追加されている (未知フラグ検出で誤検知しない)
- [x] `--test-mode` 時に warmup がスキップされている
- [x] `printUsage()` に `--no-warmup` の説明が追加されている
- [x] `RunConfig::noWarmup` のデフォルト値が `false` (warmup はデフォルト有効)
