...
// 理想形: EP アダプターの静的登録
struct EpDescriptor {
    std::string ort_name;     // "CUDAExecutionProvider"
    std::string piper_name;   // "cuda"
    int priority;             // auto-detect 優先度
    std::function<bool(Ort::SessionOptions&, int device_id)> try_append;
};

// EP レジストリ: コンパイル時に利用可能な EP を登録
static const std::vector<EpDescriptor> EP_REGISTRY = {
    {
...
- `EP_REGISTRY` はコンパイル時定数として `constexpr` または `inline static const` にし、動的割り当てを避ける
