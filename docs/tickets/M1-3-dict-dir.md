...
> **M1-6 と同一 PR で対応すること。** M1-3 (dict_dir) は M1-6 (C API 実装) の `piper_plus_create()` 内で実装される設計仕様であり、ヘッダー (M1-5) と実装 (M1-6) に一体的に含めた方が効率的。振り返りで「M1-3 + M1-5 + M1-6 は統合可能だった」と指摘されている (c-api-milestones.md Phase 1 振り返り参照)。
