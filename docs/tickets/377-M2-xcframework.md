...
| `.github/workflows/release-shared-lib.yml` | (新規) `assemble-xcframework` ジョブ | 両 slice artifact を `actions/download-artifact@v8` で取得、`xcodebuild -create-xcframework` で統合、各 slice の `Headers/` に modulemap を配置、xcframework ルートに PrivacyInfo.xcprivacy を配置、`zip -ry` で zip 化 | +60〜80 |
...
| `cmake/PiperPlusShared.cmake` | iOS 分岐末尾 (新規) | **module map 自動生成**: `framework module PiperPlus { umbrella header "piper_plus.h"; export *; module * { export *; } }` を build dir に出力、`assemble-xcframework` ジョブが各 slice の `Headers/module.modulemap` として配置 | +15 |
| `cmake/PrivacyInfo.xcprivacy` (新規) | 全体 (新規) | **空 Privacy Manifest テンプレート**: `NSPrivacyTracking=false`, `NSPrivacyCollectedDataTypes=array[]`, `NSPrivacyAccessedAPITypes=array[]` (Required Reason API 不使用宣言、iOS 17+ App Store 提出対応) | +12 |
...
> **M2 で対応 (旧 §11.7 繰り上げ採用):**
> - `cmake/PiperPlusShared.cmake` に **module map 自動生成**を追加 (Swift `import` 対応、M4 SPM 化の前提)
> - `cmake/PrivacyInfo.xcprivacy` を**新規追加**し xcframework に同梱 (iOS 17+ App Store 必須要件への対応、ORT 公式は未提供のため利用者責任で追加が前提だが、本体は piper-plus 側として空 Manifest を提供)
...
- 新規 `assemble-xcframework` ジョブ追加 (`needs: build-ios`)
  ...
  - 各 slice の `Headers/` に `module.modulemap` 配置
  - xcframework ルートに `PrivacyInfo.xcprivacy` 配置
...
- `cmake/PiperPlusShared.cmake` の両 slice 動作確認 + **module map 自動生成**追加 (旧 §11.7 繰り上げ採用)
- `cmake/PrivacyInfo.xcprivacy` 新規追加 (旧 §11.7 繰り上げ採用、iOS 17+ App Store 対応)
- M1 形式 `libpiper_plus-ios-arm64-${VERSION}.tar.gz` の **継続配布** (v1.13.0 移行期間)
- ORT framework を xcframework に含めない方針 (案 Y) の確定
...
| `Package.swift` 本体 repo 配置 (案 X) | **M4** |
| `.dSYM` の別 artifact 化 (`libpiper_plus-ios-${VERSION}.dSYM.zip`) | **将来別 issue** |
| csukuangfj/onnxruntime-libs ミラーへのフォールバック | **将来 (M2 後の運用課題)** |
| visionOS / Mac Catalyst slice 追加 | **将来 M5 候補** |
...
| T12 | xcframework.zip サイズ | ~6〜10MB (ORT framework 非同梱、案 Y 採用のため軽量) |
| T13 | **module map 同梱**: 各 slice の `Headers/module.modulemap` 存在 | `find piper_plus.xcframework -name "module.modulemap"` で 2 件 (device + simulator) ヒット、内容に `framework module PiperPlus` 宣言 |
| T14 | **Privacy Manifest 同梱**: xcframework ルートに `PrivacyInfo.xcprivacy` 存在 | `plutil -p piper_plus.xcframework/PrivacyInfo.xcprivacy` で `NSPrivacyTracking=false`、3 配列が空であることを表示 |
| T15 | **Swift `import PiperPlus` 構文成立 (CI 内)** | macos-15 runner 上で test-only `swift -I piper_plus.xcframework/ios-arm64/Headers -framework PiperPlus -emit-module` 等で modulemap 解釈成功 (実機 import 検証は M4 SPM 経由で代替) |
...
| C2 | `xcodebuild -create-xcframework` の Xcode バージョン互換性 (16.x / 26.x で仕様差) | 中 | `macos-15` runner の Xcode 16.4 を既定とし、`xcodebuild -version` を CI ログに記録。Xcode 26 系へのメジャー昇格時は別 issue で M2 フォローアップ。`-headers` ディレクトリ指定の挙動が Xcode 16 で微変更されている事例あり |
...
### 10.6 ORT 同梱方針 (案 Y、ORT 別配布) の妥当性
...
### 10.6.1 module map 同梱 (旧 §11.7 繰り上げ採用)

- [ ] `cmake/PiperPlusShared.cmake` の iOS 分岐に modulemap 生成ステップが追加されている
- [ ] modulemap の内容が `framework module PiperPlus { umbrella header "piper_plus.h"; export *; module * { export *; } }` 形式
- [ ] xcframework 内 device/simulator の両 slice の `Headers/module.modulemap` に配置済 (`find piper_plus.xcframework -name "module.modulemap"` で 2 件)
- [ ] M4 SPM `import PiperPlus` 利用が成立する状態 (CI 内 `swift -emit-module` で構文確認)

### 10.6.2 Privacy Manifest 同梱 (旧 §11.7 繰り上げ採用)

- [ ] `cmake/PrivacyInfo.xcprivacy` 新規ファイル追加
- [ ] 内容が空 Manifest テンプレート (`NSPrivacyTracking=false`, `NSPrivacyCollectedDataTypes=array[]`, `NSPrivacyAccessedAPITypes=array[]`)
- [ ] xcframework ルートに配置済 (`plutil -p piper_plus.xcframework/PrivacyInfo.xcprivacy` で正しい plist 構造)
- [ ] M3 利用者ガイドで「ORT 側 Privacy Manifest は利用者が別途追加」を明記する引き継ぎが §12.2 にある
...
>
> **⚠️ 指示子スコープ注記:** 本節の「案 X / 案 Y / 案 Z」は **M2 文脈に閉じた指示子** で、他チケットとは指す内容が異なる。M2 §11.2 では: 案 X = piper-plus.xcframework に ORT framework 同梱、案 Y = ORT 別配布 (M2 採用案)、案 Z = M2+M4 統合 SPM 同時公開 を指す。
>
> **🔔 §11.7 繰り上げ提案の取り扱い:** 旧 §11.7 で提案した「module map / Privacy Manifest を M2 スコープに繰り上げ」は **道 A 確定により採用済み**。§3.1 編集ファイル一覧および §5 Included に反映済。本節は判断記録として残す。
...
#### piper-plus 自体: STATIC archive vs SHARED framework

現行 (`cmake/PiperPlusShared.cmake:9-13`) では iOS で `add_library(piper_plus STATIC ...)` を使用し、xcframework に `.a` (ar archive) を入れる形態。代替として SHARED (Mach-O dylib + framework バンドル) も検討に値する:

| 観点 | STATIC (.a、現行) | SHARED (.framework、検討案) |
|------|------------------|-------------------------|
| 利用者の Xcode 操作 | piper-plus xcframework は static、ORT は dynamic で **Embed 設定が非対称** (混乱の原因) | piper-plus / ORT 両方 dynamic で `Embed & Sign Frameworks` 設定が **対称化**、利用者の混乱を構造的に解消 |
| アプリ起動オーバーヘッド | なし | dlopen で +1-3ms (現実的に無視可) |
| アプリサイズ | 小 (~3MB、必要なシンボルだけ最終リンクで取込) | 中 (~3MB + framework メタデータ ~50KB) |
| App Store 適合性 | ✓ (static は Embed 不要) | ✓ (iOS 8+ で dynamic framework 許容、Embed & Sign 必須) |
| CMake 修正範囲 | ゼロ | `cmake/PiperPlusShared.cmake` に iOS 専用 SHARED 分岐 ~30 行追加 |
| 利用者誤操作リスク | "Do Not Embed" を誤選択でも static は機能 | "Embed Without Signing" / "Do Not Embed" 誤選択で実機起動失敗 |
| 配布物変更 | 現行維持 | xcframework が `.a` → `.framework` バンドル形式に変化、利用者の README 更新必須 |

**M2 での判断: STATIC のまま維持** (現行)。SHARED 化の便益は「Embed 操作の対称化」のみで、デメリット (誤操作リスク + README 全面更新) と釣り合わない。M3 利用者ガイドで Embed & Sign の非対称性を **明示的に説明する** ことで補う。

**将来の再評価ゲート:**
- M3 完了から 6 ヶ月の利用者観測で **「Embed & Sign 関連の Issue が 3 件以上」発生** した場合、SHARED 化検討を別 issue で起票
- 観測閾値未満なら STATIC 維持、`cmake/PiperPlusShared.cmake` の iOS 分岐は現状維持
...
| 観点 | M2 の選択 | 白紙設計の選択 |
|------|----------|------------|
| **runner** | macos-15 単一 (本実装で `macos-14` から bump、Xcode 16.4 デフォルト + Xcode 26 利用可) | macos-15 で当面運用、Xcode 26 系完全 GA 後に macos-26 (Tahoe) へ昇格検討 |
...
**白紙設計の選択:** runner image を `macos-15` (Xcode 16.4 デフォルト、Xcode 26 系も installed) でピン留め、`xcodebuild -version` を CI ログに出力、xcframework Info.plist を artifact に含めて diff チェック可能にする。次の昇格候補は `macos-26` (Tahoe) だが iOS 18 SDK が無いため Deployment Target 拡張時に再評価。
...
- ~~**module map / Privacy Manifest を M2 スコープに繰り上げる** (~30 行 CMake 追加で済む、後付けより圧倒的に安い)~~ → **採用済 (道 A 確定、§3.1 / §3.6 / §5 / §6 / §10.6 で反映済)**
- **.dSYM 別 artifact 配布を M2 で開始する** (whisper.cpp の `dsymutil` パターン流用、~10 行追加) → **未対応、別 issue で追跡** (§5 Excluded)
- **M3 完了時点で iOS 利用者の実数把握 (download metrics、Issue 受信頻度) を行い、6 ヶ月以内に SPM 公開 (M2 §11.2 案 Z = M2+M4 統合) の採用判断を下す** → **道 A 確定により M4 を本ブランチで実装、M3 完了後の利用者観測で v1.13.0 / v1.14.0 のリリースタイミングを判断**
...
> **道 A 確定後の更新:** module map / Privacy Manifest の繰り上げ採用により、上記負債のうち主要 2 件 (Apple ecosystem 流儀外、後付けコスト) は M2 完了時点で解消される。残る .dSYM 別 artifact 化は別 issue で継続追跡。
