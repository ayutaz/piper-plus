...
`docs/guides/ios-integration.md` および `docs/spec/ios-shared-lib.md` の「Note: 未対応事項」節で以下を明記する。M3 では実装せず、利用者に状況を正直に伝える:

- **module map (`module.modulemap`)**: M3 未対応。Swift `import` を要する場合は利用者側で手動追加か、M4 (SPM パッケージ併設) 待ち。M2 §11.7 の「M2 スコープ繰り上げ」提案を採用するか、M4 まで遅延するかは M2 着手前に再評価する
- **Privacy Manifest (`PrivacyInfo.xcprivacy`)**: M3 未同梱。Apple の要件は (a) Required Reason API 使用時、または (b) "commonly used SDK list" 該当時のみ必須であり、ORT/piper-plus は Required Reason API を直接使わない見込み (M2 §11.7 参照)。空 Manifest の最小同梱は別 issue で追跡
- **.dSYM (Debug Symbols)**: xcframework 内バイナリは stripped、.dSYM は別 artifact 化されていない。クラッシュレポートの symbolication が必要な利用者は M2 ビルドフローのカスタマイズが必要。別 issue で追跡
...
| C5 | module map 不在で Swift 利用者が `import` で躓く | 高 | `examples/swift/README.md` に手動 modulemap 例を提供、M4 で正式対応する旨を明記。M2 完了の xcframework に modulemap を追加できるか M4 で再判断 (§12 引継) |
...
#### module map / Privacy Manifest 繰り上げ提案

M2 §11.7 で示した「module map による Embed & Sign 自動化」と「Privacy Manifest 同梱」は、ドキュメントで回避するより実装で解決する方が総コストが低い。M3 で「Privacy Manifest はユーザー側で作成」と書く場合、その記述自体が App Store Review Guidelines 更新で陳腐化する。**M3 をスリム化し、M2 §11.7 が提示した「M2 スコープへの繰り上げ」(module map と空 Privacy Manifest を M2 の `assemble-xcframework` に ~30 行追加で同梱) を M2 着手前に再評価する方が利用者体験・保守性ともに良い。**

### 11.7 もし今から始めるなら (推奨)

#### 推奨案 (5 段階)

1. **M3 解体・各 PR に分散**: README 更新は M1/M2 PR 内に取り込む (案 X)
...
5. **module map / Privacy Manifest を M2.5 として実装繰り上げ**: ドキュメントで誤魔化さない

#### 推奨を採用しない合理的理由

| 推奨 | 不採用条件 |
|---|---|
| #1 PR 統合 | M1/M2 PR が既に 1500+ 行あり diff 過大 |
| #2 Quick Start 集中 | 利用者の Xcode 経験差が大きく、最小手順では足りない (要観測データ) |
| #3 単一推奨 | C/C++ 利用者の存在確認が取れた場合 (現状不明) |
| #4 CI 追加 | CI 実行時間が既に 30 分超で追加余地なし |
| #5 M2.5 繰り上げ | リリース日程 (`v1.13.0`) を遅延できない場合 |
...
| Privacy Manifest 自動同梱 | 高 | M2.5 (繰り上げ推奨) |
...
- **module map は M3 未対応**: M4 で SPM 化する際、Swift `import PiperPlus` を成立させるには xcframework 内に `module.modulemap` を含める必要がある。M2 で生成した xcframework に modulemap を追加できるかを **M4 着手時に再判断** する。追加が困難な場合は M2 のビルドフローを M4 から拡張する (= 実質的に M2 への遡及修正)。
...
- [ ] M3 §11.7 の繰り上げ提案 (module map / Privacy Manifest を M2.5 化) を採用するかは、M2 完了直後に再評価する旨を明記
