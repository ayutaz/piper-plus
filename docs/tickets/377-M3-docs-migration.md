...
- **ORT バージョン全 OS 横断 bump (1.17.0 → 1.22.0)**: iOS / Android / C++ で共通の ORT 1.17.0 (2024-01) を最新 stable 1.22.0 (2025-05) に bump。CDN URL は HTTP 200 確認済 (M1 §9 C8)。`release-shared-lib.yml` の `env.ONNXRUNTIME_VERSION` 単一変更で全 OS 連動。テスト網羅性確保のため全ランタイム (Python/C#/Rust/Go/JS/CLI) のバージョンマトリクス整合確認も伴う。
...
- [ ] M1 §11.9 の「6 ヶ月以内に案 X (ORT ソースビルド) か案 Z (iOS 廃止) かを判断」のリマインダーを **本 PR マージ日起点** で登録 (`docs/tickets/README.md §利用者観測タイムライン` の規約に従う)
