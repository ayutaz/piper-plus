...
**重要ガード:** `word[i+2] !== word[i+1]` (nn/mm は鼻母音化しない: `"bonne"` → b ɔ n)
...
| **テストエージェント** | 1 | `test-french.js` 作成 + Rust 35 テスト移植 |
