...
| `--language` | - | string | `ja` | `--text` モード用言語: `ja`, `en`, `zh`, `es`, `fr`, `pt` |
...
| `lid` | int64 | `[1]` | マルチリンガルモデルのみ (`HasLanguageId` で検出) |
...
| `num_languages` | int | 言語数 (デフォルト: 1) |
| `language_id_map` | object | 言語コード→ID変換マップ (`{"ja": 0, "en": 1, ...}`) |
...
dev ブランチでは Python/C++ が6言語 (ja/en/zh/es/fr/pt) に対応済み。C# CLI は現在 ja/en のみ対応しており、6言語マルチリンガル対応が必要。

マルチリンガルモデル (`multilingual-test-medium.onnx` 等) は推論時に `lid` (language ID) 入力テンソルが必須。モデルの ONNX 入力メタデータに `lid` が含まれるかどうかで、マルチリンガルモデルかどうかを自動検出する。

### 13.2 config.json の新フィールド

マルチリンガルモデルの `config.json` には以下のフィールドが追加される:
...
`PiperConfig` に追加すべきプロパティ:

| プロパティ | 型 | デフォルト | 説明 |
|-----------|-----|---------|------|
| `NumLanguages` | `int` | 1 | 言語数 |
| `LanguageIdMap` | `Dictionary<string, int>?` | `null` | 言語コード→ID変換マップ |

### 13.3 ONNX 推論の lid テンソル

**PiperModel への追加:**

`PiperModel` コンストラクタで `lid` 入力テンソルの有無を検出する。
...
| プロパティ | 型 | 説明 |
|-----------|-----|------|
| `HasLanguageId` | `bool` | `lid` 入力テンソルの有無 (マルチリンガルモデル) |
...
**PiperSession.RunInferenceCore() での lid テンソル構築:**

- Shape: `[1]`, Type: `int64`
- `sid` と `prosody_features` の間に挿入
...
// 2. input_lengths
// 3. scales
// 4. sid           (HasSpeakerId の場合)
// 5. lid           (HasLanguageId の場合)  ← 追加
// 6. prosody_features (HasProsody の場合)
```

### 13.4 各言語 Phonemizer 仕様

#### スペイン語 (SpanishPhonemizer)

- 規則ベース (外部依存なし)
- グラフェム分割 → 音節化 → G2P → ストレス標記
- PostProcessIds: BOS/EOS/PAD intersperse (英語と同様)
- 推定行数: ~900行

#### フランス語 (FrenchPhonemizer)

- 規則ベース (外部依存なし)
- 30+ の複合文字列パターン、鼻母音、サイレントレター
- PostProcessIds: BOS/EOS/PAD intersperse
- 推定行数: ~850行

#### ポルトガル語 (PortuguesePhonemizer)

- 規則ベース (外部依存なし)
- スペイン語ベース + 鼻母音、コーダ L 母音化
- PostProcessIds: BOS/EOS/PAD intersperse
- 推定行数: ~750行

#### 中国語 (ChinesePhonemizer)

- 規則ベース (Common 3500字テーブル)
- ピンイン分割 → 初声/終声/声調分離 → IPA変換
- 声調サンディ規則 (T3+T3 → T2+T3 等)
- PostProcessIds: BOS/EOS/PAD intersperse
- 推定行数: ~650行

#### 韓国語 (KoreanPhonemizer)

- ハングル分解 (U+AC00-D7A3 → 初声/中声/終声)
- IPA変換テーブル (初声19、中声21、終声28)
- 連音化規則 (基本のみ)
- PostProcessIds: BOS/EOS/PAD intersperse
- 推定行数: ~400行

### 13.5 MultilingualPhonemizer

`UnicodeLanguageDetector` でテキスト内の言語を自動検出し、セグメント化して各言語の Phonemizer に委譲する。

**検出ルール:**

| Unicode範囲 | 検出言語 | 条件 |
|------------|---------|------|
| U+3040-30FF (Kana) | ja | ひらがな・カタカナ |
| U+4E00-9FFF (CJK) | zh | Kana が近傍になし |
| U+AC00-D7AF (Hangul) | ko | ハングル音節 |
| Latin | デフォルト言語 | en/es/fr/pt から設定で決定 |

**処理フロー:**

```
テキスト
  → [1] UnicodeLanguageDetector.Detect(text)
    → セグメントリスト [(text, lang), ...]
  → [2] 各セグメントを対応する言語 Phonemizer に委譲
    → 言語別音素列
  → [3] 結果統合 → 統一 phoneme_ids
```
...
| 現在 | 更新後 |
|------|--------|
| `ja` / `en` | `ja`, `en`, `zh`, `es`, `fr`, `pt` |

**JSONL フィールド追加:**

```json
{"phoneme_ids":[1,8,5,39], "speaker_id":0, "language_id":2, "language":"zh"}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `language_id` | int | 数値言語ID (直接指定) |
| `language` | string | 言語コード文字列 (`language_id_map` で数値IDに解決) |

**言語解決の優先順位:**
1. `language_id` (数値、直接指定)
2. `language` (文字列) → `config.json` の `language_id_map` → 数値ID
3. CLI `--language` → `language_id_map` → 数値ID
4. デフォルト: 0 (ja)

### 13.7 C++/Python との対応表

| 機能 | Python | C++ | C# (現在) | C# (Phase 7) |
|------|--------|-----|-----------|--------------|
| lid テンソル | ✅ | ✅ | ❌ | ✅ |
| language_id_map | ✅ | ✅ | ❌ | ✅ |
| JA Phonemizer | ✅ | ✅ | ✅ | ✅ |
