...
他の全作業の CI 安全性の前提。AttributeError で即座に失敗するテストを修正する。
（コード依存ではなく、壊れたテストが CI 結果を汚染することを防ぐための優先順位）
...
| M1-2 | [M1-2](tickets/M1-2.md) | `export_onnx.py` (L246-268) | `apply_ema_weights(decoder, checkpoint_path)` | `test_export_onnx.py` (L142-154) | Yes |
...
| M1-5 | [M1-5](tickets/M1-5.md) | `infer_onnx.py` (L311-320) | `resolve_config_path(model, config)` | `test_infer_onnx_config.py` 全4テスト | **M1-5 先行** |
| M1-6 | [M1-6](tickets/M1-6.md) | `infer_onnx.py` (L397-411) | `resolve_speaker_id(speaker_id, has_sid)` | `test_multilingual_inference.py` (L370-417) | **M1-5 後** |

> **注意**: M1-5 と M1-6 は同一ファイル (`infer_onnx.py`) を変更する。コンフリクト回避のため M1-5 を先にマージすること。
...
| M2-3 | [M2-3](tickets/M2-3.md) | `test_performance.py` (L25-206) | `phonemize_japanese()` 4重コピペ → モジュールレベルのヘルパーに統合 | Yes |
| M2-4 | [M2-4](tickets/M2-4.md) | `test_intersperse_padding.py` (L45-77) | `has_intersperse_padding()` 再実装 → `commons.intersperse()` の出力と比較 | Yes |
| M2-5 | [M2-5](tickets/M2-5.md) | `test_prosody_extraction.py` (L550-574) | `validate_dataset_prosody()` 自己テスト → **削除** (Option B をデフォルトとする) | Yes |
| M2-6 | [M2-6](tickets/M2-6.md) | `test_convert_fp16.py` (L14-42) | `_onnx_inference()` → 本番 `_create_dummy_inputs()` を import | Yes |
| M2-7 | [M2-7](tickets/M2-7.md) | `test_integration.py` (L28-85) | 無意味なテスト2件 (`test_model_config_validation`, `test_wav_file_generation`) → 本番コードを呼ぶか削除 | Yes |

> **注意**: M2-5 と M3-3 は同じファイル (`test_prosody_extraction.py`) の異なる行範囲を変更する。並行作業時はマージコンフリクトに注意。
...
| # | チケット | ファイル | 作業内容 | 並行可 |
|---|---------|---------|---------|:------:|
| M3-1 | [M3-1](tickets/M3-1.md) | `test_swedish_phonemizer.py` (L28-46) | `_split_ipa_to_phonemes()` 旧API関数のテスト用コピー整理。**注意**: Option A (本番追加) は M1 レベルの変更を伴うため、Option B (docstring修正のみ) を推奨 | Yes |
| M3-2 | [M3-2](tickets/M3-2.md) | `test_token_mapper_impl.py` (L16-18) | `map_sequence()` 1行ラッパー → インライン化 | Yes |
| M3-3 | [M3-3](tickets/M3-3.md) | `test_prosody_extraction.py` (L18-38) | BOS/EOS ラッパー → 共通ユーティリティ抽出を検討。**注意**: EOS統一は `test_phonemize.py` の期待値にも影響するため、変更時は全テスト再検証が必要 | Yes |
...
Agent 5: M1-5 → M1-6 (infer_onnx.py 2件、直列で実行)
...
```
M0 → M1, M2, M3  (CI 安全性の前提。コード依存ではないが、壊れたテストが結果を汚染する)
M1-5 → M1-6      (同一ファイル変更。M1-5 先行マージ)
M1-1 → なし      (conftest.py は他テストの fixture だが、既存テストは壊れない)
M1-3 → なし      (gin_channels テストは独立)
M2-* → なし      (全て独立、本番コード変更なし)
M2-5 ⇔ M3-3     (同一ファイルの異なる行範囲。並行時はマージコンフリクト注意)
```
