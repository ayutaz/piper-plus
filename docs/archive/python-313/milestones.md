...
### Entry Criteria (DR-009 適用後)

- [ ] M2 が完了 (非 GPU 系が 3.13 統一済)
- [x] host driver R570+ / nvidia-container-toolkit 1.14+ は**前提として進行** (実機準備中、 OQ-01/02 で確定)

### Exit Criteria (DR-009 適用後)

**merge 前必須 (CI で完結):**

- [ ] `docker build -f docker/python-inference/Dockerfile` success on amd64 (CI で確認可能)
- [ ] `docker build -f docker/python-train/Dockerfile` success on amd64 (同上)
- [ ] image 起動して `python --version` が `3.13.x` (CI で confirm 可能)
- [ ] `python -c "import torch; print(torch.__version__)"` が `2.11.0+cu128` (CI で confirm)
- [ ] `onnxruntime-gpu` が import 成功 (`CUDAExecutionProvider` 認識は実機依存のため post-merge)
- [ ] Trivy scan: new HIGH/CRITICAL が **0 件**
- [ ] `docker-build.yml` の CUDA image ジョブが green

**post-merge verification (実機準備完了後、 M5 release 前必須):**
...
- [ ] `torch.cuda.is_available()` が True (実機)

### 依存

- M2 完了 (distroless 系の知見が CUDA Docker でも活用される)
- 運用前提 (host driver / nvidia-container-toolkit) は前提として進行 (DR-009)
...
| **post-merge: RTX 5090 で torch.cuda.is_available() = False** | 実機 smoke FAIL | host driver R570+ 再確認、 nvidia-container-toolkit 1.14+ 再確認、 docker `--gpus all` flag 確認。 必要なら M3 を revert して再修正 |
| **post-merge: 実機 smoke 失敗による hotfix が必要** | smoke FAIL → fix が大きい | `v2.0.0rc1` 等の pre-release で吸収、 fix 完了後に GA release (DR-009) |
...
- [x] 実機環境 (Ada 6000 / RTX 5090) は前提として進行 (DR-009)、 実機 smoke は post-merge 化

### Exit Criteria (DR-009 適用後)

**merge 前必須 (CI で完結):**

- [ ] `__main__.py` の TF32 enable 2 行 (`matmul.allow_tf32` + `cudnn.allow_tf32`) が main() 内、 Trainer 構築前に存在
...
- [ ] `training-guide.md` / `CLAUDE.md` に「v2.0 では過去 ckpt の resume 非サポート」 を明記 (DR-006)
- [ ] `training-template-drift` pre-commit hook が新 default で PASS
- [ ] `python-tests.yml` matrix 3.13 ジョブが全 PASS

**post-merge verification (実機準備完了後、 M5 release 前必須):**

- [ ] **Ada 6000 実機**で `torch.backends.cuda.matmul.allow_tf32 == True` 確認
- [ ] T4 / sm_75 以下で noop 動作 (warning 出ない) 確認
...
- [ ] bf16-mixed で 100 step smoke + audio_parity Tier 4 PASS (SNR ≥ 30dB)
...
- post-merge 実機 smoke のため Ada 6000 / RTX 5090 環境が M5 着手前までに準備完了 (DR-009)
...
| カテゴリ | 内容 |
|---|---|
| バージョン | `VERSION` ファイル + 関連 pyproject.toml の version bump (例: 1.12.0 → 2.0.0 major bump) |
| CHANGELOG | "BREAKING: default Docker now CUDA 12.8 + Ubuntu 24.04 + Python 3.13" を [Unreleased] → [2.0.0] へ移動 |
| Migration guide | `docs/migration/v1.12-to-v2.0.md` を新規作成 (要 host driver R570+、 nvidia-container-toolkit 1.14+ 等) |
| Release | `dev-create-release.yml` で v2.0.0 release tag 作成 |
| Docker image tag | `piper-plus-cpu:2.0.0` / `piper-plus-gpu:2.0.0` 等を registry に push |
...
### Entry Criteria (DR-009 適用後、 release 品質ゲート)
...
- [ ] **実機環境 (Ada 6000 + RTX 5090 + T4) が準備完了** (OQ-01/02 で「前提として進行」 と決めた実機環境がここで実体化)
- [ ] **M3 post-merge verification 完了** (Ada/RTX5090/T4 起動確認、 onnxruntime-gpu の CUDAExecutionProvider 認識)
- [ ] **M4 post-merge verification 完了** (Ada from scratch smoke + TF32 比較 + bf16 smoke + audio_parity Tier 4)
- [ ] 実機 smoke の log / 計測値が WandB に保管されている
- [ ] (任意) `v2.0.0rc1` pre-release で early access フィードバック取得済

### Exit Criteria

- [ ] v2.0.0 release tag が GitHub に存在
- [ ] PyPI に `piper-plus==2.0.0` が published
- [ ] Docker Hub / GHCR に `piper-plus-cpu:2.0.0` / `piper-plus-gpu:2.0.0` が push 済
- [ ] `docs/migration/v1.12-to-v2.0.md` が存在し、 breaking change と migration step を明示
- [ ] Issue #527 が closed、 達成サマリコメント済
- [ ] CHANGELOG.md `[2.0.0]` セクションが完成
...
| 3rd party ユーザからの bug 報告 | リリース後 7 日以内に critical bug 1 件以上 | hotfix release (2.0.1) で対応、 重大なら 1.12 LTS 維持を別 issue で検討 |
...
```text
              ...
              │ 1 day            │  (docker build + Trivy + CI)
              │                  │  ← 実機 smoke は post-merge (DR-009)
              └────────┬────────┘
                       ▼
              ┌─────────────────┐
              │ M4: 新 GPU 最適化 │  Phase 4
              │ 1 day            │  (TF32 + bf16 コード変更 + docs)
              │                  │  ← 実機 smoke は post-merge (DR-009)
              └────────┬────────┘
                       ▼
        ┌─────────────────────────────┐
        │ 実機環境準備完了             │  (OQ-01/02 の実体化)
        │ + post-merge verification    │  (Ada/RTX5090/T4 smoke)
        └──────────────┬──────────────┘
                       ▼
              ┌─────────────────┐
              │ M5: リリース     │  Phase 外
              │ 0.5 day          │  (v2.0.0、 実機確認後)
              └─────────────────┘

合計: 3 日 (人日、 実装) + 環境準備待ち + post-merge verification 0.5 日

> **DR-009 により M3 / M4 は「実機環境準備中」 でも実装・merge 可能**。
> M5 (release) は実機 smoke 完了が必須ゲート。
```
...
- [ ] PyPI に v2.0.0 が visible
- [ ] Docker image tag `2.0.0` が pull 可能
- [ ] Issue #527 が close + 達成サマリ comment
- [ ] CHANGELOG `[2.0.0]` が release date 入り
