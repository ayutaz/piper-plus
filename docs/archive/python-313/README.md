...
| 文書 | 内容 | 主な読者 |
|---|---|---|
| [`README.md`](README.md) | (本ファイル) **実装計画 (HOW STEP-BY-STEP)** — Phase 分割、 影響範囲、 diff サンプル、 rollback 手順、 PR テンプレ | 実装者 |
| [`requirements.md`](requirements.md) | **要求定義 (WHAT / WHY)** — 目的、 機能/非機能要求、 スコープ、 成功基準、 制約、 前提、 ステークホルダー、 リスク、 互換性影響 | stakeholder / レビュアー |
| [`specifications.md`](specifications.md) | **要件定義 (HOW MUCH / HOW PRECISELY)** — システム構成要件、 機能要件詳細 (受入基準付き)、 非機能要件 (測定可能閾値)、 データ要件、 外部 IF 要件、 テスト要件、 移行要件、 運用要件、 トレーサビリティ | 設計者 / 実装者 |
| [`milestones.md`](milestones.md) | **マイルストーン定義 (WHEN / WHAT DELIVERABLE)** — M1-M5 の進捗管理、 deliverables、 entry/exit 基準、 リスクゲート、 ステークホルダー、 累積トラッキング表 | プロジェクト管理 / 実装者 |
| [`open-questions.md`](open-questions.md) | **未決事項 (DECISIONS NEEDED)** — 実装前に stakeholder で決めるべき 14 件の OQ-XX、 各選択肢と推奨案 | stakeholder / リポジトリオーナー |
...
```text
要求定義 (requirements.md)         "なぜやるか + 何を達成するか"
        │   WHAT/WHY、 stakeholder 承認用
        ▼
要件定義 (specifications.md)       "どこまで・どの精度で実現するか"
        │   HOW MUCH/HOW PRECISELY、 受入基準と測定閾値を pin
        ▼
マイルストーン (milestones.md)     "いつ・どんな単位で deliverable を出すか"
        │   WHEN/WHAT DELIVERABLE、 M1-M5 + リスクゲート + 進捗 tracking
        ▼
実装計画 (README.md)               "どの順序・どのファイルを書き換えるか"
            HOW STEP-BY-STEP、 Phase 別 diff + rollback + PR テンプレ
```
