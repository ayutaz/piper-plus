...
| **KO** | **520行** | 925行 | — | Hangul分解 + IPA 完全実装 |
...
```
テキスト → (小文字化) → 1文字ずつ phonemeIdMap ルックアップ → phoneme_ids
```
...
- **WASM:** `你 好` → phonemeIdMap に漢字がないため、不正なトークン列
...
- **WASM:** `b o n j o u r` → 7文字個別に渡され、鼻母音なし
...
- **WASM:** `o b r i g a d o` → 文字そのまま (r多型性なし、母音弱化なし)
...
> **Note:** ES が「動く」と報告されたのは、スペイン語の正書法が比較的音素に近く、文字パススルーでも偶然一部一致するため。完全な G2P ではない。
