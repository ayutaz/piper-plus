...
- [x] `src/android/` ディレクトリ作成
- [x] Gradle Kotlin DSL設定 (settings.gradle.kts, build.gradle.kts)
- [x] ONNX Runtime Android AAR依存追加
- [x] CMakeLists.txt: Android JNIビルド対応追加
- [x] クロスコンパイルスクリプト (arm64-v8a, armeabi-v7a, x86_64)
- [x] GitHub Actions: ネイティブビルド + AARビルド + テスト

### Phase 2: JNIブリッジ + コア推論

- [x] `piper_jni.cpp`: JNIエントリーポイント実装
...
- [x] Kotlin API: `PiperTts`, `PiperConfig`, `PiperAudio`
- [x] Speaker ID / Language ID 対応
- [x] Prosody features 対応
- [x] ユニットテスト

### Phase 3: Android統合

- [x] `PiperTtsService` (TextToSpeechService) 実装
- [x] AndroidManifest.xml TTSエンジン登録
- [x] AudioTrackリアルタイム再生
- [x] Kotlin Coroutines非同期推論 (viewModelScope統合)
- [x] ストリーミング出力 (Flow + buffer)

### Phase 4: サンプルアプリ

- [x] Jetpack Composeサンプルアプリ
- [x] テキスト入力 → 音声合成 → 再生のデモ
- [x] 言語切り替え・話者切り替えUI

### Phase 5: 配布

- [x] Maven Central公開設定 (vanniktech plugin)
- [x] GPG署名設定
- [x] GitHub Actions自動リリースワークフロー
- [x] KDoc APIドキュメント
- [x] ProGuard/R8設定
