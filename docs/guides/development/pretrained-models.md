...
> ✅ **v2.0 (`dev`) で自動移行されます**: Zero-Shot TTS ([PR #579](https://github.com/ayutaz/piper-plus/pull/579)) が `MBiSTFTGenerator.cond` を Multi-scale FiLM 化して出力チャネルを 2 倍にしましたが、 [Issue #616](https://github.com/ayutaz/piper-plus/issues/616) / [PR #621](https://github.com/ayutaz/piper-plus/pull/621) で **旧 ckpt の自動移行** が入り、 FT / ONNX エクスポート / 学習再開の全経路で load 時に透過的に変換されます (旧 `x + cond(g)` と bit 単位で同一の出力になることを公開 ckpt 実機で確認済み — max abs diff = 0.0、 音質影響なし)。 互換タグ `v1.13.0` への checkout は不要になりました。 詳細: [docs/migration/v1.12-to-v2.0.md](../../migration/v1.12-to-v2.0.md)。
