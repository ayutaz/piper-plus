...
| `docker/python-inference/Dockerfile` | Python推論 (ONNX Runtime, GPU/CPU両対応) | ✅ Phase 1で書き直し、Phase 2でuv化、Phase 6でCUDA化 |
| `docker/python-train/Dockerfile` | Python学習 | ✅ Phase 1で依存修正、Phase 2でuv化 |
| `docker/cpp-inference/Dockerfile` | C++バイナリ推論 (プロダクション) | ✅ Phase 4でCMake ExternalProject方式に全面書き直し、Phase 5でCPU専用化 |
| `docker/cpp-inference/Dockerfile.simple` | C++推論 簡易版 | 維持 |
| `docker/cpp-dev/Dockerfile` | C++開発環境 | ✅ Phase 2でUSE_CUDA定義追加、Phase 5でCPU専用化 |
...
| `docker/python-inference/inference.py` | 推論スクリプト | ✅ Phase 1で書き直し、Phase 6で`--device`引数追加 |
...
| `.github/workflows/docker-test.yml` | PRスモークテスト (python-inference, cpp-inference, webui) | ✅ Phase 3で責務整理・WebUI追加、Phase 6でcpp-inference追加 |
...
| Docker内 `uv pip install --system ".[inference]"` | Docker環境 (CPU) | ✅ extras_require経由 |
| Docker内 `uv pip install --system ".[inference-gpu]"` | Docker環境 (GPU) | ✅ Phase 6で追加 |
...
- `docker-test.yml`: PR時のスモークテスト(python-inference, cpp-inference, webui)
...
| `docker-test.yml` | ✅ 維持・拡充 | PR時スモークテスト (python-inference, cpp-inference, webui) |
...
| 4-4 | ~~ONNX Runtimeバージョン自動検出によるGPU版インストール~~ → CPU専用化により削除 | ✅ |
| 4-5 | `.gitattributes` 追加（*.sh text eol=lf）+ 全シェルスクリプトのCRLF→LF変換 | ✅ |
| 4-6 | cpp-inference推論テスト（RTF=0.094, en_US-amy-lowモデル） | ✅ |

### Phase 5: C++ Docker イメージの CPU 専用化 ✅ 完了

C++ 推論は CPU で十分な性能（RTF=0.094）が確認されたため、GPU 依存を完全に除去。

| # | タスク | 状態 |
|---|--------|------|
| 5-1 | cpp-inference: ベースイメージを `nvidia/cuda:12.4.1-cudnn-runtime-ubuntu22.04` → `ubuntu:22.04` に変更 | ✅ |
| 5-2 | cpp-dev: ベースイメージを `nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04` → `ubuntu:22.04` に変更 | ✅ |
| 5-3 | ONNX Runtime GPU 版インストール処理を削除（CPU 版は ExternalProject で自動ビルド済み） | ✅ |
| 5-4 | cpp-dev/build.sh: CUDA 関連オプション削除 | ✅ |
| 5-5 | cpp-inference/entrypoint.sh: CUDA 環境設定を削除 | ✅ |
| 5-6 | cpp-inference/test.sh: CUDA テストセクションを削除 | ✅ |
| 5-7 | docker/README.md: GPU 関連記述を CPU 専用に更新 | ✅ |
| 5-8 | CI/CD: CUDA 関連設定の確認・不要であれば削除 | ✅ |

### Phase 6: Python推論 GPU対応 + CI強化 ✅ 完了

**コミット**: `5bba3eb`, `607f073`, `9318455`, `eebfea4`

| # | タスク | 状態 |
|---|--------|------|
| 6-1 | Python推論DockerのCUDA化（`nvidia/cuda:12.4.1-cudnn-runtime-ubuntu22.04` ベースに変更、CPU/GPU両対応） | ✅ |
| 6-2 | `--device auto/cpu/gpu` 引数を `infer_onnx.py` と `inference.py` に追加 | ✅ |
| 6-3 | `pyproject.toml` に `inference-gpu` extras 追加（`onnxruntime-gpu` を含む） | ✅ |
| 6-4 | `setup.py` に `inference-gpu` extras 追加（Dockerfileが参照する`setup.py`にも反映） | ✅ |
| 6-5 | cpp-inference Docker CIテスト追加（`docker-test.yml` に `test-cpp-inference` ジョブ追加） | ✅ |
| 6-6 | Ubuntu 22.04の`pip`で`--break-system-packages`が使えない問題を修正 | ✅ |
