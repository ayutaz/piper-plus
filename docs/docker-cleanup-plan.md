...
| `docker/cpp-inference/Dockerfile` | C++バイナリ推論 (プロダクション) | ✅ Phase 2で--enable-shared追加 |
| `docker/cpp-inference/Dockerfile.simple` | C++推論 簡易版 | 維持 |
| `docker/cpp-train/Dockerfile` | C++開発環境 | ✅ Phase 2でUSE_CUDA定義追加 (CMakeLists.txt) |
...
| `docker/cpp-train/build.sh` | C++ビルドスクリプト | 維持 |
| `docker/cpp-train/test.sh` | C++テスト | 維持 |
...
| `docker/README.md` | ドキュメント (**現状と乖離**) | Phase 3で更新予定 |
...
| ファイル | 対象 | 状態 |
|----------|------|------|
| `.github/workflows/build-docker-images.yml` | Dockerイメージビルド | Phase 3で整理予定 |
| `.github/workflows/docker-build-minimal.yml` | 最小Dockerビルド | Phase 3で整理予定 |
| `.github/workflows/docker-build.yml` | Dockerビルド | Phase 3で整理予定 |
| `.github/workflows/docker-test.yml` | Dockerテスト | Phase 3で整理予定 |
...
| M-3 | `docker/cpp-inference/Dockerfile` + `Dockerfile` | ビルド方式不統一 | 📋 Phase 3 |
| M-4 | 全Dockerfiles | ONNX Runtimeバージョン不一致 | ✅ Phase 2 (シンボリックリンク動的化) |
| M-5 | `docker/cpp-inference/Dockerfile` | OpenJTalk共有ライブラリCOPY不整合 | ✅ Phase 2 (`--enable-shared`追加) |
| M-6 | `Dockerfile` + `CMakeLists.txt` | ARM64最適化レベル `-O0` | ✅ Phase 2 (`-O2`に修正) |
| M-7 | CI/CDワークフロー全体 | 責務不明確、WebUI未含有 | 📋 Phase 3 |
| M-8 | CI/CDワークフロー全体 | ghcr.io自動push未設定 | 📋 Phase 3 |
| M-9 | `docker/cpp-train/Dockerfile` | `USE_CUDA=ON` が未定義 | ✅ Phase 2 (`option(USE_CUDA ...)`追加) |
...
| L-3 | `Dockerfile` (ルート) | `COPY . .` で不要ファイル含有 | 📋 Phase 3 |
| L-4 | `Dockerfile` (ルート) | ccache効果なし | 📋 Phase 3 |
| L-5 | `Dockerfile` (ルート) | piper-phonemize master HEAD固定 | 📋 Phase 3 |
| L-6 | `docker/webui/docker-compose.yml` | ヘルスチェックでcurl使用 | ✅ Phase 1 (Python urllib化) |
| L-7 | `src/python/Dockerfile` | 完全に廃止状態 | ✅ Phase 1 (削除済み) |
| L-8 | `docker/README.md` | 現状と乖離 | 📋 Phase 3 |
...
| カテゴリ | 対応内容 |
|----------|---------|
| CI/CD | ワークフロー整理・統合、ghcr.io自動push、WebUI追加 |
| ビルド最適化 | `.dockerignore` 改善、ccache、piper-phonemizeコミット固定 |
| ドキュメント | `docker/README.md` 全面書き直し |
| C++ビルド方式 | ExternalProject vs 手動autotools の統一検討 |
...
| 3-5 | C++推論ビルド方式の統一検討 (ExternalProject vs 手動autotools) | `docker/cpp-inference/Dockerfile`, `Dockerfile` | M-3 |
| 3-6 | `docker/README.md` の全面書き直し | `docker/README.md` | L-8 |
| 3-7 | 全Dockerイメージの最終統合テスト | 全体 | — |
