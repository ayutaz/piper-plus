...
- `--precision`: 16-mixed (fp16混合精度、デフォルト) が速度とメモリ効率の観点から推奨されます。bf16-mixed は torch.stft との互換性問題があるため、PyTorchのバージョンに注意が必要です。オプション: 16-mixed (デフォルト), 32-true, bf16-mixed
