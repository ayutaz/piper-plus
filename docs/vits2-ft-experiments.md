# VITS2 実験記録 (2026-03-25 〜 2026-04-04)
...
**結論: VITS2は現時点の実装ではVITS1の品質に到達できていない。** VITS2固有機能（A〜D）のどれが悪影響を与えているか個別検証が必要。
...
- **VITS1+Eのみ (A-Dなし): 未検証** → 品質維持で軽量化できる可能性あり
...
| テスト | 構成 | 追加機能 | 目的 |
|--------|------|---------|------|
| T0 | VITS1そのまま (20ep) | なし | ベースライン |
| T-E | VITS1 + E | Mel Posterior Encoder | 軽量化の可否（品質維持されるか） |
| T1 | VITS1 + A | Deterministic DP (no-sdp) | SDPなしの品質影響 |
| T2 | VITS1 + A + B | + DurDisc | 3-optimizer干渉の影響 |
| T3 | VITS1 + C | Speaker-Conditioned Encoder | enc_p品質への影響 |
| T4 | VITS1 + D | Noise-Scaled MAS | アライメント品質への影響 |
