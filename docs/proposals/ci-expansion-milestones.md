...
**ステータス**: Top 10 + §3 軽量 5 件は PR #511 で実装完了

> このドキュメントは [ci-expansion-2026-05.md](./ci-expansion-2026-05.md) の **Top 10 項目** を実装単位のマイルストーンに分解したもの。 GitHub Milestone / Project board と紐づけできる粒度で、 各 M に **目的 / 成功基準 / タスク / 依存 / リスク / 関連ファイル** を明記する。 Top 10 + §3 軽量 5 件 (OpenSSF Scorecard / README parity / CHANGELOG validator / CI observability / Rust miri) は PR #511 で実装完了。 M-Stretch 候補 (S1-S8) の詳細実装方針は [ci-expansion-deferred-items.md](./ci-expansion-deferred-items.md) (Distroless / SLSA L3 / SHA drift / CLI help / spec sync / mkdocs / doctest / test aggregation の 8 項目) と本ドキュメント §M-Stretch の表を参照。
...
Top 10 外の領域で、 unlimited CI を本気で活かすなら検討に値するもの。 親 doc §3 から抜粋。 各々独立した別 milestone として議論可能。 S4 / S5 を含む 8 項目の詳細実装方針は [ci-expansion-deferred-items.md](./ci-expansion-deferred-items.md) に「Claude Code 実装前提」 で再評価済。
...
| 2026-05-18 | Top 10 + §3 軽量 5 件を PR #511 で実装完了。 個別実装チケット (docs/tickets/) は役割終了により削除、 M-Stretch S1-S8 の詳細実装方針は [ci-expansion-deferred-items.md](./ci-expansion-deferred-items.md) へ集約 | #511 |
