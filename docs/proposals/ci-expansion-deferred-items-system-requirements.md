...
**ステータス**: 部分実施 — Tier 1 (#3 / #4) は PR #513 で実装完了、 Tier 2 (#5 spec sync gate 5 件) は PR #517 で実装完了。 残り Tier 2 (#7 doc examples 3 phase) と Tier 3 (#1 / #2 / #6 / #8) は draft (未着手)
...
> **Tier 1 (3.1 / 3.2) は実装完了 (PR #513、 2026-05-19 merge / commit `c29a87ec`)**
> Rekor verify / Action SHA drift detector / 6 runtime CLI help canonical の 3 機能が informational tier で稼働中。 各機能に silent-zero defensive log + sticky comment + drift 時 Issue auto-create を内蔵、 4 週間 false-positive 0 確認後の blocker 昇格を user 判断に委ねる運用に入っている。 以下の I/O 仕様 / データ構造 / 処理シーケンス記述は実装の正解 baseline として保持。
...
> **実装完了 (PR #517、 2026-05-19 merge / commit `f3ef12cd`)** — 5 spec すべて drift gate 整備済み。 以下表は実装後の正解 baseline として保持。

| spec | 実装結果 |
|------|---------|
| `release-versions.toml` | **完了 (既存実装で closeout)** — `scripts/check_version_manifest_sync.py` + `.github/workflows/version-consistency.yml` + pre-commit hook が 10 polyglot manifests を sync。 `[meta].direction = "post-hoc"` を明文化 |
| `model-sha256-manifest.toml` | **完了 (構造健全性 scope)** — `scripts/check_model_sha256_manifest.py` が 6 model entry / placeholder sentinel / `[meta]` 必須 key / `MAX_KNOWN_SCHEMA_VERSION = 2` forward-compat を検証。 実 SHA256 ↔ 8 runtime loader 突合は publish パイプライン整備後の別 PR |
| `artifact-retention-contract.toml` | **完了 (sweep + mode=fail flip まで同 PR で完了)** — `scripts/check_artifact_retention.py` が 40 workflow / 63 upload step を walk、 category 許容値域 [1, 7, 30, 90] と突合。 同 PR 内で baseline 違反 6 件 (4× 14d / 1× 5d / 1× 365d) を sweep、 `[meta].mode = "fail"` に flip |
| `swift-g2p-contract.toml` | **完了 (既存実装で closeout)** — `scripts/check_swift_g2p_contract.py` + `contract-gates-extended.yml` (swift-g2p job) + pre-commit hook が ABI / PUA / xcframework / default-language を検証。 `[meta].direction = "pre-impl"` を明文化 |
| `test-flake-retry-contract.toml` | **完了 (4 runtime scope)** — `scripts/check_test_flake_retry.py` が python / rust / go / csharp について phase status + pyproject 依存 + `--reruns` flag + `retry_count_max = 2` 不変条件 + 3 invariants を検証。 WASM / C++ / Kotlin / Swift 拡張は別 spec で別 PR |
...
| 2026-05-19 | 0.2 | PR #513 (Tier 1: `#3` / `#4`) と PR #517 (Tier 2: `#5` 5 spec) の merge を反映。 §3 (Tier 1 詳細) 冒頭と §4.1 (#5 spec gate) に「実装完了 (PR #N、 commit)」 marker を追記、 §4.1 の表を「詳細化時の着目点」 から「実装結果」 に書き換え (5 spec それぞれの実装方式と scope を記録)。 全体カウントを最新化 (spec 31→32、 check 62→66)。 基準ブランチ / HEAD を `docs/ci-expansion-deferred-items-organize` / `00ea9fde` → `dev` / `eee9d5fb` に更新。 | Claude Code |
