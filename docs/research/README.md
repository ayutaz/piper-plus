...
- [5 軸改善調査 2026-07-14](improvement-survey-2026-07-14-five-axes.md) — ユーザー指定 5 軸 (話者類似度 / アクセント正確性 / 明瞭性 / 学習・推論高速化 / メモリ最適化) の補完調査。 Web 調査 5 並列 + **リポジトリ実装状態監査** (file:line で実装済み/未実装を確定) の突き合わせ。 新規論点: SECS 評価バイアス (SCL と同一 encoder)、 **FP16 配布の x86 CPU 逆効果疑い** (sherpa-onnx 実測)、 ECAPA2 非商用ライセンス判明 (06-15 A-6 の訂正)、 アクセント記号注入は実装済みの確認、 duration floor / bucketing / marine-plus 等。 Phase 0 (計測基盤) → Phase 3 のロードマップ + 既存 doc 訂正事項 (§9) 付き。
- [改善調査 統合レポート 2026-06-15](improvement-survey-2026-06-15.md) — 現行の主参照源 (A-6 ECAPA2 は上記 2026-07-14 §9 の訂正を参照)。 2 つのスナップショット (2026-06-03 / v1.12.0、 2026-06-15 / v1.13.0) を統合した、 5 軸 (A. 音声品質モデル / B. ランタイム・エッジ / C. 多言語 G2P / D. エコシステム / E. 運用ガイダンス) の **31 アクション** ロードマップ。 短期 (3〜6 ヶ月) と中期 (1 年) で分類、 KPI 案・統合判断履歴付き。 統合一次ソース 47 件 / 43 主張確定 / 7 棄却 / DRAFT PR (#222 #355 #386) + Open PR #537 (ready-for-review、 v2.0.0 候補) との重複除外済。 §G オープンクエスチョンは Phase 4 deep-research で全件分類完了 (本表 9 + companion 11 = **全 20 件、 RESOLVED 5 / CONVERGED 7 / IRREDUCIBLE 8**、 文献調査による closure 完了、 残りは PoC 待ち)。
