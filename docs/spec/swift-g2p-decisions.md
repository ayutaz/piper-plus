...
> **Status:** Resolved — implementation completed on branch `feat/swift-g2p-387` (2026-05-06).
> All decision items below have been addressed; see the [Resolution Summary](#resolution-summary-2026-05-06) at the bottom.
> **Companion:** [docs/spec/swift-g2p.md](./swift-g2p.md), [docs/guides/swift-g2p-integration.md](../guides/swift-g2p-integration.md), [docs/spec/swift-g2p-contract.toml](./swift-g2p-contract.toml) (FFI/ABI contract)
...
|----|------|-----------|-------|
| **D-2** | 辞書埋込戦略 | **Option A** 採用。`bundled-dicts` feature 新設、`include_str!`/`include_bytes!` で cmudict + pinyin を埋込 | `src/rust/piper-plus-g2p/Cargo.toml` (feature)、`src/{english,chinese}.rs::new_bundled()` |
| D-3 | crate version | workspace 共通 0.4.0 を採用 (workspace 整合優先) | `src/rust/piper-plus-g2p/Cargo.toml` |
| D-5 | bundled-dicts default | iOS xcframework build 時のみ明示 enable。default features には含めず | `Cargo.toml` `default = [...]` (bundled-dicts なし)、CI は `--features ...,bundled-dicts,ffi` |
| **D-9** | 第三者ライセンス表示 | `THIRD_PARTY_LICENSES.md` に CMU dict (BSD-style) と pypinyin (MIT) を専用セクションで明記 | `src/rust/piper-plus-g2p/THIRD_PARTY_LICENSES.md` |
| **D-10** | Cross-runtime golden test | 同一 fixture (`tests/fixtures/g2p/phoneme_test_cases.json`) を `#filePath` 経由で参照する `GoldenPhonemeTests.swift` で実装 | `tests/PiperPlusG2PTests/GoldenPhonemeTests.swift` |
| D-13 | PR 分割方針 | 1 PR にまとめる方針 (5 commits、変更が密結合のため細分化のメリット薄) | branch `feat/swift-g2p-387` |
| **D-15** | jpreprocess iOS 動作検証 | `cargo build --target aarch64-apple-ios -p piper-plus-g2p --features all-languages,naist-jdic,bundled-dicts,ffi` 成功、`libpiper_plus_g2p.a` 84 MB、`piper_plus_g2p_*` 5 シンボル定義済 | local + CI `release-shared-lib.yml::build-g2p-ios` |

### Additional follow-ups addressed in same branch

- **`bundled-dicts` 配慮 default langs** (FFI): `default_languages()` 関数を `src/ffi.rs` に追加。feature 有効言語のみ含める。`zh` は `chinese + bundled-dicts` 同時有効時のみ default に入る (path-based init を回避)。
- **Concurrency test** (Swift): `ConcurrencyTests.swift` で `Phonemizer.@unchecked Sendable` を多 Task 並列実行で実証。
- **Cross-reference**: `docs/guides/ios-integration.md` の Quick links / Distribution Selection 表に G2P-only 行を追加。
- **README**: `README.md` (ja) / `README_EN.md` の Interfaces セクションに iOS Swift G2P (SPM) 行を追加。
- **Crate README**: `src/rust/piper-plus-g2p/README.md` の version 表記を `0.4` に統一、Feature Flags 表に `ffi` / `bundled-dicts` を追加、C FFI 章を iOS 統合に合わせて拡充。
- **Source-of-truth 統一**: G2P JSON 辞書 (`cmudict_data.json` / `pinyin_*.json`) を `src/cpp/` から `src/rust/piper-plus-g2p/data/` へ移動。CMake 2 ファイル + CI 3 ファイルを更新。
- **CI で PR 検証**: `release-shared-lib.yml` に `pull_request` trigger 追加 + 既存 jobs に `if: github.event_name != 'pull_request'` を付与。PR では `build-g2p-ios` / `assemble-g2p-xcframework` のみ smoke run、文法エラー / path ミスを PR 段階で検出可能。
- **Swift CI 新設**: `.github/workflows/swift-g2p-ci.yml` で macOS runner 上に local macOS xcframework を組み立て、CI 専用 Package.swift で `swift test --filter PiperPlusG2PTests` を実行。Swift wrapper / Phonemizer / Golden / Concurrency tests を **PR で自動検証**できるようにした (xcframework 未 publish 状態でも回せる)。
- **Rust test 追加**: `default_languages()` / `register_one()` のテストを `ffi.rs` に 7 件追加。lib tests 396 → 403。
- **PhonemizerTests 強化**: Golden fixture と整合する具体トークン照合 / PUA tone 検出に書換。

---

## Post-merge follow-ups (2026-05-07)

PR レビューで挙がった「ユーザー視点の問題点」を同じ branch で潰したパッチセット。実装に対する追加 decisions:

| ID | Item | Resolution | Where |
|----|------|-----------|-------|
| **F-1** | macOS slice の追加 | `build-g2p-ios` matrix を `build-g2p-apple` に rename し、3 つ目のエントリ `macos-arm64_x86_64` (aarch64-apple-darwin + x86_64-apple-darwin) を追加。`assemble-g2p-xcframework` も 3 slice を `xcodebuild -create-xcframework` に渡すように拡張 | `.github/workflows/release-shared-lib.yml` |
| **F-2** | xcframework artifact 名の rename | `libpiper_plus_g2p-ios-` → `libpiper_plus_g2p-apple-`。macOS slice 同梱を反映。`Package.swift` の `binaryTarget(url:)` も連動 update。**v1.14.0 が初回 publish なので旧 `-ios-` 名は維持しない** | `release-shared-lib.yml` / `Package.swift` / `docs/spec/swift-g2p*.md` |
| **F-3** | Package.swift platforms 拡張 | `[.iOS(.v15)]` → `[.iOS(.v15), .macOS(.v13)]`。consumer は macOS から `.product(name: "PiperPlusG2P", ...)` のみ依存可能。PiperPlus (合成) を macOS 参照すると binaryTarget 解決で失敗する点はトレードオフとして許容 | `Package.swift` |
| **F-4** | `G2PError` に診断情報追加 | `initializationFailed(requestedLanguages: [Language])` / `phonemizeReturnedNull(language: Language)` に rewrite。multi-language caller が call site を辿らずに失敗箇所を特定できる。テストも `PhonemizerTests.swift` で 2 件追加 | `Sources/PiperPlusG2P/G2PError.swift`, `Sources/PiperPlusG2P/Phonemizer.swift`, `tests/PiperPlusG2PTests/PhonemizerTests.swift` |
| **F-5** | `examples/swift-g2p/` サンプル追加 | `HelloG2P` CLI (macOS) の最小 SPM project + README。`swift run HelloG2P` で 8 言語動作確認。tag publish 前は `path:` モードへの切替手順を README に明記 | `examples/swift-g2p/HelloG2P/`, `examples/swift-g2p/README.md` |

これらは User-perspective Review (前セッション) で挙がった Concerns B / C / E に対応する。Concern A (tag publish) と G (review approval) は branch 外作業、F (App Clip lite slice) と D (JSON decode 最適化) は別 PR の規模で本ブランチでは保留。
