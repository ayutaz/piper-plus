...
| 状態 | 対象 |
|------|------|
| ❌ サポート切れ (要対応) | Node.js engines `>=18.0.0`, piper-python `python>=3.8`, .NET 9 (別 PR で .NET 10 LTS に直接移行) |
...
- **状況:** Node.js 18 は **2025-04-30 で EOL**。なお Node.js 20 (2026-04 EOL 経過) と 22 (Maintenance LTS、2027-04 EOL 予定) も新規採用は非推奨
- **CI で使用しているバージョン:** 24.x (Active LTS、2028-04 まで)
- **対応:** `engines.node` を `>=24.0.0` (Active LTS、CI と一致) に引き上げる
...
- **対応:** **別 PR で全プロジェクトを `net10.0` LTS に直接移行** (Cli/Tests/Core 一括)
- **判断根拠:** `net8.0` LTS にダウングレードしても 2026-11 で再度 EOL を迎えるため、`net10.0` LTS (2028-11 まで) に直接移行する方が二度手間を回避できる
...
- **対応:** **別 PR で `net10.0` LTS に直接移行** (.NET 9 EOL 対応とまとめて Cli/Tests/Core 一括)
...
| **P0** | Node.js `engines.node>=18.0.0` → `>=24.0.0` | XS | 2ファイル修正 (CI と一致) |
| **P0** | .NET Cli/Tests/Core 全て `net10.0` LTS に直接移行 | M | 別 PR で対応 (Issue #371 を吸収)。Unity 対応は不要 ([検討経緯](#unity-対応の検討経緯)) |
