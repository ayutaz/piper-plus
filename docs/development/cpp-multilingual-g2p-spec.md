...
> Date: 2026-03-16
> Last updated: 2026-03-16 (Phase 1+2 complete)
...
| 多言語音素化ルーティング | 7言語レジストリ | **JA=OpenJTalk, 他=eSpeak** | :white_check_mark: Phase 2 |
| コードスイッチング | セグメント分割+言語別処理 | **実装済み** | :white_check_mark: Phase 2 |
| ES/FR/PT ネイティブG2P | ルールベース | eSpeak代替中 | Phase 3 |
| EN ネイティブG2P | g2p-en (CMU辞書) | eSpeak代替中 | Phase 4 |
| ZH ネイティブG2P | pypinyin | eSpeak代替中 | Phase 4 |
| KO ネイティブG2P | g2pk2 + ハングル分解 | eSpeak代替中 | Phase 4 |
...
| **Phase 3** | ES/FR/PT ルールベースG2Pポート（Python精度一致） | :construction: TODO |
| **Phase 4** | EN CMU辞書 / ZH pypinyin辞書 / KO ハングル分解 | :construction: TODO |
...
## 4. Phase 3: ルールベースG2Pポート (ES/FR/PT) — :construction: TODO
...
## 5. Phase 4: 外部依存言語 — :construction: TODO
...
## 6. C++ アーキテクチャ設計 — Phase 3+ で実装

> Phase 2 では `language_detector.hpp/cpp` を `src/cpp/` に直接配置（piper.cpp と同階層）。
> Phase 3+ で Phonemizer ABC / Registry パターンに移行する場合、以下の構成を採用。
...
→ **対策**: Phase 3+ で Phonemizer ABC/Registry を実装する際に `std::call_once` を使用。Phase 2 では Registry を使わず `piper.cpp` 内で直接ルーティングしているため影響なし。
