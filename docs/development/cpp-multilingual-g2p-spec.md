...
> Date: 2026-03-16
> Last updated: 2026-03-17 (Phase 1-4 + レビュー修正2ラウンド完了, commit `0f9429e`)
...
| EN OOV フォールバック | g2p-en ニューラルモデル | **eSpeak フォールバック** | :white_check_mark: Review R2 |
| 非JA prosody 抽出 | 全6言語で抽出 | **`computeNonJaProsody()` 5言語** | :white_check_mark: Review R2 |
| PUA固定マッピング同期 | `token_mapper.py` 89エントリ | **C++ `puaToPhoneme` 対応** | :white_check_mark: Review R1 |
...
| **Review R1** | CRITICAL 8件 + MAJOR 2件修正 (PUA同期, BOS/EOS, 範囲検証等) | :white_check_mark: **Done** (`b1dfd2a`) |
| **Review R2** | 非JA prosody抽出 + EN OOV eSpakフォールバック | :white_check_mark: **Done** (`0f9429e`) |
...
| M4 | MAJOR | EN | OOV語のニューラル G2P 推論なし | :white_check_mark: eSpeak フォールバック実装 (R2) |
...
| M7 | MAJOR | 全言語 | 旧バイリンガルモデル (`phoneme_type: bilingual`) との互換性未検証 | :white_check_mark: 調査完了: 問題なし (R2) |
| M11 | MAJOR | EN/ZH/ES/FR/PT | 非JA言語の prosody 特徴量が常にゼロ | :white_check_mark: `computeNonJaProsody()` 実装 (R2) |
...
| Review R1 | :white_check_mark: Done (`b1dfd2a`) | C1-C8, M1, M5(PT) 適用済み | :white_check_mark: |
| Review R2 | :white_check_mark: Done (`0f9429e`) | M4-A, M7, M11 適用済み | :white_check_mark: |

### 7.4 課題ステータス一覧

| ID | 状態 | 概要 |
|----|:----:|------|
| M1 | :white_check_mark: Applied | ES intervocalic x 母音判定修正 |
| M2 | :hourglass: 将来課題 | KO g2pk2 コアルール（現行モデルにKO未含有のためスキップ） |
| M3 | :hourglass: 将来課題 | `--dominant-language` CLI オプション |
| M4 | :white_check_mark: R2で対応 | eSpeak フォールバック実装済み。GRU ニューラルモデルは将来課題 (M4-B) |
| M5 | :white_check_mark: PT / :hourglass: ES,FR | NFD→NFC 正規化。PT は適用済み、ES/FR は未対応 |
| M6 | :hourglass: 将来課題 | ZH 声調サンディ拡大（フレーズ辞書外の連続語） |
| M7 | :white_check_mark: 調査完了 | 互換性問題なし。IDs 0-96 完全一致確認済み (Section 11.3) |
| M11 | :white_check_mark: R2で対応 | 全5言語の prosody 抽出実装済み (ZH声調/EN,ES,PTストレス/FR語末ストレス) |

**残る将来課題**:
1. **M2**: 韓国語 g2pk2 コアルール（現行モデルに KO 未含有のためブロッカーではない）
2. **M3**: `--dominant-language` CLI オプション
3. **M4-B**: EN GRU ニューラルモデルの C++ 移植（eSpeak フォールバックは実装済み）
4. **M5 (ES/FR)**: NFD→NFC 正規化の追加
5. **M6**: ZH 声調サンディ拡大
...
### 8.2 PUA固定マッピング (U+E000-E058, 全89エントリ)

`token_mapper.py` の `FIXED_PUA_MAPPING` で定義。C++側の `puaToPhoneme` マップと1:1対応。

| Codepoint範囲 | エントリ数 | 言語 | 内容 |
|-------------|----------|------|------|
| U+E000-E015 | 22 | JA | 長母音・口蓋化子音・促音 |
| U+E016-E018 | 3 | JA | 疑問詞マーカー (?!, ?., ?~) |
| U+E019-E01C | 4 | JA | Nバリアント (N_m, N_n, N_ng, N_uvular) |
| U+E01D | 1 | ES | `rr` (顫動音) |
| U+E01E | 1 | ZH/FR | `y_vowel` (前舌円唇母音) |
| U+E020-E04A | 43 | ZH | 送気音・そり舌音・複合韻母・声調マーカー (tone1-5) |
| U+E04B-E052 | 8 | KO | 緊張子音・無解放終声 |
| U+E054-E055 | 2 | ES/PT | tʃ, dʒ (破擦音) |
| U+E056-E058 | 3 | FR | 鼻母音 (ɛ̃, ɑ̃, ɔ̃) |
| **合計** | **89** | | |

**注意**: U+E053 は未使用（将来の拡張用）。`_PUA_START` は `0xE059` に設定済み。
C++ の `puaToPhoneme` マップおよび各言語 phonemizer の PUA 定数と完全同期済み (Review R1 で修正)。
...
推奨対応優先度（実施状況）
  ...
  Phase A: ブロッカー修正 (C1-C8) — :white_check_mark: 全件完了 (Review R1, `b1dfd2a`)

  1. :white_check_mark: token_mapper.py に全言語の PUA 固定マッピングを追加 (C1, C7, C8)
  2. :white_check_mark: 中国語フレーズ辞書パーサーの修正 (C2)
  3. :white_check_mark: LanguageID 範囲検証の追加 (C4)
  4. :white_check_mark: eSpeak フォールバック時の BOS/EOS ストリップ (C5)
  5. :white_check_mark: multilingual + language_id_map 欠如時のエラー (C6)
  6. :white_check_mark: 韓国語連音のIPA再マッピング (C3)

  Phase B: 品質改善 — :white_check_mark: 主要項目完了 (Review R2, `0f9429e`)

  - :white_check_mark: 非JA prosody 抽出 (M11: ZH声調/EN,ES,PTストレス/FR語末ストレス)
  - :white_check_mark: 英語 OOV eSpeak フォールバック (M4-A)
  - :white_check_mark: 旧モデル互換性調査 (M7: 問題なしと確認)
  - :hourglass: dominant language の上書き制御 (M3)
  - :hourglass: Unicode NFC 正規化の追加 (M5: ES, FR 未対応)

  Phase C: 将来課題

  - 韓国語 g2pk2 コアルールの C++ 移植 (M2)
  - 英語 OOV ニューラルモデルの C++ 推論 (M4-B)
  - C++ テストスイートの追加
...
### 11.2 M4: EN OOV（辞書外単語）の無音化 — :white_check_mark: eSpeak フォールバック実装済み (`0f9429e`)
...
### 11.3 M7: 旧バイリンガルモデルとの互換性 — :white_check_mark: 調査完了: 互換性問題なし
...
### 11.4 M11: 非JA言語のprosody特徴量欠落 — :white_check_mark: `computeNonJaProsody()` 実装済み (`0f9429e`)
...
| 言語 | a1の意味 | a2の意味 | a3の意味 | C++出力 (修正前→修正後) |
|-----|---------|---------|---------|---------|
| JA | アクセント核位置 (-4等) | モーラ位置 (1等) | 総モーラ数 (5等) | ✅ 正しい値 (元から対応) |
| EN | 0 (固定) | ストレス (0/2) | 単語音素数 | ❌→✅ `computeNonJaProsody()` |
| ZH | **声調 (1-5)** | 音節位置 | 語長 | ❌→✅ `computeNonJaProsody()` |
| ES | 0 (固定) | ストレス (0/2) | 単語音素数 | ❌→✅ `computeNonJaProsody()` |
| FR | 0 (固定) | ストレス (0/2) | 単語音素数 | ❌→✅ `computeNonJaProsody()` |
| PT | 0 (固定) | ストレス (0/2) | 単語音素数 | ❌→✅ `computeNonJaProsody()` |

**修正済み (commit `0f9429e`)**: `piper.cpp` に `computeNonJaProsody()` を追加し、全非JA言語でPython学習パイプラインと一致するprosody値を抽出。
...
### 11.5 総合優先度マトリクス（実施状況）

| 順位 | 課題 | 影響度 | 工数 | 状態 |
|:---:|------|:-----:|:---:|:----:|
| 1 | **M11-ZH**: 声調prosody欠落 | 高 | 1-2h | :white_check_mark: **実装済み** (`0f9429e`) |
| 2 | **M4-A**: EN OOV eSpeak フォールバック | 高 | 1-2h | :white_check_mark: **実装済み** (`0f9429e`) |
| 3 | **M11-EN/ES/FR/PT**: ストレスprosody欠落 | 中 | 3-4h | :white_check_mark: **実装済み** (`0f9429e`) |
| 4 | **M7**: `num_symbols` 検証追加 | 低 | 30min | :white_check_mark: **調査完了** (問題なし) |
| 5 | **M1**: ZH フレーズ辞書拡充 | 低 | 中 | :hourglass: 将来課題 |
| 6 | **M4-B**: EN GRU ニューラルモデル移植 | 低 | 3-4日 | :hourglass: 将来課題 |
