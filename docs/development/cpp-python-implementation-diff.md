...
**M1コミット `c251b1d` により、以下のCritical差分は解消済み:**

| 重要度 | 差分 | 状態 | 備考 |
|--------|------|:---:|------|
| ~~**Critical**~~ | ~~疑問詞マーカー未実装~~ | ✅ 解消 | M1で実装済み |
| ~~**Critical**~~ | ~~Nバリアント未実装~~ | ✅ 解消 | M1で実装済み |
| ~~**Critical**~~ | ~~プロソディマーク未実装~~ | ✅ 解消 | M1で実装済み |
| **Critical** | **OpenJTalkフロントエンド差異** | ❌ 残存 | バイナリ版 vs Cライブラリ版でA1/A2/A3値が異なる（M1.5で対応予定） |
| **Medium** | JSON入力で prosody_features 未サポート | 🔲 未対応 | M3で対応予定 |
| **Medium** | カスタム辞書の日本語単語境界 | 🔲 未対応 | M3で対応予定 |
| ~~**Low**~~ | ~~puaToPhoneme未登録~~ | ✅ 解消 | M1で実装済み |
| **Info** | prosody_features テンソル型の整合性 | — | ONNX Cast ノードで吸収済み |

### 新規発見: OpenJTalkフロントエンド差異

M1実装後のDockerテストで、音素化ロジック自体は正しいにもかかわらず、**OpenJTalkバイナリ（C++）とpyopenjtalk（Python）で異なるA1/A2/A3値が返される**ことが判明:

| テキスト | 差異の種類 | 詳細 |
|---------|-----------|------|
| そうなの？。 | A1値オフセット | C++: a1=-2開始, Python: a1=0開始 → `]`位置ずれ |
| あんないします。 | アクセント句分割 | C++: 2句(4+3モーラ), Python: 1句(7モーラ) → 余分な`[` |

→ **対策**: M1.5でpyopenjtalkのCライブラリを直接リンクし、フロントエンドを統一。詳細は [cpp-python-sync-milestones.md](cpp-python-sync-milestones.md#m15-openjtalkフロントエンド統一-critical) を参照。
