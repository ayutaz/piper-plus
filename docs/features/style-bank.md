...
Phase 3 で実装された以下 3 ツールの使用リファレンスです:
...
> 関連チケット: `docs/research/implementation-plan/tickets/phase-3/` (P3-T01〜P3-T04)
> 上流 Phase 4 (PE-A loss): `docs/research/implementation-plan/tickets/phase-4/`
...
- `global_centroid` は敢えて **正規化しない** まま保存します (fork 側で
  `F.normalize()` が呼ばれる)。これは Phase 4 / 5 の emotion loss 実装と
  一致させるため必須です。
...
Phase 0 P0-T03 の調査時点では **Option A (transformers) は
...
```bash
# CREMA-D (P3-T01 の download_crema_d.py 出力) から style bank を生成
...
### Q1: Phase 4 の PE-A loss で `.npz` が読み込まれない。どこを見ればよい?
...
A: Phase 0 P0-T03 時点では transformers AutoModel 経由の自動ロードは未
...
- 研究計画トップ: `docs/research/peav-style-conditioning.md`
- Phase 3 チケット: `docs/research/implementation-plan/tickets/phase-3/README.md`
- Phase 4 (PE-A loss 学習統合): `docs/research/implementation-plan/tickets/phase-4/`
- ORT セッション統一仕様: `docs/spec/ort-session-contract.toml`
- Style bank 契約 (予定): `docs/spec/style-bank-contract.toml`
