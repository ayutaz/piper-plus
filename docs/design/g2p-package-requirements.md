...
本ドキュメントは全 4 プラットフォームの要求定義を定める。

- Phase 1: Python + Rust (セクション 1-4)
- Phase 2: C# NuGet (セクション 5-6)
- Phase 3: JS/WASM npm (セクション 7-8)
- 共通機能要求 (セクション 9)
- 共通非機能要求 (セクション 10)
...
**タイトル:** `PiperPlus.Core` から `PiperPlus.Phonemize` アセンブリを分離する

**説明:**
現在の `PiperPlus.Core/Phonemize/` ディレクトリを新プロジェクト `PiperPlus.Phonemize` に移動する。`PiperPlus.Core/Mapping/OpenJTalkToPiperMapping.cs` も `PiperPlus.Phonemize` に含める。

**受入条件:**
1. `PiperPlus.Phonemize` が `PiperPlus.Core` なしでビルド・実行可能
2. `PiperPlus.Core` が `PiperPlus.Phonemize` を ProjectReference で参照
3. `PiperPlus.Phonemize` の TFM は `net8.0` (LTS)
4. ONNX 推論 (`Microsoft.ML.OnnxRuntime`) への依存が `PiperPlus.Phonemize` に存在しない
5. 既存の `PiperPlus.Core` ユーザーのコード変更がゼロ (名前空間維持)
...
#### Phase 2: C# NuGet パッケージ

| 項目 | 内容 |
|------|------|
| **スコープ** | `PiperPlus.Phonemize` (NuGet) |
| **優先度** | 中 |
| **推定工数** | 2-3 週 |
| **理由** | Unity / .NET デスクトップ開発者向け。C# エコシステムに MIT G2P ライブラリが不在 |

**タスク:**
1. `PiperPlus.Phonemize` プロジェクト作成
2. `PiperPlus.Core` からの Phonemize + Mapping 移動
3. NuGet パッケージメタデータ整備
4. 共通テストフィクスチャ対応
5. CI ワークフロー構築

**マイルストーン:**
- v0.1.0: NuGet ベータ公開
- v1.0.0: 安定版リリース (Phase 1 の v1.0.0 と同期)

#### Phase 3: JS/WASM リファクタリング
...
| 項目 | 現状 | 変更 |
|------|------|------|
| プロジェクト | `PiperPlus.Core` (Phonemize は namespace) | `PiperPlus.Phonemize` (独立プロジェクト) |
| Mapping | `PiperPlus.Core.Mapping` | `PiperPlus.Phonemize.Mapping` |
| ONNX 依存 | `PiperPlus.Core` が間接的に参照 | `PiperPlus.Phonemize` には ONNX 依存なし |
