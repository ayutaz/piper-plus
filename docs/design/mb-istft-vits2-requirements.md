...
> **作成日**: 2026-04-05
> **最終更新**: 2026-05-03
> **ステータス**: Step 1-6 完了 (master/dev マージ準備中)
...
**ステータス**: 学習完了 (2026-05-02)、ONNX エクスポート完了 (`tsukuyomi-mb-istft-500epoch.onnx`, 37.8 MB FP16)、6 言語推論テスト完了。

| 項目 | 値 |
|------|-----|
| データセット | `/data/piper/dataset-tsukuyomi-finetune-6lang/` (100 発話、再生成済) |
| ベースチェックポイント | `output-multilingual-6lang-mb-istft/checkpoints/epoch=74-step=500034.ckpt` |
| 出力先 | `/data/piper/output-tsukuyomi-mb-istft-finetune/` |
| 最終チェックポイント | `epoch=499-step=22000.ckpt` |
| 出力 ONNX | `tsukuyomi-mb-istft-500epoch.onnx` (37.8 MB FP16) |
| 主要設定 | `--mb-istft`, `--max_epochs 500`, `--batch-size 4`, `--base_lr 2e-5`, `--resume-from-multispeaker-checkpoint` (freeze_dp 自動有効化) |
| 学習時間 | 約 1 時間 (V100 1 枚) |

**6 言語推論結果** (2026-05-02、`output-tsukuyomi-mb-istft-finetune/tsukuyomi-mb-istft-500epoch.onnx`):

| 言語 | テキスト | 音声長 | RTF |
|------|---------|--------|-----|
| ja | こんにちは、つくよみちゃんです。 | 1.85s | 0.04 |
| en | Hello, how are you today? | 0.92s | 0.11 |
| zh | 你好，今天天气很好。 | 1.31s | 0.10 |
| es | Hola, cómo estás hoy? | 1.03s | 0.05 |
| fr | Bonjour, comment allez-vous? | 0.89s | 0.06 |
| pt | Olá, como você está hoje? | 1.22s | 0.08 |

全 6 言語で生成成功。聴感品質は通常 VITS 版と同等以上 (ユーザー確認済み)。

**受け入れ基準** (実装完了):
- [x] 6lang MB-iSTFT ベースから ONNX エクスポートが成功する
- [x] つくよみちゃん 100 発話データセットが再生成可能 (preprocess.py で 8 秒)
- [x] `--resume-from-multispeaker-checkpoint` で freeze_dp が自動有効化される
- [x] 500 epoch FT が完走する (V100 1 枚で約 1 時間)
- [x] 全 6 言語 (ja/en/zh/es/fr/pt) で正常な音声生成
- [x] ONNX エクスポートで `--unify-emb-lang` が自動有効化される
...
#### 6.1.1 RTF 実測結果 (2026-05-02、CPU)

ONNX Runtime CPU 推論、100 phoneme 入力、warmup=5 / runs=50、p50 値で比較:

| Model | Mean (ms) | p50 (ms) | p95 (ms) | RTF (p50) | Size |
|-------|-----------|----------|----------|-----------|------|
| Regular VITS (HiFi-GAN, 75ep) | 168.2 | 167.7 | 212.8 | 0.066 | 38.1 MB |
| **MB-iSTFT-VITS (75ep)** | **76.2** | **73.3** | **100.3** | **0.037** | 38.4 MB |
| **MB-iSTFT つくよみちゃん (FT 500ep)** | 61.9 | 62.3 | 64.9 | 0.046 | 37.8 MB |

**結果**:
- エンドツーエンド speedup: **2.21x** (要件 ≥1.15x を大幅にクリア)
- Decoder 単体 speedup (推定): **~3.6x** (Decoder MACs 75.8% を仮定して逆算、論文値 3.5x と一致)
- 論文 (LJSpeech 英語単一話者) RTF 0.27→0.078=3.5x。本実験 (6 言語 571 話者) で 2.21x 達成 → マルチリンガル/多話者でも同等の高速化を確認

#### 6.1.2 ロールバック判定結果

| 基準 | 閾値 | 実測 | 判定 |
|------|------|------|------|
| RTF speedup | ≥ 1.15x | **2.21x** | ✅ Pass |
| 聴覚アーティファクト | なし | なし (ユーザー確認) | ✅ Pass |
| 6 言語生成 | 全成功 | 全成功 (ja/en/zh/es/fr/pt) | ✅ Pass |
