...
| ResBlock 段数 | 3 段 | **2 段** |
| 最終出力 Conv | Conv1d(32, 1, 7) | **Conv1d(128, 72, 7)** |
...
#### 3.1.5 weight_norm と remove_weight_norm
...
class OnnxISTFT(nn.Module):
    def __init__(self, n_fft, hop_length):
        # DFT行列: np.fft.fft(np.eye(n_fft))
        # inverse_basis: pinv(fourier_basis).T * hann_window
        # 形状: (n_fft, 1, n_fft//2+1) — register_buffer
        self.register_buffer("inverse_basis", inverse_basis)

    def forward(self, magnitude, phase):
        # real = magnitude * cos(phase)
        # imag = magnitude * sin(phase)
        # complex_input = cat([real, imag], dim=1)  # (B, n_fft+2, T)
        # waveform = F.conv_transpose1d(complex_input, inverse_basis, stride=hop_length)
        return waveform
```

**要件**:
- 全演算が Conv1d / ConvTranspose1d で表現されること (opset 15 で十分)
- n_fft=16 の場合、DFT 行列は 9x16 でオーバーヘッド無視可能
...
def training_step_g(self, batch):
    ...
    if self.hparams.mb_istft:
        ...
        # 2. Sub-band STFT損失 (iSTFT直後のサブバンド信号 vs GT サブバンド)
        loss_sub_stft = self.sub_stft_loss(y_hat_mb, y_mb) * self.hparams.c_sub_stft

        # 3. PQMF合成 (フルバンド信号を得る)
        y_hat = self.pqmf.synthesis(y_hat_mb)  # [B, 1, T]

        loss_gen_all = loss_gen_all + loss_sub_stft
    # ... 既存のフルバンド損失 (MPD/MSD, mel, etc.) ...
```

**注意**: `self.pqmf` は `VitsModel.__init__` で `--mb-istft` 有効時にインスタンス化する。
...
現行と同じく `ema_generator_state.shadow_params` を MB-iSTFT Generator のパラメータにコピーする。新しいパラメータ名 (`subband_conv_post` 等) への対応が必要。
...
# MB-iSTFT: デコーダが内部で iSTFT + PQMF 合成を実行
# 出力は同じ [B, 1, T]
...
**受け入れ基準**:
- [ ] PQMF analysis → synthesis 往復で再構成誤差 < -90 dB
- [ ] PQMF の全テンソルが `register_buffer` で登録されている (`.cuda()` ハードコードなし)
- [ ] MBiSTFTGenerator の出力形状が `[B, 1, T]`
- [ ] MBiSTFTGenerator が `g` (speaker embedding) を `self.cond` 経由で正しく受け取る
- [ ] OnnxISTFT が `torch.istft` と同等の出力を生成する (相対誤差 < 1e-5)
...
| PQMF アーティファクト (エイリアシング) | 中 | 再構成誤差テスト (-90 dB)、スペクトログラム目視確認 |
| iSTFT の ONNX 互換性問題 | 低 | DFT 行列方式で回避済み。n_fft=16 は十分小さい |
| FP16 変換での精度低下 | 低 | PQMF の固定係数は FP16 でも十分な精度 |
...
| テストファイル | 検証内容 |
|--------------|---------|
| `test_pqmf.py` | analysis → synthesis 往復再構成 (< -90 dB)、デバイス互換性 (CPU/CUDA)、バッチ処理 |
...
| パッケージ | 用途 | ライセンス |
|-----------|------|----------|
| numpy | DFT 行列計算、PQMF フィルタ設計 | BSD | 既存依存 |
| scipy.signal.kaiser | Kaiser 窓生成 | BSD | 既存依存 (scipy) |
