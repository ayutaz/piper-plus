...
## 11. v10a-r2 最終結果 (2026-08-16 完走、swap OFF / ep27 resume)

R1 (swap-SCL 切除 + ep27 resume) + R3 (5ep 毎評価 + ECAPA abort) で 80ep 完走。
ECAPA abort は一度も発火せず、崩壊は再発しなかった — **swap-SCL cosine が
唯一の不安定要因だったことを確定**。

### 最終スコア (新ハーネス、v9 ep49 baseline と同一条件)

| 指標 | v10a-r2 ep79 | v9 ep49 | Δ |
|---|---|---|---|
| zs_ja CAM++ / ECAPA | 0.603 / 0.516 | 0.610 / 0.516 | -0.007 / ±0 (**同等**) |
| つくよみ CAM++ / ECAPA | **0.753 / 0.634** | 0.725 / 0.601 | **+0.029 / +0.034 (両 encoder 同調)** |
| 4-9kHz 帯域 | **-10.5dB** | -7.6dB | **-2.9dB 改善** |
| same/cross gap | 0.078 | 0.048 | +0.030 (監視事項) |

軌跡: zs_ja は ep59-79 で ~0.60-0.615 にプラトー (追加 epoch 単独では伸びない
形)。つくよみは ep59-79 で 0.75-0.77 を維持。ep69 が dual-encoder バランス最良
(zs 0.603/0.518、つくよみ 0.767/0.653)。

### 事前登録判定

- zs_ja ≥ 0.70 必達: **未達** (0.603) → v10a を v9.1 として公開はしない
- つくよみ ≥ 0.75: 達成 (0.753、ECAPA 同調) / 帯域: 達成 / Goodhart: 終点では
  乖離なし (gap 漸増のみ)

### 結論と v10b へのインプット

1. **土台 (M1-M3 配線修正 + SupCon 改良) は実証された**: OOD 転写が両 encoder
   同調で +0.03、帯域 -2.9dB、in-domain 非退行。zero-shot の製品ユースケース
   (ユーザー持ち込み参照 = OOD) では v9 より明確に良い
2. **in-domain 0.70 への jump には追加要素が必要**: 軌跡プラトーから、学習量
   でも係数でもなく (a) leak 対策の正攻法 = **swap の識別器形式 (R2)** と
   (b) **データドメイン拡充 (v10b C-1)** が残りの主レバー
3. 注意: v8.1 の旧数値 (0.712/0.777) は旧ハーネス測定のため本表とは比較不能。
   新ハーネス系列での序列は v10a-r2 (OOD) > v9
4. 推奨 ckpt: **ep69** (dual-encoder 最良、HF `checkpoints-v10a-r2/epoch=69-step=93520.ckpt`)。
   聴感サンプル: ローカル `piper-v8-dataset-backup/v10a_listen_samples/r2_ep79_tsukuyomi/`
5. コスト実績: v10a 全体 (smoke + r1 + 復旧 + 診断 + r2) ≈ **$150-165**
