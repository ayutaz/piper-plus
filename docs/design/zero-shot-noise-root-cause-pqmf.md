...
## 5.7 v9 本走結果 (2026-08-12 完走)

50 epoch 完走 (発散なし、~40min/epoch、実費 ~$140)。途中 2 件の launch 事故
(language-balanced sampling が ko 356 utts に合わせ epoch が 38 step に縮小 →
ko 完全除外で 6 言語 300,443 utts に / 不良 run ckpt の HF 経由自動レジューム →
即削除) はいずれも既知パターンで早期検知・復旧。

**スペクトル (主目的のがびがび): 明確に改善** — v8.1 比で 4-9kHz の非構造
エネルギー **-5〜-10dB**、倍音コム構造が 8kHz まで維持 (v8.1 は 4kHz 以上が
のっぺりしたノイズの丘だった)。ep35→ep49 でさらに低下。

**数値評価と指標の限界:**

| 指標 | v8.1 ep39 | v9 ep49 | 解釈 |
|---|---|---|---|
| 平均スペクトル 4-9kHz | ノイズの丘 | **-5〜-10dB + 倍音構造** | 主目的達成 (要聴感確認) |
| zs_ja SECS | 0.7117 | **0.6523** | -0.06 後退。学習量 (50 vs 120ep 相当) / データ -15% / 損失リバランスの複合か |
| UTMOS | 2.014 | 1.592 | **判定に使えない** (クリーン FT が 1.58、がびがび v8.1 が 2.01 と逆転する壊れた指標と実証済み) |
| voiced hi-excess | -10.6 | -9.07 | **v8.1 との比較には不適** (スペクトル全体のバランス変化で正規化帯が動く。同一構成の epoch 間比較専用) |

**教訓 (指標について)**: がびがび系の品質は、単一スカラー指標ではなく
「平均スペクトルの目視 (非構造エネルギー + 倍音構造) + 聴感」で判定する。
UTMOS/SECS はそれぞれ別の軸 (総合品質のうち別成分 / 話者類似度) の参考値。

**聴感確認 (2026-08-12 ユーザー実施): がびがび解消を確認** —
「v8.1 よりもがびがびの部分が減ってよくなりました」。**本ドキュメントの調査
対象だった zero-shot がびがび問題は v9 で解決** (PQMF canonical 修正 + MRD +
full-band loss + データゲートの複合が有効だったことの聴感実証)。

**新課題 (同聴感で指摘): 読み上げの強弱 (韻律の抑揚) が平板** — v8.1 も同様で
v9 の後退ではなく元からの制約。構造要因: zero-shot conditioning は CAM++
192 次元 (音色) のみで韻律情報を持たず、multi-speaker 学習で韻律が平均に回帰
する。即時の緩和候補: 推論時 noise_scale 引き上げ (v8 系ではがびがび悪化のため
0.4 に抑えていたが、ノイズ源が直った v9 では引き上げ余地がある可能性) —
掃引サンプル (ns 0.4/0.667/0.9 × nsw 0.5/0.8) を生成し聴感確認中。
本格対応 (pitch/energy conditioning、参照韻律転写) は v10 テーマ。
