...
| モデル | HF リポジトリ | 実測サイズ | 言語 |
|-------|-------------|----------|------|
| つくよみちゃん 6lang | `ayousanz/piper-plus-tsukuyomi-chan` | 38 MB (FP16) | 6 言語すべて |
| CSS10 JA 6lang | `ayousanz/piper-plus-css10-ja-6lang` | 38 MB (FP16) | 6 言語すべて |

> **注**: `pretrained-models.md` はつくよみちゃんを「ONNX size 75 MB」と記載しているが、
> HF 配布ファイル `tsukuyomi-chan-6lang-fp16.onnx` の実測は 38 MB (FP16) だった。
> 本設計は実測値を採る。カタログ側の記述は FP32 時代の値が残っている可能性があり、別途確認する。
...
**ライセンス同意**: つくよみちゃんは利用規約があるため、初回ダウンロード前に
規約へのリンクと同意チェックを表示する。同意なしにはダウンロードを開始しない。
...
| つくよみちゃんの利用規約 | ライセンス違反 | ダウンロード前の同意フロー (§6.5) |
