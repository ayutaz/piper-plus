...
## 9. v10b-lite 本走の最終結果 (2026-08-20、80ep 完走)

instance 48000459 (4x A100 SXM4、$3.18/hr) で from-scratch 80ep 完走 (NCCL
timeout / NaN abort / KL cap 警告いずれも 0)。構成: v9 同一データ + H-1/H-2b/
H-3/S-1a/S-1b/S-5 + cross-utt SupCon + detach-z (S-2 は smoke go/no-go 不合格で
除外 — F0 シフト追従率 0.0、設計 doc §6.5 R1 の失敗機序が発現、v10c で再設計)。

### 最終値 (ep79、同一ハーネスの v10a-r2 ep69 baseline と比較)

| 指標 | v10b ep79 | v10a-r2 ep69 (baseline) | 事前登録 gate | 判定 |
|---|---|---|---|---|
| コム超過 | **1.498dB** | 3.228dB | <1.5dB | **達成 (-1.73dB)** |
| >4kHz autocorr lag128 | **0.003** | — | <0.05 | **達成** |
| 同 lag256 | 0.056 | — | <0.05 | 僅か超え |
| 高域棚 | +8.3dB | — | ≤+2.0dB | 未達 (ep9 の 12.9 から低下傾向) |
| つくよみ CAM++ / ECAPA | 0.691 / 0.593 | 0.727 / 0.674 | 0.740 | **未達 (baseline 比 -0.04/-0.08)** |
| zs_ja CAM++ / ECAPA | 0.598 / 0.508 | 0.615 / ~0.554 | 0.662 | **未達 (-0.02/-0.05)** |
| F0 std | 38.0Hz | (v9: 27-32Hz) | ≥45Hz | 未達 (旧世代よりは改善) |
| FP16 ONNX | 38.8MB | 38.9MB | ≤40MB | 達成 |

軌跡: 話者類似は ep49 以降 0.58-0.60 (zs_ja) / 0.63-0.69 (つくよみ) で飽和。
両 encoder 同調で gap 拡大なし = 測定器 gaming ではない実体の飽和。

### 結論

1. **第 1 目標 (ざらつき根治) は達成**: コム 3.2→1.5dB (GT 帯 0.65-0.76 に接近)、
   フレーム周期性 autocorr ほぼ消失。H-1 resize 化の発生源除去が学習後も維持
   された — §1.1 A1 の診断と §3.1 H-1 の対策が正しかったことの実証
2. **第 2 目標 (話者類似の壁越え) は未達**: S-1 識別器形式 (JCU + 共進化
   classifier) の敵対圧力を足しても in-domain 0.60 / つくよみ 0.69 の壁は
   越えられず、v10a-r2 を下回った (識別器追加は音響品質側に効いた可能性は
   あるが類似度は動かせていない)。**§8 oracle 診断の帰結が確定: 律速は学習
   信号ではなく条件付け経路の容量 — v11 でのアーキ改修 (注入経路の全面
   AdaLN/容量増、当初計画の「置換ではなく補強」の範囲を超える) が次の本丸**
3. S-2 (F0 経路) は decoder が F0 を無視する形で不発 — prior 残差主体の
   再設計を v10c/v11 で (韻律平板さは未解決のまま)

成果物 (HF ayousanz/piper-plus-zero-shot-multi-7lang-v8): checkpoints-v10b/
...
### §10 追補: lowpass 切り分けの結果 — 主犯は A3 (中域調波間ノイズ) と確定 (2026-08-20)

8.5kHz LPF でも「がびがび」が消えない (ユーザー聴感) → 中域を直接測定:

| comb-HNR (調波 bin / 中間 bin パワー比、voiced median) | GT | v10b EMA | 差 |
|---|---|---|---|
| **1-3kHz** | 13.2dB | **4.6dB** | **-8.6dB** |
| 3-5.5kHz | 2.4dB | 0.1dB | -2.3dB (GT 自体ほぼノイズ性) |

**聴感の「がびがび」の主犯 = §1.1 A3 (調波間ノイズ充填) の 1-3kHz 成分**。
声の芯の帯域で調波の間がノイズで埋まっている (ブレシーなざらつき)。高域
ノイズ床 (§10 前段) は第 2 成分。優先順位の更新:

1. **A3 が最優先**: コム (A1) 根治で覆いが取れ、主犯に昇格。H-3 (hires MRD)
   では動かなかった (v10b で有効化済みなのに -8.6dB)。原因候補は iSTFT head の
   位相整合誤差 (§1.3 #4) — head bin 幅 344.5Hz は F0 (~300Hz) の調波微細構造を
   表現できず、frame 間位相整合のみが頼りという構造制約 (S-2 設計 doc §1 の分析
   どおり)。**対策の本命は S-2 の本来の狙い (F0 明示経路) を「無視できない配線」
   で入れ直すこと** — smoke で確定した「decoder が F0 を無視する」失敗を、
   励振をオプション特徴ではなく担体 (carrier) として飲み込ませる形 (source-filter
   化 / prior 残差の強制) で回避する。v11 の条件付け容量改修と同時に設計
2. 高域ノイズ床 (6-11kHz): trainable filter 除去 + 高域重み付き GT 参照 loss
   (§10 処方 1-2、そのまま有効)
3. 聴感はしご: `lpf_ladder_{7000,5500,4000}/` — 4kHz LPF でも がびがび が残る
   はず (A3 は 1-3kHz のため)。残れば A3 主犯説の最終確認になる
