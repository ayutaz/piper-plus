...
- g2p: Hindi / Hinglish G2P (Phase 1). Rule-based `HindiPhonemizer` for Devanagari (akshara parse, schwa deletion, nukta, anusvāra) and Latin Hinglish with word-level hi vs en split. `the`/`to`/`main` are Hindi lexicon hits (थे/तो/मैं). English tech words use `EnglishPhonemizer`. Language code `hi`; Devanagari `U+0900–U+097F` in the Unicode detector.
- g2p: Hindi phoneme inventory (Phase 0). Append-only 17 symbols on the multilingual map (`num_symbols` 185 → 202, `symbol_set_version` 1.1 → 1.2). New PUA U+E065–U+E072 for dentals / breathy stops / `ə̃` / `q_uvular`. Ids 0–184 and shipping 6lang models (173) are unchanged.
