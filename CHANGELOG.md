...
- **T-006 artifact-retention gate** (`scripts/check_artifact_retention.py`): 全 `.github/workflows/*.y*ml` (40 workflow / 63 upload step) の `retention-days:` を抽出し、 `[[categories]]` 許容値 [1, 7, 30, 90] と突合。 初期 baseline に 6 件の違反 (4× 14d / 1× 5d / 1× 365d) があるため `[meta].mode = "warn"` で導入、 fail 化は別 PR でベースライン修正後
