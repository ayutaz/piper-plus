...
- Docker: 全イメージのビルドで `apt-get upgrade` を実行するようにし、 **fix が公開済みの OS パッケージ CVE をそのまま出荷していた**のを修正。 `trivy-container-scan.yml:155,253` は `ignore-unfixed: true` を指定しているため、 code scanning に上がっている Trivy の 341 件は「upstream に修正が存在するのに当たっていない」ものだけである (util-linux 108 / openssl 95 / glibc 26 / gnutls 20 / gnupg 20 ほか)。 一方で対象 5 Dockerfile はいずれも `apt-get update && apt-get install` のみで **`upgrade` を実行しておらず**、 base image が固定時点で含む古いパッケージがそのまま最終イメージに入っていた。 **これがないと何が起きるか**: `piper-plus-python-inference` (157 件) / `piper-plus-webui` (109 件) / `piper-plus-wyoming` (63 件) が既知の修正可能な脆弱性を抱えたまま公開され続ける。 distroless イメージ (`Dockerfile.cpu.distroless` / `Dockerfile.distroless` ×2) は最終段に apt が無いが、 builder の共有ライブラリを `COPY --from=builder /opt/runtime-libs/ /usr/lib/` で最終イメージへ持ち込むため、 builder stage 側に適用した。 `docker/python-inference/Dockerfile` のみ apt retry ループ (`for i in 1 2 3; ... break`) の**後段**に置いている — ループ内に入れると `break` で抜けた際に実行されないため。 なおビルド日時によって取得パッケージが変わるため再現性は下がるトレードオフがある (hadolint DL3005 が warning を出すが、 `.hadolint.yaml` の `override.error` に含まれないため CI は block しない)
...
- ci: 6 ランタイムに逆引きマップが実装されているかを検査する gate (`scripts/check_reverse_map_parity.py`) を pre-commit に追加した。 PUA レンジ境界・`U+` 出力形式・first-wins ガードの存在を各実装ファイルに対して検査する。 値レベルの pin は各ランタイムの単体テスト (Python doctest / Rust `timing::tests` / Go `TestBuildPhonemeIDReverseMap_*` / JS / C# / C++) が担い、 本 gate は「ランタイムが逆引きマップ**なしで**追加・退行される」ことを防ぐ — それが Rust と Go が置かれていた状態である。 導入時に C# の非準拠 2 件を検出した
- Rust / Go: 「実音素名に解決するか placeholder に落とすか」の判定を CLI から共有 helper (`piper_plus::timing::resolve_timing_tokens` / `piperplus.ResolveTimingTokens`) に切り出し、 単体テスト 7 件ずつを追加した。 判定は当初 両 CLI の `main()` にインラインで書かれており、 **どのテストも CI step も 2 つの分岐のどちらも実行していなかった** — 長さ不一致時のフォールバックと警告は一度も動いたことがない状態だった。 検査内容: 整合時に逆引き名を返すこと、 PUA 明示名が効くこと、 不一致時に `ph_N` へ落ちて**逆引き名が 1 つも混入しない**こと、 ID が無いとき / durations が 0 のときも unresolved 扱いになること (空 ID 列は長さ 0 と自明に一致するためガードが必要)、 padding 後の長さで整合が保たれること、 未知 ID は `<id>` のままで `ph_N` にはならないこと (整合は取れており 1 スロットだけ未知という別の失敗なので区別する)。 mutation 5 種 (`==` を `<=` に / 0 件ガード削除 / placeholder 改名 / フォールバックが resolved を主張 / PUA 引数の無視) が Rust・Go それぞれ 5/5 検出されることを実測確認した。 併せて Go の placeholder を `p0` から Rust と同じ `ph_0` に揃え、 `[reverse_map.unaligned_fallback]` として spec に記録した
- ci: 逆引きマップ gate に **CLI wiring section** を追加した (`scripts/check_reverse_map_parity.py`)。 逆引きマップを持っているだけでは足りない — 判定が CLI にインラインで戻れば再びテスト不到達になるため、 共有 helper の存在・0 件ガード・placeholder 綴りの Rust/Go 一致・CLI が helper を呼ぶこと・CLI が逆引きマップを自前で組まないこと・両分岐のテストが名前で存在することを強制する。 pre-commit の trigger パスに CLI とテストのファイルも追加した (含めないと CLI だけを書き換えた変更では gate が走らず、 インライン化の再発を検出できない)。 mutation 5 種 (CLI での逆引き再実装 / helper 呼び出しの削除 / ガード削除 / placeholder 分岐 / テスト削除) が 5/5 検出されることを実測確認した
- Rust / Go: timing の PUA 音素名を実名に解決するようにした。`phoneme_id_map` のキーは `a:` / `cl` / `N_m` / `ky` / `?!` のような多文字トークンが PUA コードポイント 1 文字に畳まれて入っており、逆引きテーブルを渡さないと `[reverse_map.pua_handling]` のフォールバックが効いて `U+E019` のようなコードポイント表記になる。 in-tree の 173 キーモデルで **173 ID 中 79 ID が該当**し、日本語の長音・促音・拗音・N バリアントがほぼ全滅していた (Python canonical / C# / C++ は読める名前を出す)。 `piper_plus::timing::builtin_pua_names()` / `piperplus.BuiltinPUANames()` を追加し、`pua_to_multi_char` が `None` / `nil` のときの**既定値**にした — 「渡し忘れ」が起きない形にするため。素の PUA フォールバックが必要なら空 map を明示的に渡す。実モデルの全 173 ID について canonical と一致することを実測確認済み (差分 0)
- docs: `[reverse_map.collision_resolution]` に `key_order` を追加し、first-wins の「first」が 6 ランタイムで揃っていないことを明記した (Python / JS / C# = 挿入順、 C++ = コードポイント順、 Rust / Go = UTF-8 バイト順)。 出荷済みモデルはすべて ID 衝突を持たない (in-tree fixture は 173 キーで衝突 0 件) ため実挙動は一致するが、contract が「first」を定義していない状態だった。 統一は canonical の挙動変更と fixture 再生成を伴うため #698 に分離した

- C++: #662 で追加した 7 引数 `outputTimingsAsJSON` の直接テストを 4 件追加した (`test_phoneme_timing_text_path.cpp`、model 非依存)。 追加時点では CLI 経由の E2E だけがこの経路を通っており、 **算術そのものは一切 pin されていなかった**。 検査内容: 出力サンプル数から `total_duration_ms` が導かれること (entry の `max(end_ms)` ではない)、 旧 5 引数オーバーロードが `max(end_ms)` フォールバックを保つこと、 `channels` が**除算**方向であること (44100 interleaved / 2ch = 1000 ms、 乗算なら 4000 ms になる) と `channels <= 0` が 1 に clamp されること、 entry が空でも出力ストリーム長を報告すること。 3 種の mutation (channels の除算を乗算に / `emittedSamples` を無視 / 空 timing で 0 を返す) でそれぞれ 6 / 2 / 2 件が fail することを実測確認した
- C++: `emittedSamples` が**インタリーブ済みサンプル数** (フレーム数 × チャンネル数) であることを `piper.hpp` に明記し、 `writeWavHeader` (`wavfile.hpp`) が同じ `audioBuffer.size()` を**フレーム数**として `× channels` する逆向きの規約であることを併記した。 CLI は同じ値を両方に渡すため `channels > 1` では片方が必ず誤るが、 `channels` は既定値 1 から変更する経路が C++ ツリーに存在しないため現時点では潜在的。 暗黙に継承せず記録し、 #693 として起票した

- C++ CLI: `--output-raw` の**非 streaming 経路だけ**出力サンプル数を記録しておらず、 `total_duration_ms` が `max(end_ms)` フォールバックに戻っていたのを修正した (#662)。 同一入力・同一出力先で `--streaming` の有無だけで値が 69% ↔ 100% に変わる状態だった (実測: 1715.0 ms ↔ 2475.5556 ms)。 `textToAudio` は `audioCallback` 呼び出しの**直後に** `audioBuffer.clear()` する (callback の契約が「呼び出し側がコピー済み」であるため) ので、 合成呼び出し後に `audioBuffer.size()` を読んでも 0 になる — streaming 側と同型に **callback 内で copy の前に**加算する必要がある。 検出されなかった理由は E2E gate の `run_cli` が `--output_file` 固定で、 4 出力モードのうち 3 つを 1 ケースも通していなかったこと。 `run_cli` を出力モードで parameterize し、 5 つの sink すべてについて `total_duration_ms` が**その sink が実際に受け取った長さ**と一致することを検査するケースを追加した (WAV は WAV ヘッダから、 raw は stdout のバイト数から逆算するので、 同じ値を 2 か所から読む恒真検査にならない)。 raw 非 streaming のカウントを削除する mutation で当該ケースが fail することを実測確認済み

- C++: `--output-timing` の JSON が返す `total_duration_ms` を**出力ストリーム長**にした (#662)。 `docs/spec/phoneme-timing-contract.toml` の `[concatenation].aggregate_total_duration_ms` は集約 timing の総長を「最終 unit 後のオフセット (unit 間無音を含む)」と規定し、 「per-unit cursor walk の合計ではない」と明記しているが、 C++ の writer は entry 由来の `max(end_ms)` を出力していた。 entry 由来では (1) `extractTimingsFromDurations` が id 0/1/2 を entry から除外しつつ cursor だけ進めるため最終音素の後ろの区間が見えず、 (2) 文末 `sentenceSilenceSeconds` や SSML の `<break>` は audioBuffer に積まれるが entry を生まず、 加えて (3) #653 (durations が ceil 前) の分だけ entry 自体が実音声より短い。 **実測**: `"Hola, esta es una prueba de sincronizacion."` で修正前 `1715.0 ms` に対し実ストリームは `2475.5556 ms` (**69%**)、 修正後は `2475.5556 ms` で**サンプル単位で完全一致**する。 修正は `piper::outputTimingsAsJSON` に出力サンプル数を受け取る**追加オーバーロード**を設け、 CLI が 5 つの sink (`--output_file` / `--output_dir` / `-f -` / `--output-raw` / `--output-raw --streaming`) すべてで記録した値を渡す形にした。 既存の 5 引数シンボルは `max(end_ms)` フォールバックのまま維持しており (`nm -gU` で両シンボルの共存を確認済み)、 mangled name が変わらないため `ABI diff (libpiper_plus.so head vs base)` gate に影響しない — `SynthesisResult` へのフィールド追加は struct のレイアウトが変わり、 同 gate が `abi-dumper` の DWARF 情報で検出するため採らなかった
- C++ CLI: 3 つの WAV 出力モード (`--output_dir` / `--output_file` / stdout) の合成経路を**バッファ経由に統一**した。 従来は `phonemeIdsToWavFile` / `phonemesToWavFile` / `textToWavFile` が直接 ostream へ書くため、 **呼び出し元が出力サンプル数を知る手段が無かった** (SSML 分岐のみバッファ経由だった)。 各モードで同じ 4 分岐 + `writeWavFromBuffer` という 1 つの形に揃え、 `emittedSamples` を記録する。 `textToWavFile` の実体は `textToAudio` + WAV 書き出しそのものなので挙動は同一で、 音声出力が 3 形態 (単一文 / `--phoneme_silence` による複数 phrase / padding 発動の短文) でサンプル数一致することを実測確認した。 `--output-raw` (streaming) は chunk callback で加算する。 併せて phoneme ids からバッファへ合成する `piper::phonemeIdsToAudio` を追加した (既存 `phonemeIdsToWavFile` の WAV 書き出しを除いたもの、 additive export)
- ci: CLI timing E2E gate に `total_duration_ms` が実 WAV 長と一致することの検査を追加した。 修正前は 2 ケース (`single_sentence_json` / `multi_unit_offsets`) が RED になることを実測確認済み。 併せて従来の「`total_duration_ms` == `max(end_ms)`」という等値検査を、 spec の定義変更に合わせて「`max(end_ms)` が `total_duration_ms` を超えない」という片側検査に変更した — 集約 total は最終 entry の終端より**長い**のが正しい (末尾の pad/eos フレーム・末尾無音・#653 の分だけ)

- C++: Strategy A / EOS / silence の post-trim ヘルパー 6 個と `padPhonemeIds` を `src/cpp/trim_helpers.hpp` に切り出し、 `test_short_text_mitigation.cpp` の**手書きレプリカを削除して production の実体を検査**するようにした。 これらは `piper.cpp` 内の `static` 関数だったためどのテストからも到達できず、 同テストが持つ**絶対サンプル数の assert 16 件超はレプリカを叩いていた** (リンクは自身の `.o` + gtest のみで `piper.cpp` にも `libpiper_common.a` にも繋がっていない)。 **実測された帰結**: production に対する trim 境界の変異 5 種 (int16 前方 erase off-by-one / 後方 erase off-by-one / float 前方 erase off-by-one / **2 段 erase の順序反転** / EOS tail off-by-one) が**リポジトリ内の全スイートを素通り**した。 とくに順序反転は #655 の対応で trim が 2 段 erase になったことで初めて生じた failure mode (dev の単一スライスには存在しない) であり、 それを持ち込んだ変更自体に検出器が無い状態だった。 切り出し後は同じ変異が 6〜12 ケースで fail する。 併せて float 版の絶対長テスト (padding trim 6 / EOS trim 4) と `baseOffset != 0` のユニットレベル検査 6 件を追加し、 テストは 56 → **71 ケース**になった。 ヘッダは `phoneme_ids.hpp` (ORT 非依存) のみに依存する inline 実装なので、 テストは引き続き model-free / onnxruntime-free のまま
- C++: 上記により `trimSilenceInt16` / `trimSilenceFloat` に**初めてテストが通った**。 両関数は `haveDurations == false` のフォールバックだが in-tree の ONNX 4 個すべてが `durations` を宣言するため到達不能で、 stderr プローブを挿したビルドで 10 スイートを走らせてもヒット 0 件だった (#690)。 `baseOffset` を無視する変異が新テストで検出されることを確認済み。 音声出力は不変 (ヘッダ切り出し前後でサンプル数一致、 短文 / 複数 phrase / 単一文の 3 形態で確認)

- C++ tests: fixture 欠損時の anti-vacuity ゲート 4 件が**一度も fail できない状態**だったのを修正した。 `FixtureModelIsPresent` / `FixtureModelDeclaresDurationsOutput` はいずれも fixture 内の `TEST_F` として書かれていたが、 fixture の `SetUp()` が `GTEST_SKIP()` を呼ぶため gtest はテスト本体より先に skip を適用する — つまり `EXPECT_TRUE(g_model_found)` に到達できない。 **実測**: モデルが解決しないディレクトリからバイナリを実行すると全ケースが `[ SKIPPED ]`、 `[ PASSED ] 0 tests`、 **exit code 0** になり ctest は PASS と報告する。 vacuous green を実際に止めていたのは `_build-test-cpp.yml` の独立した "Verify test model is present" ステップだけで、 ローカル `ctest` や他経路では無言で緑になっていた。 該当 4 ファイル (`test_synthesize_append_scope.cpp` / `test_ssml_synth.cpp` / `test_c_api_integration.cpp` / `test_phoneme_timing_text_path.cpp`) のゲートを fixture 外の素の `TEST()` に移し、 自前でモデル探索するようにした。 修正後は 4 スイートすべてがモデル欠損時に **exit 1** になることを実測確認した。 `test_phoneme_timing_text_path.cpp` のゲートは `voice.session.hasDurationOutput` の検査 (fixture 必須) を含んでいたため、 静的な宣言検査を `TimingTextPathGate` へ、 実行時検査を `LoadedSessionExposesDurationOutput` として fixture 内に残す形に分割した
- C++ tests: `test_synthesize_append_scope.cpp` の sentinel を定数から **index 依存パターン** (`(i * 37) % 30000 - 15000`) に変更し、 padding 発動の前提条件を assert するようにした。 定数 prefix では「境界を越えてサンプルが移動していないこと」しか示せず、 prefix 内の in-place 書き換えや並べ替えは全サンプルが sentinel 値のまま通過する。 また padded ケースが `durations[0] * hopSize` の truncate 等で padding を踏まなくなった場合に無言で vacuous になるため、 `interspersePad` が有効であることと phoneme id 数が Strategy A の窓 (`MIN_BODY_FOR_STRATEGY_A <= len - 2` かつ `len < MIN_PHONEME_IDS`) に収まることを事前に assert する

- C++: `trimSilenceFloat` の debug log が `"Trimming silence (float):"` から int16 版と同一の `"Trimming silence:"` に変わっていたのを戻した。 #655 の対応で同関数を書き換えた際の退行で、 **唯一テストから到達できない経路**でログからどちらが走ったか判別できなくなっていた
- C++: `test_synthesize_append_scope.cpp` のコメントの過大表現 2 件を訂正した。 (1) `RealTimeFactorMatchesReportedAudioSeconds` は「#654 の後半」と書いていたが **#654 の検出器ではない** — `realTimeFactor` は誤った `audioSeconds` と同じ場所で `inferSeconds / audioSeconds` として計算されるため、 バグ版でも等式が成立する (完全 revert したビルドで green を実測)。 派生フィールドの整合性 pin である旨に書き換えた。 (2) ヘッダの「サンプル数を assert するので決定論的」という説明が、 `GrowthIsIndependentOfPriorBufferContents` だけが**別々の 2 回の推論の長さを `EXPECT_EQ` で比較**しており duration predictor の run 間決定性に依存することを述べていなかったため明記した (本 fixture の `RandomNormalLike` 4 個は decoder の z noise 用で DP は決定的、 6 連続実行で安定を確認)。 併せて各ケースの検出力 (revert ビルドに対し int16 2 件 / float 2 件が fail、 残り 3 件は pin) をヘッダに記録した

- C++: `trimEosRegion` / `trimEosRegionFloat` に `trimSamples < 0` のガードを追加した。 `eosExcess * hopSize` の int overflow で負値になると `bufferSize - static_cast<std::size_t>(trimSamples)` が巻き戻ってギガバイト級の `resize` になる。 修正前も `resize(totalSamples - trimSamples)` で同じ露出があり **regression ではない**。 `durations.back() > 8.4M` フレーム (hop=256) が必要で正常なモデル出力からは到達不能だが、 当該関数を触るついでに潜在的な罠を閉じた
- C++: `test_synthesize_append_scope.cpp` に float 経路 (`phonemesToAudioFloat` → `synthesizeFloat`) の 2 ケースを追加した (計 8 ケース)。 `trimPaddingByDurationsFloat` / `trimSilenceFloat` / `trimEosRegionFloat` も同じ `baseOffset` 対応をしているが、 int16 経路のテストだけでは float 側のみの回帰を検出できない (C API イテレータ `synth_next` は float 経路を使う)。 **当初の float `audioSeconds` テストは mutation で無効であることが判明したため修正した**: `phonemesToAudioFloat` は文末無音を付加し、 既定の 0.2 秒 (4410 サンプル) が sentinel プレフィックス (2048 サンプル) より大きいため、 バグ版の `audioSeconds` が水増しされた増分の範囲に収まって上限比較を通過していた。 `sentenceSilenceSeconds = 0` にして厳密一致を検査する形に変え、 float 経路のみを dev に戻す mutation で 2 ケースとも fail することを実測確認した
- C++: `test_short_text_mitigation.cpp` の trim ヘルパーが **修正前 (pre-#655) 実装のコピー**であることをファイル冒頭に明記した。 production は `baseOffset` を取り `[baseOffset, size)` のみを対象にするようになったが、 同ファイルのレプリカは旧絶対スライスのまま。 単一 unit 挙動 (`baseOffset == 0`) では両者が厳密同値であることを総当たり検証済みなので既存 40 件超のアサーションは有効だが、 **production へコピーバックしてはならない**旨と、 `baseOffset != 0` 経路は `test_synthesize_append_scope.cpp` のみがカバーする旨、 および `scripts/check_short_text_contract.py` は Python 側の数値定数しか照合しないため**この乖離を検出する gate が存在しない**旨を記録した

- C++: `synthesize()` / `synthesizeFloat()` の post-trim が累積バッファの先頭から切っていたため、 2 フレーズ目以降でパディングが発生すると**発話冒頭 (前フレーズの音声) が削られていた**のを修正した (#655)。 両関数は append API で、 呼び出し元は複数 unit (文 / `phoneme_silence` による phrase 分割 / インライン音素セグメント) を 1 つのバッファに積む。 一方 `trimPaddingByDurations` は `audioBuffer.begin() + frontSamples` から絶対スライスしていたので、 2 フレーズ目以降の `begin()` は**前フレーズの音声の先頭**を指す。 `frontSum` は必ず `durations[0]` (BOS) を含むため `frontPad == 0` でも発生し、 パディングが起きた全フレーズが該当する。 dev での実測では 3840 サンプルしか寄与していないフレーズがバッファ先頭から 642 サンプル (29 ms) を除去していた。 音声が静かに壊れるだけでエラーにならない。 修正は 6 つの trim ヘルパー (`trimPaddingByDurations` / `trimSilenceInt16` / `trimEosRegion` とそれぞれの Float 版) に `baseOffset` を渡し、 `[baseOffset, size)` のみを対象にした。 絶対スライスからの再構築ではなく `erase` で前半を保持する形にしている。 `trimEosRegion` は `resize` で末尾しか削らないため呼び出し元の音声を書き換えることはなかったが、 「trim 量 >= バッファ全体」のガードがバッファ全長を見ていたため、 unit より大きい trim が skip されずに前 unit を食う可能性があった — こちらも unit 長基準に修正した。 durations 非対応モデル用フォールバックの `trimSilence*` は RMS 走査自体が累積バッファ全体を見ていたため、 走査範囲も含めてスコープ限定した
- C++: `synthesize()` / `synthesizeFloat()` の `result.audioSeconds` が累積バッファ全体から再計算され、 複数フレーズで過大計上されていたのを修正した (#654)。 trim 分岐が `audioBuffer.size() / sampleRate` を再代入する一方、 呼び出し元 (`textToAudio` / `textToAudioFloat` / `phonemesToAudioFloat`) は `result.audioSeconds += phraseResults[i].audioSeconds` と積算するため、 フレーズ N の値が「フレーズ 1..N の合計長」になり、 合計はフレーズ数に対して二次的に膨らむ。 `haveDurations` が true のモデル (現行の全モデル) では必ずこの分岐を通る。 dev での実測では実長 1.5341 秒の WAV に対し `Real-time factor: 0.0125 (infer=0.0501 sec, audio=4.0081 sec)` と **2.6 倍**を報告し、 RTF も同率で過小になっていた。 `audioBuffer.size() - audioBase` から算出するよう変更した。 `realTimeFactor` は `audioSeconds` から導出されるため自動的に整合する
- C++: 上記 2 件の回帰テスト `src/cpp/tests/test_synthesize_append_scope.cpp` (6 ケース) を追加した。 `phonemesToAudio` は public かつ append なので、 バッファに sentinel (値 12345 × 2048 サンプル) を詰めてから呼ぶことで「呼び出し元の既存サンプルが残っているか」(#655) と「`audioSeconds` がこの呼び出しの出力を表しているか」(#654) を**決定論的に**観測できる — 本モデルは `noise_scale: 0.4` で seed も無く同一バイナリでも波形が実行毎に変わるため、 検査はサンプル数と既知プレフィックスの同一性のみに限定している。 修正前に 2 ケースが RED になることを実測確認した (#654 は `audioSeconds` が 1.7995 秒に対し実際の append が 1.7067 秒 = 差分がちょうど sentinel 長 0.0929 秒、 #655 は sentinel の 1513 サンプル目以降が破壊)。 なお `GrowthIsIndependentOfPriorBufferContents` は **#655 を検出しない** no-regression pin である (buggy な trim も `front + back` サンプルを削るため増分は同じで、 sentinel の内容だけがバグを露出する) — 修正前のビルドで green になることを確認済みで、 コメントにもその旨を明記した

- Python / C++: SRT の ms 丸めを `floor(ms + 0.5)` 系のイディオムから真の round に変更した (#681)。 `math.floor(ms + 0.5)` / `static_cast<long long>(ms + 0.5)` は round の等価物に見えるが**そうではない** — 0.5 の加算が binary64 で丸め上がるため、 `ms = 0.49999999999999994` (0.5 未満で最大の double) では和がちょうど `1.0` になり、 イディオムは 1 を返す一方で Rust `f64::round` / Go `math.Round` / JS `Math.round` はいずれも 0 を返す。 ECMA-262 が `Math.round` の注記で挙げている反例そのものである。 実測で 6 実装すべてを走らせて確認した。 Python は小数部の比較 (`ms - whole >= 0.5`) に、 C++ は `std::llround` に変更し、 非負入力で 6 ランタイムが厳密一致するようにした。 C++ 側は `PhonemeInfo` が秒を float で保持するため当該入力に到達できないが、 contract が 6 ランタイムで pin するイディオムである以上「偶然踏まれないもの」ではなく正しいものを置くべきと判断した。 contract の `rounding_cases` に本ケースを追加し、 float64 の formatter を持つ 4 ランタイム (Python / Rust / Go / JS) のテストに入れた — C# は `FormatSrtTimestamp(float)` で、 C++ は秒が float なので当該値が `0.5` に潰れるため `requires_float64` として免除し、 spec と gate の双方に記録した
- ci: SRT 丸め gate の検出力を強化した (#681)。 レビューで 3 種のすり抜けが実証されたため対応した: (1) **イディオムをコメントに残したままコードだけ変更**すると substring 検索を通過する → コメント (および Python の docstring) を除去してから検索する。 docstring の除去が必要なのは、 formatter 自身が canonical / forbidden 双方のイディオムを説明のために文字列として含むため。 (2) **`round(ms, 0)` は `round(ms)` の部分文字列でない**ため forbidden 検査をすり抜ける (しかも ToEven = #681 そのもの) → forbidden を正規表現化し、 `floor(ms + 0.5)` 系・`nearbyint`・`RoundToEven` 等も追加した。 (3) **各ランタイムのテスト表を「整理」して非判別値にする**と、 テストも gate も通ったまま 5 ランタイムが黙って判別力を失う (contract 駆動なのは Python のみで他 5 つはハードコピー) → `[output_formats.srt.rounding_impl]` に `test_file` を宣言させ、 判別ケースの**入力リテラル**がテストに存在することを検査する。 期待タイムスタンプではなく入力で照合するのは、 `0.49999999999999994` と `0.0` がどちらも `00:00:00,000` を返すため期待値だけでは空振りするからである。 3 種すべてで gate が fail することを実測確認した
- ci: `pre-commit.yml` の paths に `.rs` / `.go` / `.cs` / `.js` / `.cpp` / `.hpp` / `.h` / `.kt` / `.swift` を追加した。 同 workflow のコメントは「any source change で全 hook を走らせる」と述べているが、 実際の paths は `.py` / `.pyi` / `.json` / `.toml` / `scripts/**` のみで、 **`src/go/piperplus/timing.go` や `src/cpp/piper.cpp` だけを触る PR では workflow 自体が起動しなかった**。 その結果 `cargo fmt --check` / `gofmt -l` / `go vet` / `ktlint` / `swiftformat --lint` と本 PR の SRT gate は、 対象言語のファイルだけが変わった PR で一度も実行されていなかった

- C++: `outputTimingsAsSRT` の cue index が呼び出し元の locale を継承し、 1000 以降が `1,000` と書き出されていたのを修正した (#684)。 index は `output << (i + 1)` で書かれる一方、 CLI は `main.cpp:311-312` で global に `en_US.UTF-8` を設定し `--output-timing` の `ofstream` はその後に構築されるため grouping numpunct を継承する。 同ファイルの `outputTimingsAsTSV` は**まさにこの理由で** `std::locale::classic()` を imbue しているが、 SRT 側は漏れていた。 timestamp は `snprintf` で組むため locale 非依存で、 影響は index のみ。 1000 音素を超える発話の SRT が index 行でパース不能になる。 `--timing-format srt` は #657 の対応で初めて CLI から到達可能になったため、 それ以前はライブラリ API を直接呼ぶ利用者にのみ到達していた。 1000 エントリを渡して grouping numpunct 下で index が裸の整数であることを検査する model 非依存テストを追加した (修正前は `1,000` で fail することを確認済み)
- 全ランタイム: SRT タイムスタンプの負値を丸め前に 0 へ clamp する規則を contract に明記し、 唯一 clamp していなかった JS を修正した (#681)。 JS の `formatSrtTimestamp` は `Math.round(ms)` をそのまま分解するため、 `-1500 ms` が **`-1:-1:-2,-500`** という符号が全フィールドに漏れた文字列になっていた (実測)。 他 5 ランタイムは clamp 済みか実質 0 になる (Go / C# / C++ は明示 clamp、 Rust は `as u64` の saturating cast、 Python は本 PR で追加)。 `[calculation.negative_handling]` は cursor walk 側の規定で SRT formatter には及んでいなかったため、 `[output_formats.srt].negative_input = "clamp_to_zero_before_rounding"` と `negative_cases` を追加して規則化し、 gate でも必須化した。 併せて `rounding` の説明が「Rust / Go / JS / C++ はいずれも half-away-from-zero」と書いていた点を訂正した — 正確には Rust / Go / C# が half-away-from-zero、 Python / JS / C++ が half-up であり、 **clamp によって定義域が `ms >= 0` に限定されるため両者が一致する**。 この不正確な記述が、 唯一 clamp を欠いていた JS をちょうど覆っていた

- C++ CLI: `--timing-format srt` を受理するようにした (#657)。 `docs/spec/phoneme-timing-contract.toml` の `[implementations.cpp].formats` は 6 ランタイム全てと同じく `srt` を含むと宣言し、 README 5 言語版も「C++ CLI — phoneme timing output (JSON/TSV/SRT)」と記載し、 実装 `piper::outputTimingsAsSRT` (`src/cpp/piper.cpp`) とその単体テストも存在していたが、 **CLI からの露出だけが欠けていた** — `main.cpp` の定数は `FORMAT_JSON` / `FORMAT_TSV` のみで、 `--timing-format srt` は `Invalid timing format: srt (must be json or tsv)` で exit 1 していた。 定数・引数バリデーション・書き出し分岐・usage 文言の 4 箇所を追加した。 併せて `(json/tsv)` と書かれていた README 5 言語版 (DE / ES / PT / KO / ZH) の表記を `(json/tsv/srt)` に更新した (EN / JA / FR の表には該当行が無い)
- 全ランタイム: SRT タイムスタンプの ms 丸めを half-away-from-zero に統一した (#681)。 `[output_formats.srt]` は書式 (`HH:MM:SS,mmm`) を規定していたが**丸め規則を規定していなかった**ため、 言語既定がそのまま出ていた: Python の `round()` と .NET の `Math.Round(double)` は round-half-to-**even** (banker's)、 Rust `f64::round` / Go `math.Round` / JS `Math.round` / C++ の `(long long)(ms + 0.5)` は half-away-from-zero。 結果として **Python (canonical) と C# だけが `.5` 境界で 1 ms 早い値**を出力していた (`ms = 1234.5` → 他 4 ランタイムが `00:00:01,235`、 Python / C# が `00:00:01,234`)。 逸脱している 2 実装を含め 3 つのソースが「Rust 実装に合わせる」とコメントで明記しており (Python `timing.py:193` / JS `timing.js:196` / C++ `piper.cpp`)、 両言語の既定が ToEven であることを踏まえずに `round` を呼んでいたことが原因。 Python の `_format_srt_timestamp` の doctest が `>>> _format_srt_timestamp(1234.5)` → `'00:00:01,234'` を期待値として固定しており、**誤った挙動を doctest が保証している**状態でもあった。 **検出されなかった理由**: srt の parity fixture が存在せず (`tests/fixtures/phoneme_timing/golden_matrix.json` は ms の数値のみを持ち書式化出力を持たない)、 かつ 6 ランタイムいずれの既存 srt テストも小数部が `.5` にならない値しか使っていなかった (Go の `TestFormatSRTTimestamp_VariousMagnitudes` は整数 ms のみ)。 修正は Python を `math.floor(ms + 0.5)`、 C# を `MidpointRounding.AwayFromZero` に変更し、 Python 側は負値の clamp も Go / C# / C++ に合わせた。 spec に `rounding = "half_away_from_zero"` と `.5` 境界を含む `rounding_cases` を追加し、 6 ランタイムに同じ境界ケースのテストを入れた (Python は spec を読んで駆動するため fixture が二重化しない)。 境界ケースはいずれも**整数部が偶数**である — `1.5` のように整数部が奇数だと両規則で同じ値に丸まり検査が空振りするため。 C++ のみ `PhonemeInfo` が秒 (float) 保持で ms が `1234.5` に厳密に乗らないので、 float で厳密表現でき整数部が偶数になる値 (`0.0625s = 62.5ms` / `0.3125s = 312.5ms` / `0.5625s = 562.5ms`) を使い、 fixture 自体が `.5` 境界に乗っていることを `ASSERT_DOUBLE_EQ` で先に検査している
- ci: SRT 丸めが言語既定へ逆行しないことを検査する gate (`scripts/check_srt_rounding_parity.py`) を pre-commit に追加した。 spec の `[output_formats.srt.rounding_impl]` から 6 ランタイムの formatter ファイルと canonical idiom を読み、 (1) idiom が残っていること、 (2) 言語既定 (`round(ms)` / `Math.Round(ms)`) を含まないこと、 (3) spec の `rounding_cases` に判別力のあるケース (`.5` 境界かつ整数部が偶数) が 1 件以上あることを検査する。 idiom 検査にしたのは、 6 つの formatter のうち 5 つが module private で、 C++ は秒 (float) を受けるため値レベルの一括検査ができないため — 値の pin は各ランタイムのテストが spec の `rounding_cases` を用いて行う。 Python を `round(ms)` に戻す mutation で gate が 2 項目とも fail することを確認済み
- ci: C++ CLI timing E2E gate (`scripts/test_cpp_cli_timing.py`) に `srt_format` ケースを追加した (計 6 ケース)。 cue の 1-based 連番・`HH:MM:SS,mmm --> HH:MM:SS,mmm` の書式・単調増加・プレースホルダ音素名でないことを検査する。 `--timing-format srt` が exit 1 する状態で RED になることを確認済み

- C++ CLI: `--output-timing` の TSV 出力が数値列に桁区切りカンマを書き込んでいた問題を修正した (`1,011.541`)。 `main.cpp:309` が `std::locale::global(std::locale("en_US.UTF-8"))` を設定するため、 以降に構築される `ofstream` (= `--output-timing` の出力先) がその `numpunct` を継承し、 1 秒を超えるエントリが 3 桁ごとに区切られていた。 TSV の数値列としてパースできず (`float("1,011.541")` は `ValueError`)、 `docs/spec/phoneme-timing-contract.toml` の `[output_formats.tsv].float_precision = 3` が pin する書式にも反していた。 **これが見つからなかった理由**: (1) `en_US.UTF-8` が存在しない環境では `main.cpp:316-318` の catch 節が classic locale に戻すため再現しない、 (2) JSON 側は nlohmann が locale 非依存に serialize するため無傷、 (3) 既存の writer テスト (`test_phoneme_timing.cpp`) の fixture が 1 秒未満のため桁区切り境界を超えない。 修正は `outputTimingsAsTSV` で `std::locale::classic()` を imbue して呼び出し元の locale に依存しないようにし、 既存の flags / precision と同様に復元する (ライブラリが呼び出し元の global locale に出力書式を左右されるべきではない)。 `test_phoneme_timing.cpp` の in-test mirror も同じ形に同期した。 回帰テストは production の `piper::outputTimingsAsTSV` を自作 `numpunct` (`do_thousands_sep() = ','`) を imbue したストリームに対して実行する model 非依存テストとして追加した (named locale を使うと runner の locale 導入状況に依存するため)
- C++ CLI: `--output-timing` を指定したのに timing が得られなかった場合、 ファイルも警告も出さず exit 0 していたのを修正し、 理由を述べる warning を出すようにした。 `main.cpp:994` の `if (runConfig.outputTimingPath && result.hasTimingInfo)` は false 側に何も持たないため、 観測できる挙動が **#652 のバグと byte-for-byte 同一** (ファイルなし・メッセージなし・rc=0) だった。 利用者は「ビルドが壊れている」のか「モデルに `durations` 出力が無い」のかを区別できず、 実際に #652 の報告でも `--output-timing` が黙って何もしないことが症状として挙がっていた。 `durations` を持たないモデルは正当な入力なので error ではなく warn とし、 exit code は変えていない (`--output-timing` を無条件に渡す呼び出し側を壊さないため)
- ci: C++ CLI の `--output-timing` を実際に実行する E2E gate (`scripts/test_cpp_cli_timing.py`) を追加し、 `_build-test-cpp.yml` の integration matrix (ubuntu / macOS × Release / Debug) で常時実行するようにした。 **#652 は CLI に対する報告だったが、 CI は `--output-timing` を一度も実行していなかった** — `.github/workflows/` 全体で `output-timing` に一致するのは `cli-help-docs-sync.yml` の `--help` allowlist 1 件のみで、 PR #658 で追加した gtest は `textToAudio` / C API を直接叩くため `main.cpp` の flag 解析・`hasTimingInfo` gate・JSON/TSV writer はどのテストからも到達していなかった (`main.cpp` にテストから到達できる入口が存在しない)。 5 ケース構成: 報告者のコマンドそのもの (JSON 生成 + `Wrote phoneme timing to` ログ) / multi-unit オフセット (`--phoneme-silence` で 1 行を複数 phrase に分割し、 `start_ms == 0` が厳密に 1 個であることを検査) / TSV (ヘッダ完全一致 + 全数値列の `std::stod` 相当パース) / timing 不在時の診断 / flag 未指定時に副作用が無いこと。 **検出力を mutation で実証済み**: `appendUnitTimings` を早期 return にすると (= #658 前の状態) 3 ケースが fail、 `ConcatCursor::seconds()/frames()` を 0 にすると multi-unit ケースが fail、 TSV の imbue を外すと TSV ケースが fail する。 各ケースは空振りしないための検査を内蔵する — multi-unit ケースは `Extracted timing for` の出現回数が 2 以上であることを assert し (平文の複数文入力は C++ text path では 1 推論に収まるため、 これが無いと single-sentence ケースの重複に劣化する。 実測で確認済み)、 TSV ケースは最大 `start_ms` が 1000 ms を超えることを assert し (桁区切りは 1000 未満では現れない)、 fixture model が `durations` 出力を宣言していることを byte scan で確認する。 CI では `--allow-skip` を渡さないため、 バイナリやモデルが欠けた場合は skip ではなく fail する
- ci: `_build-test-cpp.yml` の C API シンボル検査 (`REQUIRED_SYMBOLS`) に `piper_plus_get_phoneme_timing` を追加した。 #652 の報告者が依存している export そのものが検査対象に入っておらず、 rename や export 漏れが起きても CI は緑のままだった

- npm: `piper-plus` の `exports` にあった `./wasm/ja` / `./wasm/ja-lite` を削除した。 この 2 つは #301 (2026-04-04) で追加されて以来 **実体 0 ファイルのまま 0.3.1〜0.6.0 を通じて出荷され続けていた**。 `files` が `dist/rust-wasm/**` しか含まず path segment 境界で兄弟ディレクトリを拾わないこと、 および `npm-publish.yml` が `piper-plus-wasm` artifact しか download しないことの 2 点が原因で、 build matrix は 3 variant すべてをビルドしていたのに publish job が 2 つを取りに行かなかった。 再発防止として 3 枚のゲートを追加した — `test-npm-package.js` の exports↔files 整合 (宣言同士)、 `scripts/check_wasm_variant_shipping.py` の exports→build matrix→publish download 突合 (CI 出荷チェーン)、 `scripts/check_npm_exports_shipped.py` の `npm pack --dry-run --json` 突合 (実 tarball)。 3 枚目が必要なのは、 **wasm-pack が出力する `pkg/.gitignore` (中身 `*`) を `dist/` へ移すと `files` エントリが無警告で完全に無効化される**ため (実測 2→0 ファイル)。 現在 27 ファイルで出荷できていたのは `actions/upload-artifact` が hidden file を既定で除外するという文書化されていない偶然に依存していたので、 `mv` の前に `.gitignore` を削除して根治した
- npm: `@piper-plus/g2p` が `"license": "MIT"` を宣言しながら **公開 tarball に許諾文を 1 バイトも含んでいなかった**のを修正し、 `LICENSE.md` を同梱した。 MIT は全複製への著作権表示と permission notice の添付を要求するため、 識別子の宣言だけでは条件を満たさない。 symlink は `npm pack` が無言で捨てるため regular file としてコピーしている。 併せて `files` から `dist/openjtalk.wasm` / `dist/openjtalk.js` を削除した — #301 で Emscripten OpenJTalk 経路ごと廃止された後も宣言に残っており、 npm は実在しない `files` エントリを無警告で捨てるため tarball には `dist/` が 1 ファイルも無かった
- Rust: `piper-plus-wasm` の `setJapaneseDictionary` の cfg gate を `feature = "ja-external"` 単独から `all(not(feature = "ja"), feature = "ja-external")` に変更した。 Cargo の feature は加算的なので下流が `--features "multilingual,ja-external"` を選べる。 `create_phonemizer` は同梱辞書を優先するが setter は生えたままで、 呼べば `replace_phonemizer` で日本語音素化ごと差し替えられ、 自己完結のはずのビルドの出力が静かに変わる。 テストモジュール側の 4 gate も同時に揃えた (片方だけだと `--all-features --all-targets` が E0599 になる)
- wasm: 実行不能だった Emscripten ビルドスクリプト 3 本 (`build-production.sh` / `build-unified.sh` / `build-espeak.sh`) と、 それらからのみ参照されていた `phonemizer_wrapper.cpp` / `simple_wrapper.cpp` を削除した。 `build-production.sh` は #301 で削除された `openjtalk_safe.c` と不在の `libopenjtalk.a` を入力に要求するため実行即失敗し、 `build:debug` が指す `build-safe.sh` はファイル自体が存在せず `npm run build` も連鎖して壊れていた。 併せて `clean` を `rm -rf dist/rust-wasm*` に絞った (従来の `rm -rf dist build/build-*` は git 追跡下の build スクリプト 3 本と `dist/` 配下の 7 ファイルを削除する)
- CI: `scripts/check_dictionary_versions.py` が **4 target すべてを見失ったまま「OK」を出していた**のを修正。 3 つは `src/python_run/setup.py` を grep していたが、 同ファイルは Issue #418 以降 `requirements.txt` を `install_requires` に読み込む形になり pin を持たない (実 pin は `src/python_run/requirements.txt`)。 4 つ目は Rust の pattern `jpreprocess\s*=\s*"..."` が実際の宣言 `jpreprocess = { version = "0.9", optional = true }` に一致しなかった。 drift 判定ループが「見つかった version」を反復する実装だったため、 **全滅すると 1 度も回らず `[OK] All discoverable dictionary versions are referenced in spec` を出して exit 0** していた。 守っている対象が消えたときに限って緑になる #628 / #629 / #632 と同一クラス。 併せて `model-quality-gate.yml` の `continue-on-error: true` を削除し (元コメント「spec text scan は false negative あり」は当時の実装を指したもの)、 gate 自身の self-test step を追加した
- CI: 同 gate の比較ロジックを **spec を TOML として読み、各 pin を自分の宣言値と突き合わせる**形に作り直した。 旧実装は `version not in spec_text` という spec 全文への部分文字列一致で、 別パッケージの行にある版でも条件を満たしてしまう。 併せて比較モードを導入した: Python の floor は `exact`、 Rust の `jpreprocess = { version = "0.9" }` は Cargo の caret 記法 (`>=0.9.0, <0.10.0`) なので spec の `0.9.1` が範囲内かを見る `prefix`。 両者を同一視すると整合しているペアに drift を報告することになる
- Docs: 上記 gate が機能していれば検出されていた **実 drift を 1 件解消**。 `docs/spec/dictionary-versions.toml` は `pyopenjtalk-plus 0.4.1.post7` を宣言していたが、 `src/python_run/requirements.txt` は PR #410 (Dependabot) 以降 `>=0.4.1.post8`、 `cmake/ExternalDeps.cmake:142` も `post8` の sdist を落としていた。 spec 側の `version` / `sdist_url` / `sdist_sha256` / 本文注記 / cpp 欄を post8 に更新し、 `meta.last_updated` を 2026-08-26 に更新。 新しい `sdist_sha256` (`f4dfbfbe…`) は sdist を実際に取得して算出し、 `cmake/ExternalDeps.cmake` の `URL_HASH SHA256=` と一致することを確認済み
- CI: `scripts/check_ort_versions.py` の `FLOOR_TARGETS` が **中身の空な target を無言で通していた**のを修正。 「ファイルが存在しないこと」は hard failure だった一方、 「ファイルはあるが ORT pin を 1 つも含まないこと」は `(no matches)` と表示して素通りする非対称があった。 `src/python_run/setup.py` は Issue #418 で pin を失って以来この状態で、 gate は 12 target を検査すると称しながら実質 11 だった。 同 target を `FLOOR_TARGETS` から削除し (理由をコメントに明記)、 併せて **pin が 0 件の target を hard failure に**した — 対象が消えたときこそ緑になる経路を塞ぐため
...
- C++ / C API: 複数回の推論を集約する経路で phoneme timing が全て破棄され、 `--output-timing` が何も書き出さず `piper_plus_get_phoneme_timing` が常に失敗する問題を修正した (#652)。 `textToAudio` / `textToAudioFloat` は phrase ごとの `piper::SynthesisResult` から `audioSeconds` / `inferSeconds` だけを積算し、 `phonemeTimings` / `hasTimingInfo` を呼び出し元の result へ一度も転送していなかった (`phraseResults` は sentence loop 内のローカル変数なので iteration ごとに破棄される)。 `phonemesToAudioFloat` は `phraseIdx == 0` の 1 unit 分だけを代入しており (「textToAudioFloat の経路と同じ」と書かれたコメント自体が誤りだった)、 `textToAudioStreaming` / `phonemesToAudioStreaming` は per-unit result を丸ごと捨てていた。 症状は、 `--debug` に `Extracted timing for N phonemes` が出るのに `main.cpp` の `result.hasTimingInfo` gate が false のままなので `--output-timing FILE` がファイルを作らず `Wrote phoneme timing` も出ない、 C API では `piper_plus_get_phoneme_timing` が rc=-1 と `No timing information available (model may not support duration output)` を返し続ける、 というもの (dev で動いていたのは `--raw-phonemes` = `phonemesToAudio` 経路と、 事前音素化に成功した場合の C API イテレータ `synth_next` = `phonemesToAudioFloat` 経路の先頭 phrase 1 unit 分だけだった)。 オフセット方針は**実出力サンプル基準**を採用した: 1 回の synthesize 呼び出しで出力バッファが実際に増えた サンプル数と、 その後に挿入される phrase 間 / sentence 間の無音サンプル数を積算し、 秒と frame に換算して 次 unit の timing に加算する。 これにより timing 値は「呼び出し元が受け取る WAV 上の位置」になり (#655 が絡む場合の精度は後述)、 先頭 unit のオフセットは厳密に 0 なので単一 unit の出力は従来とビット単位で同一である。 `phraseResults[i].audioSeconds` は使えない: `synthesize` / `synthesizeFloat` の trim 分岐が累積バッファ全体から `audioSeconds` を再代入するため (#654)、 3 unit の発話で実長 1.1907 秒に対し 1.8224 秒 (1.53 倍) を報告する。 Python 側の「durations 合計をオフセットにする」方式 (`src/python_run/piper_plus/voice.py`) も意図的に踏襲していない: ONNX が `durations` を `torch.ceil` 適用前の値で出力しているため (#653) 合計は実長の 0.55-0.83 倍にしかならず、 779 ms の発話では 3 unit 目で -140 ms ずれる。 `docs/spec/phoneme-timing-contract.toml` に `[concatenation]` 節を 追加して本規則を明文化し、 Python との差異を `divergence` として記録した (Python 側の移行は別途)。 残差として #655 (`trimPaddingByDurations` が累積バッファの先頭を削るため、 padding される非先頭 unit があると 先行 unit の音声内容が ~23-35 ms 左にずれる) は未修正: オフセットは全経路でサンプル数として正確、 単一 unit / padding されない後続 unit / streaming 2 経路では内容としても正確であり、 #655 が修正されても 本方式の式は変更不要である。 検出漏れの原因も同時に塞いだ: `test_c_api_integration` の `TimingAfterSynthesis` は timing を `if (rc == PIPER_PLUS_OK)` の内側でしか検査していなかったため、 rc が常にエラーでも `[ PASSED ]` を 報告していた (#628 / #629 / #632 と同じ「検査対象が消えると gate が緑になる」類型)。 この条件分岐を撤去して無条件 assert に書き換え、 fixture model が `durations` 出力を宣言していることを 検査する anti-vacuity gate、 複数 unit にまたがって timing が連続することを検査するテスト、 text 経路と streaming 経路を直接叩く C++ テスト、 offset 演算だけを model 無しで検査するテストを追加した。 併せて `_build-test-cpp.yml` のテストモデル存在確認を integration test 実行時限定から常時実行に変更し、 モデルが欠けた場合に全 model テストが黙って GTEST_SKIP に劣化する経路も塞いだ。 また CLI は stdin 全行で 1 個の `SynthesisResult` を共有し timing ファイルを行ごとに書き直すため、 `processLine` の入口で timing フィールドを行単位にリセットするようにした (SSML 行は per-segment の result で合成して呼び出し元の timing を触らないので、 これが無いと直前の平文行の音素が SSML 行の timing として書き出される)
- CI: `src/python_run/tests/` の 7 ファイル (131 テスト) がどの workflow からも参照されておらず CI で 1 度も実行されていなかったのを修正した。 `python-tests.yml` が pytest にディレクトリではなくファイルを明示列挙しているため、 新規テストファイルは追加しても CI に載らない構造だった (`test_download_resilience` / `test_french_phonemization` / `test_json_input_cli` / `test_pt_dialect_parity` / `test_swedish_lid_runtime` / `test_voice_speaker_embedding` / `test_zero_shot_parity_fixture`)。 7 ファイルを列挙に追加し、 併せて `scripts/check_python_run_test_coverage.py` を新設して 「`src/python_run/tests/test_*.py` が 1 つ以上の workflow から参照されていること」を pre-commit / CI で強制した (#652 / #659 と同じ「検査対象が gate の視野外にあると緑になる」類型を構造的に塞ぐ)
- C++: `synthesize()` の NEON 経路が `audioBuffer.resize(audioCount)` で呼び出し元の累積バッファを切り詰めてから index 0 に書き込んでいたのを、 既存サンプルの後ろへ append するよう修正した。 `USE_ARM64_NEON` は `cmake/CompilerSettings.cmake` でコメントアウトされており 現在は到達しないが、 再有効化すると連結音声と PR #658 の実出力サンプル基準オフセットの両方が壊れる。 同経路の `reserve(audioCount)` も `synthesizeFloat` と同じ `reserve(size() + audioCount)` に揃えた
- C++: `phoneme_timing_concat.hpp` の `appendShifted` が unit ごとに正確サイズで `reserve` していたため、 `reserve` が要求分だけを確保する実装では unit 数に対して再確保とコピーが二次オーダーになっていた。 容量不足時のみ、 かつ最低 2 倍で確保するようにして vector 本来の償却を保つようにした
- C++: `piper.cpp` に 3 箇所重複していた `hop_size` 解決 (`configRoot["audio"]["hop_size"]` else 256) を 既存の `resolveHopSize()` に統一した。 挙動は完全に同一で、 音声出力が 5 形態すべてでサンプル数一致することを確認済み
- Python: `PiperVoice.synthesize_with_timing()` の複数文 timing オフセットを実出力サンプル基準に移行した (#660)。 従来は `cumulative_ms += timing.total_duration_ms` で durations の合計だけオフセットを進めていたが、 ONNX の `durations` は `torch.ceil` 適用前の値なので (#653) 合計は実際に書き出した音声の 0.55-0.83 倍にしかならず、 文が進むごとに timestamp が前へずれていた (3 unit の発話で 3 文目が -140 ms)。 書き出した PCM サンプル数 (音声 + 文間無音) を積算してオフセットにするよう変更し、 PR #658 で移行済みの C++ と規則が一致した。 **挙動変更**: 複数文入力の `start_ms` / `end_ms` が変わる (実音声上の位置に一致するようになる)。 単一文入力の per-phoneme 値は従来どおり (先頭 unit のオフセットは 0)。 `TimingResult.total_duration_ms` は「出力ストリーム長」の定義に変わるため単一文でも値が変わる (従来は per-unit cursor 走破長の合計で、 #653 の分だけ実音声より短かった)。 `durations_to_timing()` 単体の挙動と parity fixture は不変。 `docs/spec/phoneme-timing-contract.toml` の `[concatenation].divergence` を解消し、 集約 total の定義を `aggregate_total_duration_ms` として明文化した
- C++ CLI: `--ssml` および SSML 自動検出経路が音声を一切出力せず、 `<break>` の無音だけを書き出していた問題を修正した (#659)。 `synthesizeSsmlToBuffer` が `piper::textToAudio` に `[]() {}` を audioCallback として渡していたため。 `textToAudio` は callback が入っていると「callback 側が既にサンプルをコピーした」という前提で文ごとに出力バッファを `clear()` するので、 何もしない lambda では segment ごとのバッファが空のまま返り、 `insert` が 0 サンプルを追加していた。 実測では `<speak>Hola mundo</speak>` が **0 フレーム**、 `<speak>Hola<break time="200ms"/>mundo</speak>` が **4410 フレーム (= 200ms の無音のみ)** になっていた (PR #477 で SSML CLI 経路が追加された時点から一貫して無音)。 あわせて `result.audioSeconds` を segment が実際に出力したサンプル数から算出するようにした (従来は `segResult.audioSeconds` を加算しており、 音声 0 サンプルなのに `audio=0.476 sec` と報告していた。 この値は #654 の影響も受ける)。 回帰テストのために合成ループを `src/cpp/ssml_synth.{hpp,cpp}` へ切り出した: 従来は main.cpp 内の `static` 関数でテストから到達できず、 既存の `test_ssml.cpp` は parser だけを検証していたため 「parser は正しいが出力が無音」という状態を検出できなかった
- CI: Android 系ワークフローが `Setup Android NDK` step で `sdkmanager` exit 1 により全 ABI とも失敗していた問題を修正した。 `android-actions/setup-android` の `packages` 既定値が `tools platform-tools` であり、 このうち `tools` は Android SDK リポジトリから削除済みのため `Warning: Failed to find package 'tools'` で異常終了する (runner image の cmdline-tools が 12.0 に更新されて露出した。 リポジトリ内の 9 箇所すべてが `packages` を明示しておらず既定値を継承していた)。 `packages: platform-tools` を明示して廃止済みパッケージを要求しないようにした。 対象は `android-build.yml` / `kotlin-g2p-ci.yml` (5 箇所) / `release-kotlin-g2p.yml` (2 箇所) / `codeql.yml`。 `release-kotlin-g2p.yml` も同じ既定値を継承していたため、 放置するとリリース公開時に同じ地点で失敗していた
