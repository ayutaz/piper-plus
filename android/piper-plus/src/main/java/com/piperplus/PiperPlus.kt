...
class PiperPlus private constructor(
    ...
    val sampleRate: Int
        get() {
            checkNotClosed()
            return PiperPlusNative.nativeSampleRate(nativeHandle)
        }
    ...
    val numSpeakers: Int
        get() {
            checkNotClosed()
            return PiperPlusNative.nativeNumSpeakers(nativeHandle)
        }
    ...
    val numLanguages: Int
        get() {
            checkNotClosed()
            return PiperPlusNative.nativeNumLanguages(nativeHandle)
        }
    ...
    fun synthesizeStream(text: String, speakerId: Int = 0): Flow<ShortArray> = flow {
        ...
        while (true) {
            val chunk = PiperPlusNative.nativeSynthNext(nativeHandle) ?: break
            emit(chunk)
        }
