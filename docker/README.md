...
|---------|-----------|---------------|------|-----|
| Python 推論 | `docker/python-inference/Dockerfile` | `nvidia/cuda:12.6.3-cudnn-runtime-ubuntu22.04` | ONNX モデルによる CPU/GPU 推論 | GPU 対応（CPU/GPU 両対応） |
| Python 推論 (CPU) | `docker/python-inference/Dockerfile.cpu` | `python:3.11-slim-bookworm` | ONNX モデルによる CPU 推論 (arm64+amd64) | 不要 |
| Python 学習 | `docker/python-train/Dockerfile` | `nvidia/cuda:12.6.3-cudnn-devel-ubuntu22.04` | モデル学習 (multi-stage) | 必要 |
| WebUI | `docker/webui/Dockerfile` | `python:3.13.13-slim-trixie` | Gradio ベースの Web インターフェース | 不要 |
