...
| C++ 推論 | `docker/cpp-inference/Dockerfile` | `nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04` | C++ バイナリによる推論 (multi-stage) | 必要 |
| C++ 開発 | `docker/cpp-train/Dockerfile` | `nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04` | C++ ビルド・デバッグ環境 | 任意 |
...
C++ バイナリ (`piper`) による推論環境です。espeak-ng、piper-phonemize、OpenJTalk をビルドステージで構築し、ランタイムステージにコピーする multi-stage ビルドです。
...
```bash
echo "こんにちは" | docker run -i --rm --gpus all \
  -v $(pwd)/models:/app/models:ro \
  -v $(pwd)/output:/app/output \
  piper-cpp \
  piper --model /app/models/model.onnx --output_file /app/output/output.wav
```
...
docker run -i --rm --gpus all \
  ...
  piper-cpp \
  piper --output_file /app/output/output.wav
```

## C++ 開発環境

CMake、Ninja、clang、gdb、valgrind 等の開発ツールを含むフル装備の開発環境です。ccache によるビルドキャッシュをサポートします。
...
Python 学習イメージと C++ イメージは NVIDIA GPU を使用します。
...
| `NVIDIA_VISIBLE_DEVICES` | GPU イメージ | GPU デバイス選択 |
| `GRADIO_SERVER_NAME` | WebUI | サーバーバインドアドレス (デフォルト: `0.0.0.0`) |
| `GRADIO_SERVER_PORT` | WebUI | サーバーポート (デフォルト: `7860`) |
| `CUDA_VISIBLE_DEVICES` | GPU イメージ | CUDA デバイス選択 (ONNX 変換時は `""` を指定) |
...
# C++ 開発
docker pull ghcr.io/ayutaz/piper-plus/cpp-train:main
```
...
- CUDA バージョンの不一致が疑われる場合は `nvidia-smi` でドライバーバージョンを確認してください。
