...
| Wyoming | `docker/wyoming/Dockerfile` | `python:3.11-slim-bookworm` (multi-stage) | Home Assistant Wyoming Protocol TTS | 不要 |
| Go | `src/go/docker/Dockerfile` | `golang:1.22 -> debian:trixie-slim` (multi-stage, multi-arch amd64/arm64) | HTTP API サーバー + CLI (piper-plus-go), `serve` サブコマンド対応 | 不要 |
