...
| C++ 推論 | `docker/cpp-inference/Dockerfile` | `ubuntu:22.04` → `nvidia/cuda:12.4.1-cudnn-runtime-ubuntu22.04` | C++ バイナリによる推論 (multi-stage) | 必要 |
...
C++ バイナリ (`piper`) による推論環境です。CMake ExternalProject で全依存関係（piper-phonemize, espeak-ng, OpenJTalk 等）を自動ビルドし、ランタイムステージにコピーする multi-stage ビルドです。ONNX Runtime GPU はビルド時のバージョンを自動検出してインストールします。
