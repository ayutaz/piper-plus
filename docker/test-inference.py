...
def test_python_inference():
    ...
    try:
        ...
        if os.path.exists(model_path) and os.path.exists(config_path):
            ...
            input_ids = np.array(
                [[1, 20, 18, 24, 24, 27, 3, 35, 27, 30, 24, 17, 2]], dtype=np.int64
            )
            scales = np.array(
                [
            ...
            # Run inference
            input_dict = {inputs[0].name: input_ids, inputs[1].name: scales}
            ...
            print(
                f"  Audio duration: {audio.shape[-1] / config['audio']['sample_rate']:.2f} seconds"
            )
