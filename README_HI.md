...
> **📢 v1.12.0 ब्रेकिंग चेंजेस (2026-05):** HiFi-GAN डिकोडर हटाया गया (MB-iSTFT में एकीकृत, `--mb-istft` फ़्लैग बंद) / Flask → FastAPI HTTP सर्वर / HTS-voice निर्भरता हटाई गई (केवल Python रनटाइम) / Unity UPM अलग रिपॉज़िटरी में स्थानांतरित (`ayutaz/uPiper`) / सभी .NET प्रोजेक्ट `net10.0` LTS पर अपग्रेड किए गए। विवरण: [docs/migration/v1.11-to-v1.12.md](docs/migration/v1.11-to-v1.12.md)
...
- **8 भाषाओं का समर्थन** — जापानी, अंग्रेज़ी, चीनी, स्पेनी, फ़्रेंच, पुर्तगाली, स्वीडिश, कोरियाई (ja=0, en=1, zh=2, es=3, fr=4, pt=5, sv=6, ko=7) *प्रशिक्षित मॉडल 6 भाषाओं (JA/EN/ZH/ES/FR/PT) को कवर करता है*
...
|---|---|---|
| Linux | x86_64 / ARM64 / ARMv7 | पूर्ण समर्थन |
...
मुख्य विकल्प: `--speaker-id` (वक्ता ID), `--device auto|cpu|gpu`, `--noise-scale` (ध्वनि भिन्नता), `--noise-scale-w` (फ़ोनीम लंबाई भिन्नता, डिफ़ॉल्ट: 0.8), `--length-scale` (बोलने की गति)
...
> **नोट:** webui इमेज CI द्वारा स्वचालित रूप से नहीं बनाई जाती है। मैनुअल बिल्ड करें: docker build -t piper-webui -f docker/webui/Dockerfile .
...
- **Linux**: निर्भरताएँ (ONNX Runtime, OpenJTalk आदि) CMake द्वारा स्वचालित डाउनलोड होती हैं
...
| `--config/-c PATH` | कॉन्फ़िगरेशन फ़ाइल पथ | - |
| `--output_file/-f PATH` | आउटपुट WAV फ़ाइल पथ | - |
| `--output_dir/-d DIR` | आउटपुट डायरेक्टरी | - |
| `--output-raw` | stdout पर raw PCM ऑडियो आउटपुट | बंद |
| `--language/-l CODE` | भाषा कोड | - |
| `--timing-format FMT` | टाइमिंग आउटपुट प्रारूप json/tsv | - |
| `--test-mode` | टेस्ट मोड, ONNX इंफ़रेंस स्किप | बंद |
| `--debug` | डीबग लॉगिंग सक्षम करें | बंद |
| `--quiet/-q` | लॉगिंग अक्षम करें | बंद |
...
# WavLM मॉडल (--stochastic डिफ़ॉल्ट रूप से सक्षम)
CUDA_VISIBLE_DEVICES="" uv run python -m piper_train.export_onnx \
...
`scripts/evaluation/` में मूल्यांकन के लिए परीक्षण टेक्स्ट हैं।
...
- **[तकनीकी विवरण और एकीकरण गाइड](src/wasm/openjtalk-web/README.npm.md)**
...
piper-plus मॉडल: [piper-plus-base](https://huggingface.co/ayousanz/piper-plus-base) (6-भाषा बेस) · [Tsukuyomi-chan](https://huggingface.co/ayousanz/piper-plus-tsukuyomi-chan)

> **नोट:** piper-plus अपनी स्वयं की G2P और फ़ोनीम प्रणाली का उपयोग करता है, इसलिए upstream Piper के मॉडल (rhasspy/piper-voices) संगत नहीं हैं।
...
### piper-plus-g2p (स्वतंत्र G2P पैकेज)

बहुभाषी G2P (Grapheme-to-Phoneme) स्वतंत्र पैकेज के रूप में उपलब्ध:

- **Python**: `pip install piper-plus-g2p` — [सोर्स कोड](src/python/g2p/)
- **Rust**: `cargo add piper-plus-g2p` — [सोर्स कोड](src/rust/piper-plus-g2p/)
- **Go**: `go get github.com/ayutaz/piper-plus/src/go/phonemize` — [सोर्स कोड](src/go/phonemize/)
- **JavaScript/WASM**: `npm install @piper-plus/g2p` — [सोर्स कोड](src/wasm/g2p/)
