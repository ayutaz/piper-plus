...
//!
//! Loads the shared fixture (`tests/fixtures/swedish_lid_matrix.json`, mirrored
//! byte-for-byte from the canonical `tests/fixtures/g2p/swedish_lid_matrix.json`
//! by `scripts/check_swedish_lid_consistency.py`) and verifies that
//! [`piper_plus_g2p::multilingual::segment_text`] agrees with each case's
//! `expect_contains_sv` flag.
//!
...
//! piper-core / Python / Go / C++ / C# / WASM consume the *same* fixture, so
//! cross-runtime agreement on these cases is the parity proof.
//!
//! Mirror of `test_fixture_matrix_loadable_and_well_formed` (zh_en loanword).
