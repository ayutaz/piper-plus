    ...
    let result = std::panic::catch_unwind(|| {
        ...
        let langs: Vec<&str> = if languages.is_null() {
            ...
            match unsafe { CStr::from_ptr(languages) }.to_str() {
                Ok(s) if s.is_empty() => vec!["en", "es", "fr", "pt", "sv"],
...
pub unsafe extern "C" fn piper_g2p_phonemize(
    ...
    if handle.is_null() || text.is_null() { return ptr::null_mut(); }
    let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
        let h = unsafe { &*handle };
        let text = unsafe { CStr::from_ptr(text) }.to_str().ok()?;
        let lang = if language.is_null() { "en" } else {
    ...
    match result { Ok(Some(s)) => s.into_raw(), _ => ptr::null_mut() }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn piper_g2p_available_languages(handle: *const PiperG2pHandle) -> *mut c_char {
...
fn register_one(registry: &mut PhonemizerRegistry, lang: &str) -> Result<(), crate::G2pError> {
    match lang {
        #[cfg(feature = "english")]
        "en" => { registry.register("en", Box::new(crate::english::EnglishPhonemizer::new()?)); }
        #[cfg(feature = "chinese")]
        "zh" => {
        ...
        #[cfg(feature = "korean")]
        "ko" => { registry.register("ko", Box::new(crate::korean::KoreanPhonemizer::new())); }
        #[cfg(feature = "spanish")]
        "es" => { registry.register("es", Box::new(crate::spanish::SpanishPhonemizer::new())); }
        #[cfg(feature = "french")]
        "fr" => { registry.register("fr", Box::new(crate::french::FrenchPhonemizer::new())); }
        #[cfg(feature = "portuguese")]
        "pt" => { registry.register("pt", Box::new(crate::portuguese::PortuguesePhonemizer::new())); }
        #[cfg(feature = "swedish")]
        "sv" => { registry.register("sv", Box::new(crate::swedish::SwedishPhonemizer::new())); }
        #[cfg(feature = "japanese")]
        "ja" => { registry.register("ja", Box::new(crate::japanese::JapanesePhonemizer::new()?)); }
        _ => { return Err(crate::G2pError::UnsupportedLanguage { code: lang.to_string() }); }
    }
    Ok(())
}
