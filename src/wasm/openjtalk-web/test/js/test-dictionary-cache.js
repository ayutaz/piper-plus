  ...
  describe("fetchAndCacheDictionary()", () => {
    ...
    it("fetch 失敗時にエラーがスローされる", async () => {
      ...
      await assert.rejects(
        () =>
          mgr.fetchAndCacheDictionary("https://example.com/missing-dict.tar.gz", "missing-dict"),
        (err) => {
    ...
    it("ネットワークエラー時にエラーがスローされる", async () => {
      ...
      await assert.rejects(
        () => mgr.fetchAndCacheDictionary("https://example.com/unreachable.tar.gz", "unreachable"),
    ...
    it("fetch 失敗後もキャッシュは汚染されない", async () => {
      ...
      try {
        await mgr.fetchAndCacheDictionary("https://example.com/bad-dict.tar.gz", "bad-dict");
      } catch {
        // expected
      }
