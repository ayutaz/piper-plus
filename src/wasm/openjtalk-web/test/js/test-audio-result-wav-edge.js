...
describe("AudioResult WAV 生成エッジケース", { skip }, () => {
  ...
  describe("極値サンプルデータ", () => {
    it("全サンプルが 0.0 の場合の WAV データ", () => {
      ...
      // Assert — 全 Int16 値が 0
      assert.ok(
        Array.from({ length: numSamples }, (_, i) => readInt16(wav, i)).every((v) => v === 0)
      );
    });
    ...
    it("全サンプルが 1.0 の場合の WAV データ", () => {
      ...
      assert.ok(
        Array.from({ length: numSamples }, (_, i) => readInt16(wav, i)).every((v) => v === 32767)
      );
    });
    ...
    it("全サンプルが -1.0 の場合の WAV データ", () => {
      ...
      assert.ok(
        Array.from({ length: numSamples }, (_, i) => readInt16(wav, i)).every((v) => v === -32768)
      );
    });
  });
  ...
  describe("正弦波整合性", () => {
    it("正弦波データの WAV 変換整合性", () => {
      ...
      assert.ok(
        Array.from({ length: numSamples }, (_, i) => readInt16(wav, i)).some((v) => v !== 0)
      );
    });
  });
});
