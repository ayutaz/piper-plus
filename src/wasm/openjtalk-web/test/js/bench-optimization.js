...
import {
  ...
  ChunkCrossfader,
} from "../../src/streaming-pipeline.js";
...
for (const duration of [0.5, 1.0, 3.0, 5.0, 10.0]) {
  ...
  console.log(
    `  ${duration}s (${inputSamples}→${outputSamples}サンプル): ${avgMs.toFixed(2)}ms (平均${iterations}回)`
  );
}
console.log();
...
// プールなし: 毎回new Float32Array
{
  ...
  console.log(
    `  プールなし (${ALLOC_ITERATIONS}回 new Float32Array(${ARRAY_SIZE})): ${elapsed.toFixed(2)}ms`
  );
}
...
// プールあり: get/return サイクル
{
  ...
  console.log(
    `    hits: ${stats.hits}, misses: ${stats.misses}, hit率: ${((stats.hits / (stats.hits + stats.misses)) * 100).toFixed(1)}%`
  );
}
console.log();
...
const jaText =
  "これは長いテキストです。音声合成では文ごとに区切って処理します。そうすることで、最初の文の音声を再生しながら、次の文を並列で処理できます。これにより体感レイテンシが大幅に低下します。ストリーミング再生の基盤技術です。";
const enText =
  "This is a long text for benchmarking. Speech synthesis processes text sentence by sentence. This allows playing the first sentence while processing the next one in parallel. Mr. Smith noted that this reduces perceived latency significantly. Dr. Johnson agreed with the assessment.";

{
  ...
  console.log(
    `  日本語 (${jaText.length}文字→${chunks.length}チャンク): ${((elapsed / iterations) * 1000).toFixed(1)}μs/回 (${iterations}回)`
  );
}

{
  ...
  console.log(
    `  英語 (${enText.length}文字→${chunks.length}チャンク): ${((elapsed / iterations) * 1000).toFixed(1)}μs/回 (${iterations}回)`
  );
}
console.log();
...
// キャッシュミス (初回fetch): ネットワーク遅延シミュレート
{
  ...
  const elapsed = performance.now() - start;
  console.log(
    `  初回ロード (キャッシュミス, ${DICT_FILES.length}ファイル): ${elapsed.toFixed(2)}ms`
  );
}
...
// キャッシュヒット (2回目): IndexedDBから
{
  ...
  const elapsed = performance.now() - start;
  console.log(
    `  2回目ロード (キャッシュヒット, ${DICT_FILES.length}ファイル): ${elapsed.toFixed(2)}ms`
  );
}
...
const testText =
  "今日は良い天気です。明日も晴れるでしょう。来週は雨かもしれません。気温は20度くらいです。過ごしやすい日が続きます。";
...
const chunkSize = 22050; // 1秒分
{
  ...
  const elapsed = performance.now() - start;
  console.log(
    `  50msクロスフェード (${chunkSize}サンプル/チャンク × ${iterations}回): ${elapsed.toFixed(2)}ms`
  );
  console.log(
    `  = ${(elapsed / iterations).toFixed(3)}ms/チャンク (リアルタイム比 ${((elapsed / iterations / ((chunkSize / 22050) * 1000)) * 100).toFixed(2)}%)`
  );
}
console.log();
