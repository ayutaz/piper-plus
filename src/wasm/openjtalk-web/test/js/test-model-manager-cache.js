...
describe("ModelManager キャッシュライフサイクル", { skip }, () => {
  ...
  it("loadModel 後に getFromCache でモデルを取得できる", async () => {
    ...
    globalThis.fetch = createMockFetch(directUrlRoutes(modelUrl, expectedConfig, expectedData));
  ...
  it("clearCache 後に getFromCache は null を返す", async () => {
    ...
    globalThis.fetch = createMockFetch(directUrlRoutes(modelUrl, config, data));
  ...
  it("キャッシュヒット時に fetch が呼ばれない", async () => {
    ...
    const mockFetch = createMockFetch(directUrlRoutes(modelUrl, config, data));
  ...
  it("大きなモデルデータがキャッシュされる", async () => {
    ...
    globalThis.fetch = createMockFetch(directUrlRoutes(modelUrl, config, largeData));
