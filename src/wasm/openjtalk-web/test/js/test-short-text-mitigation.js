...
describe("padPhonemeIds (Strategy A - padding)", { skip }, () => {
  ...
  it("preserves EOS as last element after padding", () => {
    ...
    assert.equal(result.phonemeIds[result.phonemeIds.length - 1], 2, "last element should be EOS");
  });
  ...
  it("pads prosodyFeatures in parallel when present", () => {
    ...
    const prosody = [
      [0, 0, 0],
      ...new Array(bodySize).fill(null).map((_, i) => [i, i + 1, i + 2]),
      [0, 0, 0],
    ];
  ...
  it("padding prosody arrays are independent references (not shared)", () => {
    ...
    for (let i = 1; i < allPad.length; i++) {
      assert.equal(allPad[i][0], 0, `mutating padding[0] should not affect padding[${i}]`);
    }
  });
...
describe("adjustScalesForShortInput (Strategy B)", { skip }, () => {
  ...
  it("applies linear scaling in the mid-range", () => {
    ...
    assert.ok(
      Math.abs(result.noiseScale - expectedNs) < 1e-6,
      `expected ${expectedNs}, got ${result.noiseScale}`
    );
    assert.ok(
      Math.abs(result.noiseW - expectedNw) < 1e-6,
...
describe("synthesize() short-text mitigation integration", { skip }, () => {
  it("applies padding and scale adjustment for short phonemeIds", async () => {
    ...
    assert.equal(
      inputIds.length,
      MIN_PHONEME_IDS,
      `padded phonemeIds should be ${MIN_PHONEME_IDS}`
    );
  ...
  it("applies post-trim when padding was applied", async () => {
    ...
    assert.ok(result.samples.length < 10000, `should be trimmed: got ${result.samples.length}`);
    assert.ok(result.samples.length >= 2205, "should keep at least TRIM_MIN_SAMPLES");
  });
  ...
  it("does NOT trim when no padding was applied", async () => {
    ...
    assert.equal(result.samples.length, 10000, "should NOT trim when not padded");
  });
  ...
  it("Strategy B uses original phoneme count (before padding) for ratio", async () => {
    ...
    const len = Math.max(2 + MIN_BODY_FOR_STRATEGY_A, Math.floor(MIN_PHONEME_IDS / 2) + 1);
    ...
    assert.ok(
      Math.abs(scales[2] - expectedNoiseW) < 1e-4,
