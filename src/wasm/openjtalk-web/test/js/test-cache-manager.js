...
describe("CacheManager", { skip }, () => {
  ...
  describe("eviction動作", () => {
    it("容量超過時にlow優先度のエントリが先に退避される", async () => {
      ...
      await localCache.set("low1", new ArrayBuffer(20 * 1024 * 1024), {
        version: "v1",
        priority: "low",
      });
      await localCache.set("low2", new ArrayBuffer(20 * 1024 * 1024), {
        version: "v1",
        priority: "low",
      });
      // Add a high-priority item that pushes over quota
      await localCache.set("high1", new ArrayBuffer(15 * 1024 * 1024), {
        version: "v1",
        priority: "high",
      });
    ...
    it("high優先度のエントリはevictionの対象にならない", async () => {
      const localCache = new CacheManager({ dbFactory: () => new MockIndexedDB() });
      await localCache.set("high1", new ArrayBuffer(25 * 1024 * 1024), {
        version: "v1",
        priority: "high",
      });
      await localCache.set("high2", new ArrayBuffer(25 * 1024 * 1024), {
        version: "v1",
        priority: "high",
      });
      ...
      await assert.rejects(
        () =>
          localCache.set("extra", new ArrayBuffer(5 * 1024 * 1024), {
            version: "v1",
            priority: "medium",
          }),
        /quota/i
    ...
    it("同一優先度内では古いエントリが先に退避される", async () => {
      const localCache = new CacheManager({ dbFactory: () => new MockIndexedDB() });
      await localCache.set("old", new ArrayBuffer(20 * 1024 * 1024), {
        version: "v1",
        priority: "low",
      });
      // Slight delay to ensure different storedAt
      await localCache.set("new", new ArrayBuffer(20 * 1024 * 1024), {
        version: "v1",
        priority: "low",
      });
      // Trigger eviction
      await localCache.set("trigger", new ArrayBuffer(15 * 1024 * 1024), {
        version: "v1",
        priority: "medium",
      });
