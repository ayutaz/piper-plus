...
describe("StreamingTTSPipeline", { skip }, () => {
  ...
  it("音素化と推論がパイプライン並列化されている", async () => {
    ...
    if (ph2Start && syn1End) {
      assert.ok(
        ph2Start.time <= syn1End.time,
        "Chunk2 phonemization should overlap with chunk1 synthesis"
      );
    }
  });
  ...
  it("コールバックが関数でない場合TypeErrorをスローする", () => {
    assert.throws(
      () =>
        new StreamingTTSPipeline({
          phonemize: "not a function",
          synthesize: async () => {},
          onAudioChunk: () => {},
        }),
      { name: "TypeError" }
