 ...
 * uses the correct data type (float32) to match Python ONNX export/inference.
 */
...
TEST(ProsodyInferenceTest, ProsodyTensorDataType) {
    // Verify ONNX tensor is created with float type (not int64)
    std::vector<float> prosodyData = {-2.0f, 1.0f, 5.0f, 0.0f, 2.0f, 5.0f};
    ...
    EXPECT_EQ(typeInfo.GetElementType(), ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT);
...
TEST(ProsodyInferenceTest, IntToFloatConversion) {
    // Test conversion from ProsodyFeature (int) to float array
    ...
    std::vector<float> prosodyFlat;
    prosodyFlat.resize(prosody.size() * 3);

    for (size_t i = 0; i < prosody.size(); i++) {
        prosodyFlat[i * 3 + 0] = static_cast<float>(prosody[i].a1);
        prosodyFlat[i * 3 + 1] = static_cast<float>(prosody[i].a2);
        prosodyFlat[i * 3 + 2] = static_cast<float>(prosody[i].a3);
    }

    EXPECT_EQ(prosodyFlat.size(), 9);
    EXPECT_FLOAT_EQ(prosodyFlat[0], -4.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[1], 1.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[2], 5.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[3], -3.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[4], 2.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[5], 5.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[6], 0.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[7], 3.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[8], 5.0f);
}
...
TEST(ProsodyInferenceTest, InterspersePaddingMapping) {
    ...
    std::vector<float> prosodyFlat(numPhonemeIds * 3, 0.0f);

    // Map to odd positions (1, 3, 5) - intersperse padding logic
    size_t prosodyIdx = 0;
    for (size_t i = 1; i < numPhonemeIds && prosodyIdx < prosody.size(); i += 2) {
        prosodyFlat[i * 3 + 0] = static_cast<float>(prosody[prosodyIdx].a1);
        prosodyFlat[i * 3 + 1] = static_cast<float>(prosody[prosodyIdx].a2);
        prosodyFlat[i * 3 + 2] = static_cast<float>(prosody[prosodyIdx].a3);
        prosodyIdx++;
    }

    // Verify padding positions are zero (indices 0, 2, 4, 6)
    EXPECT_FLOAT_EQ(prosodyFlat[0], 0.0f);  // PAD at index 0
    EXPECT_FLOAT_EQ(prosodyFlat[1], 0.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[2], 0.0f);

    // Verify phoneme 1 at index 1 (odd position)
    EXPECT_FLOAT_EQ(prosodyFlat[3], -2.0f);  // a1
    EXPECT_FLOAT_EQ(prosodyFlat[4], 1.0f);   // a2
    EXPECT_FLOAT_EQ(prosodyFlat[5], 3.0f);   // a3

    // Verify PAD at index 2 (even position)
    EXPECT_FLOAT_EQ(prosodyFlat[6], 0.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[7], 0.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[8], 0.0f);

    // Verify phoneme 2 at index 3 (odd position)
    EXPECT_FLOAT_EQ(prosodyFlat[9], 0.0f);   // a1
    EXPECT_FLOAT_EQ(prosodyFlat[10], 2.0f);  // a2
    EXPECT_FLOAT_EQ(prosodyFlat[11], 3.0f);  // a3

    // Verify PAD at index 4 (even position)
    EXPECT_FLOAT_EQ(prosodyFlat[12], 0.0f);

    // Verify phoneme 3 at index 5 (odd position)
    EXPECT_FLOAT_EQ(prosodyFlat[15], 1.0f);  // a1
    EXPECT_FLOAT_EQ(prosodyFlat[16], 3.0f);  // a2
    EXPECT_FLOAT_EQ(prosodyFlat[17], 3.0f);  // a3
}
...
TEST(ProsodyInferenceTest, NegativeAndPositiveValues) {
    ...
    EXPECT_FLOAT_EQ(prosodyFlat[0], -10.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[1], 1.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[2], 15.0f);

    // Verify zero value
    EXPECT_FLOAT_EQ(prosodyFlat[3], 0.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[4], 5.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[5], 10.0f);

    // Verify positive value
    EXPECT_FLOAT_EQ(prosodyFlat[6], 10.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[7], 10.0f);
    EXPECT_FLOAT_EQ(prosodyFlat[8], 10.0f);
}
...
TEST(ProsodyInferenceTest, ZeroProsodyFallback) {
    ...
    std::vector<float> zeroProsody;
    zeroProsody.resize(numPhonemeIds * 3, 0.0f);

    EXPECT_EQ(zeroProsody.size(), 15);
    for (size_t i = 0; i < zeroProsody.size(); i++) {
        EXPECT_FLOAT_EQ(zeroProsody[i], 0.0f);
    }
