...
//
// Mirrors src/python_run/piper_plus/voice.py::_resolve_g2p_parallelism.
...
// going wider tends to oversubscribe physical cores. PIPER_PLUS_G2P_PARALLELISM
...
// * PIPER_PLUS_G2P_PARALLELISM=1   → returns 1 (legacy serial path).
// * PIPER_PLUS_G2P_PARALLELISM=N>=2 → returns min(N, nSentences).
...
inline int resolveG2pParallelism(std::size_t nSentences,
    ...
    const char *env = envOverride ? envOverride : std::getenv("PIPER_PLUS_G2P_PARALLELISM");
    if (env) {
        const char *p = env;
        while (*p == ' ' || *p == '\t') ++p;
        if (*p != '\0') {
