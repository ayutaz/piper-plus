...
def duration_consistency_loss(pred_durations, text_lengths, phoneme_ids=None):
    ...
    if pred_durations.size(1) > 1:
        ...
        smoothness_loss = (
            (duration_diff_adjacent**2) * adjacent_mask
        ).sum() / adjacent_mask.sum()
    else:
    ...
    if phoneme_ids is not None:
        # Penalize very short durations for vowels
        # (assuming vowel IDs are in a certain range)
        # This is a simplified example - in practice,
        # you'd have proper phoneme categories
        ...
        phoneme_penalty = (short_duration_mask.float() * vowel_mask).sum() / (
            vowel_mask.sum() + 1e-5
        )
