...
class CustomDictionary:
    ...
    def __init__(self, dict_paths: Optional[Union[str, List[str]]] = None):
        """
        Args:
            dict_paths: 辞書ファイルのパス（単一または複数）
        """
        self.entries: Dict[str, Dict[str, Union[str, int]]] = {}
        self.case_sensitive_entries: Dict[str, Dict[str, Union[str, int]]] = {}
        self.pattern_cache: Dict[str, re.Pattern] = {}
        
        # デフォルト辞書のパスを設定
        self.default_dict_dir = Path(__file__).parent.parent.parent.parent.parent / "data" / "dictionaries"
        ...
        for dict_name in default_dicts:
            ...
            if dict_path.exists():
                try:
                    ...
                    print(f"Warning: Failed to load default dictionary {dict_path}: {e}")
    ...
    def _add_entry(self, word: str, entry: Dict[str, Union[str, int]]) -> None:
        """エントリを辞書に追加
    ...
    def apply_to_text(self, text: str) -> str:
        ...
        for word, entry in sorted(self.case_sensitive_entries.items(), 
...
def apply_custom_dictionary(text: str, dict_paths: Optional[Union[str, List[str]]] = None) -> str:
