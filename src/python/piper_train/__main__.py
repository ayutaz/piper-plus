...
def create_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Gaussian noise sigma for speaker embedding perturbation during "
        "training (default: 0.0 = disabled). v10 F7: the former default 0.05 "
        "is 500x the Latent Filling literature value (sigma=1e-4) and was "
        "measured destructive (Phase 0 Arm D) — pass 0.05 explicitly to "
        "reproduce v9-era runs.",
    )
    parser.add_argument(
        "--d-update-interval",
        type=int,
    ...
    # v10 学習信号 (S1 swap-SCL / SupCon 改良 / Latent Filling、design doc §3)。
    # default は全て off / 現行値 = v9 bit 互換 — opt-in でのみ挙動が変わる。
    parser.add_argument(
        "--c-swap-spk",
        type=float,
        default=0.0,
        help="v10 S1 (swap-SCL, ASCL-style): weight of the speaker loss "
        "computed on a waveform generated by running the flow in reverse "
        "with a SAME-LANGUAGE other speaker's embedding (z_p is detached; "
        "gradients reach flow / decoder / spk_proj only). Routes speaker "
        "supervision through the inference path, making the same-utt "
        "Goodhart shortcut structurally impossible. Requires "
        "--speaker-encoder-torch-path. Default: 0.0 (disabled).",
    )
    parser.add_argument(
        "--swap-spk-start-epoch",
        type=int,
        default=10,
        help="Epoch at which the swap-SCL weight ramp starts (after KL "
        "annealing completes; design doc §3.4). Default: 10.",
    )
    parser.add_argument(
        "--swap-spk-ramp-epochs",
        type=int,
        default=5,
        help="Number of epochs to linearly ramp the swap-SCL weight from 0 "
        "to --c-swap-spk after --swap-spk-start-epoch (0 = step function). "
        "Default: 5.",
    )
    parser.add_argument(
        "--spk-loss-gather",
        action="store_true",
        help="v10 SupCon improvement: gather SCL embeddings across all DDP "
        "ranks before the InfoNCE/SupCon loss via the GRADIENT-PRESERVING "
        "torch.distributed.nn.all_gather (plain dist.all_gather cuts "
        "gradients), multiplying the negative pool by the world size. "
        "No-op without DDP. Default: off.",
    )
    parser.add_argument(
        "--spk-loss-temperature",
        type=float,
        default=0.07,
        help="Softmax temperature of the InfoNCE/SupCon speaker loss "
        "(--spk-loss-type infonce). Default: 0.07 (legacy); the v10 recipe "
        "specifies 0.1 (SupCon literature value).",
    )
    parser.add_argument(
        "--latent-filling-tau",
        type=float,
        default=0.0,
        help="v10 §3.3 (arXiv:2310.03538): probability of a Latent Filling "
        "step. On an LF step the speaker embedding is replaced by a "
        "same-language interpolation (lambda~Beta(0.5,0.5), p=0.5) or "
        "s + N(0, 1e-4) noise (p=0.5), the generator is trained with the "
        "LF consistency loss ONLY (recon/mel/KL/GAN skipped) and the "
        "discriminator update is skipped. The step decision is a "
        "deterministic function of global_step (DDP rank-consistent). "
        "Requires --speaker-encoder-torch-path. Default: 0.0 (disabled; "
        "replaces the removed spk-emb-noise-sigma regularizer, F7).",
    )
    # v10 構造介入 (M1/M2/M3/E1、design doc §4)。default は全て off / 0 で
    # v9 と bit 互換 — opt-in でのみ挙動が変わる。
    parser.add_argument(
        "--speaker-cond-layer",
        type=int,
        default=0,
        help="v10 M2: inject speaker conditioning at the input of the Nth "
        "transformer layer (1-based) of the text encoder, so self-attention "
        "can see speaker identity (VITS2 configuration). 0 (default) keeps "
        "the legacy post-encoder addition (v9 bit-compatible). Recommended: "
        "3 (requires N <= --n-layers).",
    )
    parser.add_argument(
        "--dp-spk-head",
        action="store_true",
        default=False,
        help="v10 M3: route duration-loss gradients into a lightweight "
        "residual speaker head (spk_proj_dp) on the duration predictor "
        "conditioning: g_dp = g.detach() + spk_proj_dp(g.detach()). The "
        "main spk_proj / enc_p stay protected from duration gradients. "
        "Default: off (g fully detached inside the DP, v9 behaviour).",
    )
    parser.add_argument(
        "--use-snac-flow",
        action="store_true",
        default=False,
        help="v10 M1+E2: SNAC-style speaker normalization inside each flow "
        "coupling layer (SN at the forward entrance, SDN at the reverse "
        "exit; statistics predicted from the speaker component only while "
        "the coupling WN is conditioned on the language component only). "
        "The flow logdet is wired into the KL loss. Default: off "
        "(v9 bit-compatible, no new parameters).",
    )
    parser.add_argument(
        "--film-init-std",
        type=float,
        default=0.0,
        help="v10 E1: replace the zero-init of the decoder FiLM stage layers "
        "(cond_layers) with N(0, std) small-Gaussian init (AdaLN-Zero "
        "analysis). 0.0 (default) keeps zero-init (v9 bit-compatible). "
        "Newly added heads (sn_linear / spk_proj_dp) always use N(0, 1e-3) "
...
def main():
    ...
    # v10 構造介入 flags (default 全 off = v9 bit 互換) — 有効時のみ明示 log
    if getattr(args, "speaker_cond_layer", 0) > 0:
        _LOGGER.info(
            "v10 M2 enabled (--speaker-cond-layer=%d): speaker conditioning "
            "injected at the encoder layer input (VITS2 configuration)",
            args.speaker_cond_layer,
        )
    if getattr(args, "dp_spk_head", False):
        _LOGGER.info(
            "v10 M3 enabled (--dp-spk-head): duration gradients routed "
            "through the spk_proj_dp residual head (spk_proj protected)"
        )
    if getattr(args, "use_snac_flow", False):
        _LOGGER.info(
            "v10 M1+E2 enabled (--use-snac-flow): SNAC flow (speaker-only "
            "SN/SDN, lang-only WN conditioning, logdet wired into KL)"
        )
    if getattr(args, "film_init_std", 0.0) > 0:
        _LOGGER.info(
