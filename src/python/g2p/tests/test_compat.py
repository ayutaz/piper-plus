"""Standalone correctness tests for piper_g2p output.

These tests were originally comparing piper_g2p output vs piper_train.phonemize
output. Since piper_train.phonemize has been removed (code moved to piper_g2p),
...
class TestJACorrectness:
    def test_ja_tokens_ipa_to_pua(self):
        """piper_g2p tokens + BOS/EOS + PUA should produce valid phoneme sequence."""
...
class TestENCorrectness:
    def test_en_phonemize_produces_output(self):
        """piper_g2p EN should produce valid phoneme output."""
...
class TestMultilingualIDMap:
    def test_multilingual_8lang_id_map_size(self):
        """piper_g2p multilingual ID map should have expected size."""
        from piper_g2p.encode.id_maps import get_phoneme_id_map
...
class TestZHCorrectness:
    def test_zh_phonemize_produces_output(self):
        """piper_g2p ZH should produce valid phoneme output."""
...
class TestESCorrectness:
    def test_es_phonemize_produces_output(self):
        """piper_g2p ES should produce valid phoneme output."""
...
class TestFRCorrectness:
    def test_fr_phonemize_produces_output(self):
        """piper_g2p FR should produce valid phoneme output."""
...
class TestPTCorrectness:
    def test_pt_phonemize_produces_output(self):
        """piper_g2p PT should produce valid phoneme output."""
...
class TestSVCorrectness:
    def test_sv_phonemize_produces_output(self):
        """piper_g2p SV should produce valid phoneme output."""
...
class TestJAProsodyCorrectness:
    def test_ja_prosody_a1_a2_a3(self):
        """piper_g2p JA prosody should produce valid a1/a2/a3 values."""
        from piper_g2p.japanese import JapanesePhonemizer
